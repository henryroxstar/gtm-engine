...
**Then you run it one of two ways** — **Claude Cowork** (default: open the folder, type prompts in
chat, everything local) or an **advanced self-hosted agent** (autonomous, 24/7, pausing only at the
two human gates). [Two ways to run](#two-ways-to-run) has the full comparison. But first, the fun part:
...
*Skip this if you're here to use it rather than to assess it.* The one-paragraph version:
...
> call, what runs next, and what is allowed to leave the building.

The design bet is that for a well-defined business process, **control flow belongs in code, not in
the model's context**. The engine is a DAG scheduler: it advances a frontier of runnable nodes,
persists a run manifest at every transition, resumes a crashed run from the first incomplete node,
and dispatches fan-out concurrently under a single per-tenant lock. Spend is capped twice, by two
independent mechanisms — a ledger check before every dispatch batch, and a hard per-run ceiling at
the SDK layer. The model is a step *inside* a node; it doesn't decide what happens next, so a bad
generation is a bad draft, not a bad state machine.

The capability boundary is the actual engineering. Every tool call is adjudicated by a policy
callback before it executes — fail-closed, with a declarative deny floor beneath it. But the
load-bearing property is that the dangerous operations aren't *forbidden*, they're
**unrepresentable**: there is no publish tool and no send tool in the model's vocabulary. To publish,
it emits bytes into a gate block; a human approves those exact bytes and Python makes the call to a
destination pinned server-side. Email sequences are staged paused, and the sequencer connector
exposes no resume tool. You can't prompt-inject your way to a capability that isn't in the schema.

**The question worth asking is "what's the blast radius of one bad model turn?"** It's bounded to a
rejected draft — every path to external effect terminates at a human approving exact bytes, and the
tool that would bypass that human doesn't exist in the model's schema.

**What isn't done**, stated plainly: pack-skill reachability is computed but not yet wired into the
live permission callback; output-side DLP/PII moderation is a tracked gap, not a shipped control;
and in advanced mode the mechanical, no-PII nodes route to a non-US/EU inference endpoint.
...
skills run, in what order, under which gates. Three are sequential chains; planning is a **batch** of
independent nodes that run side by side, proving a pack is a *graph*, not necessarily a pipeline. All
...
|---|---|
| **Channels** | LinkedIn post · long-form blog · X thread · podcast · builder-in-public update |
...
| **Flow** | radar → plan → research → studio → publish (sequential chain) |
...
| **After a reply lands** | `inbound-triage` reads it (read-only), classifies intent (P0–P3), and drafts a reply behind the same human gate. Nothing auto-sends |
...
Download this repo and open the folder in **Claude Cowork**, then say `"set me up"`. All the GTM
...
**See onboarding work first** — the whole thing is about two minutes, mostly the engine doing the
reading:
...
`_template` bundle. *(Manual equivalent: `bash scripts/bootstrap.sh`.)* Onboarding through a different
...
| **Media** (image · video · voice) | headless carousels, video, podcast TTS | Gemini · Higgsfield · ElevenLabs keys | **Mode 2** (advanced) | not used in Cowork mode |
...
## GTM skill suite (43 skills — all profile-driven)
...
| Category | Skills |
|---|---|
| **Prospecting** | `prospect`, `market-scan`, `events-tracker`, `draft-outreach`, `email-sequence`, `inbound-triage` |
...
| **GTM planning** | `gtm-planning`, `campaign-plan`, `solution-discovery`, `solution-design`, `solution-scope-check`, `gateway-runbook`, `voice-of-customer` |
| **Risk assessment** | `airq-scan` |
| **Founder journey** | `builder-radar`, `builder-evidence`, `builder-studio` |
| **Operations** | `setup`, `profile-onboard`, `knowledge-refresh`, `outcomes-sync` |
