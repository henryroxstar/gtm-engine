...
*The open-source GTM agent engine — safe by construction.*
...
**GTM Engine is a tool that makes sales, pre-sales, and marketing teams faster and more effective
at their day jobs** — cold prospecting, call prep, account plans, decks, market scans, and
on-brand, multi-platform content (e.g. LinkedIn post, blog article, podcast, pictures) — driven by
real news signal, running as an AI agent that _structurally cannot_ publish, send, or leak on its
own. It doesn't ask for your trust; it's built so it can't overreach.

Two human-approval gates are permanent (nothing sends or publishes without your exact sign-off),
every company's data lives in its own isolated profile (one tenant can never see another's prospects
or drafts), and the agent has no raw HTTP or shell access — every external action goes through a
vetted tool. Most agent frameworks ask you to trust broad permissions; this one is designed so
there's nothing broad to trust.
...
There's no install command to run first — in Cowork mode you open the folder and type a sentence.
One example, drafting a LinkedIn post:

```
You:     "draft my LinkedIn post about [today's news item]"

Engine:  picks a hook angle + format (text / carousel / infographic) from real research
      →  GATE 1 — you approve the plan before anything is drafted
      →  drafts the post, runs it through the content linter (hook length, structure,
         platform rules) automatically — a malformed draft never reaches you
      →  GATE 2 — you approve the exact bytes before anything publishes

You:     "prep me for my call with [account]" / "find prospects in [market]" / "build a
         deck for [account]" — same one-sentence pattern, every skill, your voice, your
...
1. **Two human gates are permanent; nothing publishes itself.** Every workflow pauses at **Gate 1**
   ...
   exact text.
2. **Tenants are isolated by construction.** Each company is a separate profile with its own state,
...
a pack is a *graph*, not necessarily a pipeline. All skills are shared, so a pack composes
...
## GTM skill suite (39 skills — all profile-driven)
...
| Category | Skills |
|---|---|
| **Prospecting** | `prospect`, `market-scan`, `events-tracker`, `draft-outreach`, `email-sequence` |
...
docs/          architecture overview + DEPLOY guide + enforced rules (RULES.md) + content-craft playbooks
