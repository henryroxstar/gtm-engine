...
[GTM skill suite](#gtm-skill-suite-58-skills--all-profile-driven) ·
...
| **Media connectors** (Higgsfield · HeyGen) | carousel and infographic visuals, short-form video renders, the synthetic presenter (`carousel-visuals`, `video-render`, `video-avatar`) | OAuth connector in Claude — no key stored | Both | the skill stops at the storyboard and hands you the shot list to render yourself |
...
| **Flow** | radar → plan → **script** → **storyboard** → render → finish → score → publish. Render fans out (vertical + feed cuts, or presenter + inserts) and rejoins at `finish` |
| **Extra gates** | Beyond the two permanent gates, the **storyboard** is its own approval step — you sign off the shot list before anything renders, because rendering is the expensive part |
...
| **Output & gates** | A finished, captioned, scored cut. **Both gates apply**, plus the storyboard gate |
...
not break: **[`CONTRIBUTING.md`](CONTRIBUTING.md)**. The enforced Python rules (§R1–§R10, each
