...
PAGE_BG = "FFFFFF"
MIN_CONTRAST = 4.5  # WCAG AA for body text; these runs are under the 14pt-bold large-text bar.
...
def readable_on(fg, bg, min_ratio=MIN_CONTRAST):
    """``fg`` if it is legible on ``bg``, else the same hue scaled toward black or
    white until it is.

    A tenant's palette slot carries no promise about the surface it will be drawn
    on: one tenant's ``accent3`` was pure black, which the banner painted onto a navy
    ``bg`` — a real doc shipped with a near-invisible date line. Scaling preserves
    the brand hue (gold stays gold, just deep enough to read) instead of falling
    back to a generic grey, so this degrades a palette rather than overriding it.
    """
    ...
    def toward(target, t):
        return "".join(f"{round(c + (target - c) * t):02X}" for c in (r, g, b))

    # Try both directions and take the smallest shift that clears the bar — scaling
    # only away from the surface fails when fg already sits at that extreme (white
    # text on a mid-tone fill can only be fixed by going darker, not whiter).
    best = hx(fg)
    best_ratio = contrast(fg, bg)
    for step in range(1, 21):
        t = step / 20
        for cand in (toward(255, t), toward(0, t)):
...
class Renderer:
    ...
    def render_banner(self, blk):
        ...
        if blk.get("meta"):
            ...
            meta_color = readable_on(self.brand["accent3"], self.brand["bg"])
            self._run(p2, blk["meta"], size=18, color=meta_color)
    ...
    def render_heading(self, blk):
        ...
        # The rule is decorative and stays the exact brand accent; only the words
        # are darkened when the raw accent is too pale to read on the page.
        self._add_heading_rule(p, self.brand["accent1"])
        self._run(
            p,
            str(blk.get("text") or "").upper(),
            size=26,
            bold=True,
            color=readable_on(self.brand["accent1"], PAGE_BG),
        )
        ...
        if header:
            for c, text in enumerate(header):
                ...
                self._run(
                    ...
                    size=20,
                    bold=True,
                    color=readable_on(self.brand["textOnDark"], self.brand["accent1"]),
                )
            r_idx = 1
        for row in body:
    ...
    def render_two_col(self, blk):
        ...
        for c in range(2):
            ...
            self._run(
                ...
                size=20,
                bold=True,
                # accent2 is white in some palettes — unguarded this printed white on white.
                color=readable_on(self.brand["textOnDark"], header_fills[c]),
            )
