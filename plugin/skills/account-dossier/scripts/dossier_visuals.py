...
Colors are passed in (zero hardcoded company strings). ALWAYS pass them, resolved from the active
profile's brand kit:
  uv run python -m gtm_core.brandkit --profile <active> [--product <slug>] --key palette.canvas
  ... --key palette.primary   ... --key palette.accent

If omitted, the flags fall back to a NEUTRAL ACHROMATIC set so the script always runs — grey on
near-black, which looks obviously unbranded. That is the point.

  Corrected 2026-09-01. These defaults used to be navy #0A0E27 / blue #3E7BFA / purple #B57BFF /
  cyan #22D3EE, documented as "the account-dossier fallback palette". They matched no tenant in
  this repo: it was an invented palette that read as deliberate, so a dossier rendered without
  --bg/--accent* looked branded while being branded to nobody. A fallback must look unset.

Both images are sized for ~6in render width in the Word doc.

Usage:
  python dossier_visuals.py --out ./work --which both \\
      --bg "$(uv run python -m gtm_core.brandkit --profile <active> --key palette.canvas)" \\
      --accent1 "..." --accent2 "..." --accent3 "..."   # palette.primary / .accent / .accent
...
def main():
    ...
    p.add_argument("--bg", default="#0B0B0F", help="background hex — pass palette.canvas")
    p.add_argument("--accent1", default="#9AA0A6", help="primary accent — pass palette.primary")
    p.add_argument("--accent2", default="#7E858B", help="secondary accent — pass palette.accent")
    p.add_argument("--accent3", default="#C3C8CC", help="tertiary accent — pass palette.accent")
