...
name: value-case
description: >-
  Build the quantified business case for a deal: the cost of the status quo, the modelled
  delta, and the assumption each number rests on. Trigger when the user says "build the value
  case for [company]", "ROI model for [account]", "quantify the business case", or "cost of
  inaction for [company]".
metadata:
  version: "0.1.0"
  phase: "5"
