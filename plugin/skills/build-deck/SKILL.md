...
description: >-
  ...
  [persona]", "create a presentation about [topic]", "put together a deck for [meeting]", or
  "build a one-pager for [use case]".
metadata:
  version: "0.13.0"
  phase: "4"
...
**Zero, tier 0 — real motion, when the argument is "the product does this."** A 6-second product
capture outperforms every one of the tiers above on the slide it fits: nothing sells "this actually
works" like watching it work. Reach for it before diagrams-as-code specifically on product-proof
slides — a live demo, a real dashboard, a real flow — not as a replacement for argument slides that
have no product to show.

| Skill / module | Wire into |
|---|---|
...
component (tier 1's fallback) or fall through to tiers 2–3 above.

**Live and print can want different densities.** When a slide's live-room version and its printed
page should genuinely carry different amounts of content — a live slide rests on one number, the
PDF a recipient reads later needs the table that number came from — author both with Slidev's
`<RenderWhen context="main">` / `<RenderWhen context="print">`, per
`profiles/<active>/knowledge/deck-composer.md`. This is not a default wrapper on every slide; it's
for the specific slides where deleting content to fit the room, or padding the room with a table
nobody will read live, would both be wrong.
...
## Step 7c — Definition of Done (five gates; none of them is "it looks fine")
...
**0 · Deck-level consistency — runs everywhere, including headless.**

```
uv run python -m gtm_core.deck_consistency <slides.md>
```

Gates 1–3 below each ask a question about ONE slide or ONE file. This one asks about the
**deck**, which is where a whole class of defect was hiding: measured 2026-08-29 across ten
decks, five components were **85.1%** of 377 component instances, eleven components had never
been used once, and one deck ran **seven** consecutive `layout: cover` slides. Every one of
those slides passes a per-slide check individually.

Rules: **C1** concentration (no component above 35%) · **C2** breadth (12+ slides → 8+ distinct
components) · **C3** layout run-length (max 2 consecutive) · **C4** tempo · **C5** reach past
the default five · **C6** a component the theme does not ship (Vue renders it as *nothing*, so
the slide silently loses its content — this is what shipped an `AskBox` deck the sidecar could
not render).

Errors block. Suppress a rule that is genuinely wrong about a deck with
`<!-- lint-ok C3: reason -->` inside the offending slide — same convention as `deck_lint`.
...
**4 · Typography — runs everywhere, including headless.** Invoke the `ui-typography` skill in
enforcement mode against the composed `slides.md`: hierarchy that actually reads as hierarchy
(not four headings at the same visual weight), consistent scale steps, no orphaned single words
on a wrapped line. This is a composer-authoring check, not a rendering one — it runs on the
Markdown/Vue source, not the export, so it belongs in the same "everywhere" bucket as gates 0
and 1.
