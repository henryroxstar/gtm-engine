...
`story_format` is `true` when the item's `brief` carries a `protagonist` and `false` when it
carries a `brief` without one. **Omit the key entirely if you cannot see the item's brief** — it is
read as a three-state field, and absent means *unknown* rather than *not a story*. It goes at the
top level, never inside `recommended`: everything scalar in that block is swept into the outcome
row's `meta`, which the correlator cannot query, and only when a `band` happens to be set. This is
the one part of the storytelling method this system can ever check for itself — whether story
pieces earn more shares than the rest of the bank — and it can only be checked if the tag is
...
```
uv run python -m gtm_core.outcomes attribute --profile <active> \
...
- Never edit the live knowledge corpus. If a pattern looks worth promoting, the target is
  **`hooks.toml`** (the feed hook bank — `hook-matrix.md` holds 1:1 outreach openers and is not
  where a video pattern lands), and it is the outcomes loop's promote candidates plus a human
  decision that get it there — say it, don't do it. `gtm_core.gtm_distill` computes those
  candidates against the bank, so naming the wrong file sends the human to a file the mechanism
  never touches.
