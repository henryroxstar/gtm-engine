...
description: >-
  ...
  prompt problem — a live models_explore query proved `soul_id` is accepted only by IMAGE
  models, so a trained Soul cannot reach any video model and identity is re-derived from a
  JPEG every frame; and `audio_references` is a reference input, not a lip-sync switch (mouth
  closed in 9 of 10 sampled frames across a full spoken sentence, verified 2026-08-19). For a
  speaking presenter, route to real footage (the primary lane, no Article 50 disclosure duty)
  or the faceless format (b-roll plus held soul_2 stills, VO and burned captions) — see
  `video-router` Step 0.5. Still in scope and unchanged: every shot with no real person in
  frame, plus soul_2 identity STILLS (as stills, never animated into video). Every generation
  prompt is assembled per references/prompt-recipes.md and recorded verbatim with its seed in
  the render manifest, which refuses a synthetic asset without one — and now also refuses one
  it cannot tie to a costs.jsonl row, because ~580 credits of August 2026 video spend never
  reached the monthly cap.
metadata:
  version: "1.0.0"
  phase: "6"
