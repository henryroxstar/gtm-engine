...
Colour values come from the active profile's brand kit, never from this table:
`uv run python -m gtm_core.brandkit --profile <active> [--product <slug>] --key palette.canvas`.

| Element | Dark (default) | Light |
|---|---|---|
| Background | `palette.canvas` | `palette.canvas_light` |
| Body text | `palette.ink` | `palette.ink_light` |
| Panel / card | `palette.surface` | `palette.surface_light` |
