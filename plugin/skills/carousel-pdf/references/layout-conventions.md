...
```css
[data-page="<product-slug>"] → --product-accent: <resolved from the brand kit>
```

**Resolve `--product-accent` from the brand kit — never write a hex here.** For the product the
slide is about:
...
A product that carries a sub-brand/suite has its own `palette.accent` and `palette.gradient` in
`products/<slug>/BRAND.toml`; one that does not inherits the company's. Either way the value comes
from the kit, so a sub-brand added later needs no edit here.

> Corrected 2026-09-01. This block used to hardcode one tenant's suite accents (including a blue
> that has since been retired as website drift) inside a company-agnostic skill. Three failure
> modes in four lines: a tenant leak into `plugin/`, a stale value, and a tenth palette.
