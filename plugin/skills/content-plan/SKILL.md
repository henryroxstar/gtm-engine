...
description: >-
  ...
  the last few content-radar digests, the platform playbooks, and the content history, then
  proposes a weekly theme plus 3–5 concrete content ideas (each tied to a pillar and a story
  cluster, with platform, format, and locale) across LinkedIn, X, and Instagram — one item per
  ...
  ContentItem[]. This skill should be used when the user says "plan this week's content",
  "make a content plan", "what should we post this week", "content plan", "draft the plan", or
  after a radar run.
metadata:
  version: "0.2.0"
  phase: "2"
