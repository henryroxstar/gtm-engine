...
snapshot file (see `discovery-and-budget.md`), not the API facet. After totalling the rubric, add:

| Signal | Points |
|---|---|
| Topic-intent **score ≥75** on **either** feed — a Vibe row score ≥75, or an Intentsify snapshot score ≥75. A RocketReach API `intent`-facet **filter hit** (which exposes no score) also qualifies, since a tracked-topic match is itself the surge signal. | **+2** |
| **Both** feeds ≥75 on the same account (**double-intent**) | **+1 more** |
