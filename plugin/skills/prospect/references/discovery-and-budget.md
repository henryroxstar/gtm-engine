...
|---|---|---|
| Cold ICP **company discovery** + firmographics | **Vibe Prospecting** (`fetch-entities`) | web search |
...
**Topic-intent mechanics (Bombora):** `business_intent_topics` is a `fetch-entities` filter — object form `{topics: [...], topic_intent_level: "high_intent"}`. Topic strings MUST come from `autocomplete` (field `business_intent_topics`); take the list to run from the profile's `market-scan-config.md` → "Intent topics" section (~12 topics), re-verifying against autocomplete on the first run each month (the taxonomy drifts). The MCP exposes intent as a **filter only** (a match ≈ Bombora composite score >60); per-company numeric scores aren't returned — treat a hit as boolean + level, not a number. Filtering costs nothing at fetch time; the usual export pricing applies to rows you export.
...
|---|---|
| `entity_type` | `businesses` |
| `business_intent_topics` | `{topics: [<~12 from the profile's market-scan-config.md → "Intent topics">], topic_intent_level: "high_intent"}` — topic strings via `autocomplete` |
...
## Web-search fallback (when Vibe isn't connected)
...
The fallback is more labour-intensive but costs nothing — fine for the budget. Flag in the run header that it was a web-search-only run and that contacts are unverified.
