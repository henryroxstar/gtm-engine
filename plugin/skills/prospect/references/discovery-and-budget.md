...
### Builder / agent-factory filters — a third segment, and a different instrument

**Firmographic discovery structurally cannot find this segment.** These companies are 1–50 people,
bootstrapped, and describe themselves in operator language rather than infrastructure language. The Startup
block above rejects them twice over: its smallest size band starts at 11, and `new_funding_round` — its
primary discovery trigger — never fires for a services-funded company. Run this block **alongside** the
evidence-first sourcing the profile prescribes (resolve `market-scan-config.md` → § "Builder sourcing
roster"; `icp-personas.md` § Builder segment keeps the gates, rubric and discriminators). Here the Vibe
pass is the second instrument, not the first.

| Filter | Value |
|---|---|
| `entity_type` | `businesses` |
| `company_size` | **the smallest bands, including the 1–10 band** — re-run `autocomplete` for the current enum before the run (§Schema drift: the exact band label drifts, and this segment lives entirely in the band the other two blocks exclude) |
| `company_revenue` | `null` — no revenue floor; these are services-funded |
| `is_public_company` | `false` |
| `has_website` | `true` |
| `company_country_code` | one ISO code per market pass (from PROFILE `target_markets`) |
| `linkedin_category` | autocomplete: **"IT services and IT consulting"**, **"business consulting and services"**, **"outsourcing and offshoring consulting"**, "software development" — the SI/agency categories carry this segment; "software development" alone misses most of it |
| `website_keywords` | the words they use about themselves: `["AI workforce", "AI employee", "digital worker", "AI agency", "agents for SMEs", "AI automation", "WhatsApp automation", "back office automation", "multi-agent"]` — **not** `"verifiable credentials"` or `"agent governance"`, which is the seller's vocabulary, not the buyer's |
| `events` | **do not gate on `new_funding_round`** — its absence is not a negative signal here. Optional and non-gating: `["new_product", "increase_in_engineering_department", "new_partnership"]` |
| `number_of_results` | `30` |

**Two failure modes to expect, and what defends against each:**
- **Demo-ware.** An "AI agents" page with nothing shipped is the highest-volume false positive in this
  segment, and `website_keywords` cannot tell the difference — only the Step 4 web sweep can. Every Builder
  candidate needs a **named production deployment for a paying third-party client** confirmed before it is
  scored. That is the segment's firmographic floor, not a nice-to-have.
- **Their customers, not them.** The SMEs these builders serve (cleaning, logistics, facilities, clinics)
  describe the same workflows in the same words, and the smallest size band is full of them. Qualify on
  *who operates the agents*, never on whose workflow is described.

**Contact resolution:** at 1–10 headcount the founder is the buyer and the address is usually on the site.
RocketReach reliably misses country-suffixed domains — probe `.com.sg` / `.com.hk` / `.hk` explicitly, or
this segment resolves to nulls.
...
filters:
  ...
  company_size: { values: ["1001-5000", "5001-10000", "10001+"] }          # startup: ["11-50","51-200","201-500"] · builder: smallest bands incl. 1–10 (autocomplete first)
...
### Site-published generic inboxes (Builder/Startup only — never Enterprise)

When RocketReach → Vibe → Apollo → the web sweep above all miss a **named** person, and the
company's own official site publishes a generic inbox (`hello@`, `enquiries@`, `info@`), that
address is not the same kind of "unverified" as a guessed `firstname@domain` or a name pulled off
a directory listing — the company's own site is the source of truth for its own contact channel.
Stamp it, and it earns `high` deliverability confidence (`gtm_core.prospects_consolidate
.classify_confidence`'s `site-published` status) — but only under all three of these conditions:

1. **The account's `segment` is Builder or Startup.** At that size a generic inbox plausibly
   reaches the founder or a small team directly. Never do this for Enterprise — an `info@`
   there reaches a mailroom or a support queue, not a buyer; an Enterprise account with no named
   contact stays `unverified` and goes to `needs-verification.csv`, same as today.
2. **The page is the company's own domain** — its own contact/about/footer page, not a
   third-party directory, review site, or social profile describing the company.
3. **The email's domain matches the company's own domain** (`company_domain` on the row). This is
   checked in code too (`classify_confidence` fails closed to `unknown` on a mismatch) — but
   resolve the right domain yourself rather than relying on the code catch: an address on a
   *different* domain found on the company's site (e.g. a linked partner's inbox) does not
   qualify even if the page itself is legitimate.

Ideally the same address appears on more than one of the company's own pages (footer + contact
page + privacy policy) — not required, but the strongest form of the signal. Write `email_status:
site-published` (not `unverified`) on the row so consolidation grades it correctly; note the
source page URL in the account record the same way any other contact source is cited. **Keep any
parenthetical detail free of the words "invalid", "unavailable", "pattern", or "RocketReach F"** —
`classify_confidence`'s blocked-status match is substring, not anchored, so a note like
`site-published (named-exec guess RocketReach F/invalid)` silently reclassifies the row as
`blocked` instead of `high`, exactly backwards from what the status is supposed to mean.
