...
name: market-scan
description: >-
  Sweep weekly news, competitor moves, and standards to produce a dated market signal brief,
  post drafts, and campaign ideas. Trigger when the user says "run my market scan", "weekly
  market scan", "what's moving in the market this week", "scan for market signals", "what
  should I be posting about", or "content ideas".
metadata:
  version: "0.3.1"
  phase: "2"
...
>
> Also read the **competitor registry** from the same resolved path:
> `python -m gtm_core.resolve_knowledge competitors.toml --profile <active> [--product <slug>]`.

Then read `profiles/<active>/knowledge/market-scan-config.md` (resolved as above) — the company-specific scan config: the regulatory & standards bodies (1C), the content pillars elaboration, and the own-brand signal queries (1D). The **competitor watchlist (1B)** now lives in the competitor registry (`competitors.toml`, resolved the same way), not in `market-scan-config.md`.
...
Use the **competitor registry** (`competitors.toml`, resolved as in Step 0). Iterate every competitor whose `status` is `active` or `watch`. For each competitor, run a web search using its `name` and `aliases`:

```
"{name OR alias}" agent OR agentic OR "agent identity" OR governance — last 7 days
```

Also check each competitor's `watch_urls` (official blog / newsroom / changelog) directly.

Use the competitor's `tier` to decide how much weight to give the result: `direct` competitors get the full deep-read treatment; `nhi-native`, `adjacent`, and `si-channel` get a lighter scan unless the signal is H-strength. Proposed additions from `registry-proposals-<date>.json` are listed for awareness but are not yet part of the watchlist.

If `competitors.toml` is absent or empty, fall back to the legacy competitor watchlist in `market-scan-config.md` §"Competitor watchlist" and note the fallback in the output.
...
For each competitor in the registry (`competitors.toml`) with `status` = `active` or `watch`, assess: **No change**, **Moved**, or **Threat**. Record what shipped and a one-line implication. Flag: Has any competitor closed a gap the active company currently owns? Has any competitor shifted positioning in a way that requires a messaging response? Propose any brand-new competitor surfaced this run as a registry addition (write to `content/<active>/market-signals/registry-proposals-<date>.json`) — do **not** append directly to `competitors.toml`.
