...
description: >-
  ...
  knowledge pack — never hardcodes any geography or company. All sources are free (web search,
  browser) — no metered calls, no budget impact. This skill should be used when the user says
  "run my market scan", "weekly market scan", "what's moving in the market this week", "scan
  for market signals", "what should I be posting about", "catch me up on agentic AI", "content
  ideas", or on the Monday weekly cadence.
metadata:
  version: "0.2.0"
  phase: "2"
...
**1A — Agentic AI & developer news (last 7 days)**

```
"agentic AI" OR "AI agents" OR "MCP" OR "multi-agent" — published last 7 days
AI agents funding OR launch site:techcrunch.com OR site:venturebeat.com — last 7 days
GitHub trending agent framework OR MCP — this week
```

Sources: Hacker News (`news.ycombinator.com/news`), TechCrunch AI, VentureBeat AI, The Register, GitHub Trending (filter: Python, weekly).
...
For each 1C signal: note the regulator, the signal, any enforcement deadline, and which ICP account types it creates urgency for (use the ICP personas in `profiles/<active>/knowledge/icp-personas.md`). If a signal is strong enough to be a "why now" hook in prospecting outreach, write the hook as a ready-to-paste block — note it as a §7.3 hook update to apply before the week's prospecting run.
...
- The profile knowledge pack (`profiles/<active>/knowledge/`) and the scan config (`profiles/<active>/knowledge/market-scan-config.md`) are read-only during a scan — flag discrepancies, do not edit.
