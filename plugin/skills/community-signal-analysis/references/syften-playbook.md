...
- **Space = AND.** `northwind agent` requires both terms, any order.
- **No `OR`. No parentheses / grouping.** Express alternatives as **separate filters** — one filter
  per alternative. `Halyard AND (AI OR LLM)` is **invalid** Syften syntax; split it into
  `Halyard AI` and `Halyard LLM` (or scope with a single disambiguator).
