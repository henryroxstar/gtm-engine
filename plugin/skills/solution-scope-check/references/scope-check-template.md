...
# ---- SPACING SCALE (do not tighten below these to win a line) ----
# A scope check is a worksheet the buyer marks up, not a page to cram. Dense output is the
# failure mode this scale exists to prevent: a wall of 8pt text reads as unfinished and does
# not survive a two-minute skim. Section air is load-bearing, not decoration.
#   heading   before 9.5  after 4      table cell   before/after 2.4
#   bullet    after 4.2                qblock       before 9  after 2.5
#   step/line after ~3                 q-bullet     after 2
# When it spills past two pages, CUT CONTENT — drop the lowest-leverage question group or
# tighten prose. Never claw the space back out of the spacing scale.

def heading(text, before=9.5, after=4):
    p = para(before=before, after=after); run(p, text, size=12, color=NAVY, bold=True); return p

def bullet(segs, after=4.2):            # segs: list of (text, color, bold?)
...
def qblock(num, title, unlocks, questions, assume=None):
    ...
    if assume:
        pa = para(before=1.5, after=1, line=1.05); pa.paragraph_format.left_indent = Inches(0.22)
        run(pa, "Our assumption today: ", size=8.5, color=FAINT, bold=True, italic=True)
        run(pa, assume, size=8.5, color=FAINT, italic=True)

# Table cells: set space_before/space_after = Pt(2.4) on every cell paragraph — python-docx
# defaults to 0 and the table is the first thing that reads as cramped.

# ---- PAGE 1 ----  kicker → title → recipient → one-liner+rule → draft note → proposing → table → V1
# ---- (build with the helpers above; honour the spacing scale — cut content, not air) ----
...
- **Claims:** every capability claim sourced to a `file:line`, maturity tagged at first mention, and
  the caveat register intact — done in markdown at Step 4, **before** this render. If it is not done
  by now, do not verify layout on a doc whose claims are unchecked; go back.
- **Page count:** `soffice --headless --convert-to pdf --outdir <tmp> <docx>` then
  `pdfinfo <pdf> | grep Pages` — must be **2**. If 3, return to the markdown and drop a **whole unit**
  (the lowest-leverage question group, a bullet, a table row), then re-render. Never shave hedges,
  qualifiers, or the spacing to claw back a line — that is how an accurate claim becomes an overclaim.
- **Counts:** every count word in the prose ("three capabilities", "the four things") matches what
  actually survived the cuts.
