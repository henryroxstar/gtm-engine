...
description: >-
  ...
  **pre-design**, sourced from a `solution-discovery` brief (page 1 = what we heard + the
  ...
  question-type bank (`references/question-types.md`). Sources every capability claim to a
  `file:line` in the profile's own reference layer and fits the text in markdown *before*
  rendering. Reads PROFILE for brand, byline, output folder, and language. Read-only; builds
  the .docx with python-docx (direct formatting, no style overrides). Produces files in
