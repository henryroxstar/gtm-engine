...
- **`profiles/<active>/PROFILE.md`** — `brand_palette` (hex list), `brand_name` / `deck_byline`,
  `output_folder`, `language`, `default_product`.
- **`profiles/<active>/knowledge/brand-notes.md`** — palette, typography, the "don'ts".
...
**Brand colours:** use `brand_palette` from PROFILE / `brand-notes.md` for the HTML companion accent
in Step 10. If no profile resolves, fall back to a neutral slate/indigo pair rather than inventing a
brand.
...
`brand_palette` accent (a light-mode and a dark-mode value, since one hex rarely works on both).
