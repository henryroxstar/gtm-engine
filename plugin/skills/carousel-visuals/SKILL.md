...
If the colleague wants a single document-post PDF rather than a native multi-image post, assemble the
verified PNGs with Pillow:
```python
...
for each. Tell the colleague this replaces the Slidev-rendered version at the same path — note any
