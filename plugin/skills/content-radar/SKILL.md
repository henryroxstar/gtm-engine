...
name: content-radar
description: >-
  News-driven content radar for the active company. Reads fresh PROD discovery_items via the
  ...
  summaries; Claude ranks and composes the brief. After ranking, compares top clusters against
  the existing hook bank in profiles/<active>/knowledge/hooks.toml and appends new angles as
  candidate hooks (status="candidate") via python -m gtm_core.hooks add-candidate; candidates
  never enter rotation until promoted to test. For an X-native angle, may cite a structure
  from docs/x-tweet-patterns.md in source_signal as a suggestion. Produces a dated radar
  digest plus a StoryCluster[] the content-plan skill consumes. Falls back to a free web sweep
  (market-scan) when the news rows are stale or empty. This skill should be used when the user
  says "run content radar", "what's trending for content", "scan the news for content", "what
  should we post about", "content radar", "refresh the radar", or on the content cadence
  before planning.
metadata:
  version: "0.2.0"
  phase: "1"
...
python -m gtm_core.radar --rows content/<active>/radar/.rows.json --profile <active> \
  --content-signals content/<active>/plans/market-intelligence/content-signals-<YYYY-MM-DD>.json
```
...
If `market-intelligence` did not produce a content-signals handoff for this date, omit the
`--content-signals` flag — the radar will fall back to its default path and run on news-derived
clusters only.
...
## Step 5 — Hook candidate discovery

After ranking, compare the top clusters against the existing hook bank to surface new angles that
should enter the library as candidates. Read the current hooks:

```bash
python -m gtm_core.hooks list --profile <active> --include-candidates
```

For each top cluster, ask: **does this signal support a new hook angle that is not already covered by
a proven/test/candidate hook?** A candidate is warranted when:

- The cluster has a clear, repeatable attention pattern (not a one-off announcement).
- The angle is on-narrative for the active company's pillars.
- The angle is distinct from every existing `angle` (not a paraphrase).

When all three hold, propose a candidate hook and persist it to `profiles/<active>/knowledge/hooks.toml`
with `status = "candidate"`:

```bash
python -m gtm_core.hooks add-candidate --profile <active> --content-root content/<active> \
  --json '{
    "id": "<kebab-case-hook-id>",
    "angle": "<one-sentence hook angle>",
    "payoff_promise": "<what the audience gets if they keep reading>",
    "formats": ["linkedin-text", "thread", "single", "carousel", "infographic", "reel", "short"],
    "pillar": "<matching pillar from PROFILE.md>",
    "goal": "<awareness|consideration|conversion>",
    "source_signal": "radar: <cluster title> — <one-line why this signal supports the angle>",
    "status": "candidate"
  }'
```
...
- `id` must be kebab-case, unique across the bank, and stable.
- `formats` should include every format the angle could plausibly take; do not guess performance.
- `payoff_promise` is a placeholder — the operator refines it when promoting to `test`.
- `source_signal` must name the cluster and the evidence so the provenance is reviewable. For an
  X-native angle, you may additionally cite a structure from `docs/x-tweet-patterns.md` in
  `source_signal` (e.g. `pattern=harsh-truth`) as a suggestion — the pattern name is taxonomy only,
  the angle text stays original. `add-candidate` writes no `opening_beats`, so `pattern_id` itself
  is not set here; it is chosen and written on the beat when the operator promotes the candidate
  and content-studio drafts it.

Candidates **never enter rotation** until an operator promotes them to `test`. If a cluster matches an
existing candidate, do not duplicate it. Log any candidates you add in the run manifest outputs under
`candidate_hooks`.
...
python -m gtm_core.ledger_cli append-history --profile <active> \
  --json '{"event":"radar_complete","skill":"content-radar","run_id":"<run-id>","digest":"<digest-path>","clusters":<N>,"source_items":[<ids surfaced>],"candidate_hooks":[<added ids or null>]}'
python -m gtm_core.ledger_cli write-run-manifest --profile <active> \
...
- Count of new candidate hooks added (if any)
...
- Candidate hooks are **append-only** to `hooks.toml`; never overwrite existing hooks from the radar.
- A candidate must not duplicate an existing `id` or `angle`. If unsure, skip rather than duplicate.
