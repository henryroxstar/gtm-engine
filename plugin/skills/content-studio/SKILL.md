...
description: >-
  ...
  (atomic repurposing, not N independent drafts). Gates EVERY asset through the content linter
  before it is shown for review. Copy/brief-only — no PDF render and no paid image generation;
  visual render is a separate, operator-gated hand-off. This skill should be used when the
  user says "draft the post", "make the carousel", "build the asset", "content studio",
  "create the LinkedIn/X/Instagram post", "make the thread", "write the reel", or after
  content-research.
metadata:
  version: "0.2.1"
  phase: "2"
