...
- `profiles/<active>/PROFILE.md` (`brand_name`, `social_handle`, `voice_style`, `content_pillars`,
  ...
  product facts.
- `profiles/<active>/knowledge/hooks.toml` — **the hook bank, which owns the `hook_id` namespace.**
  Not `hook-matrix.md`: that file holds **1:1 outreach openers** and in some tenants is an id-less
  persona × signal grid, so a feed `hook_id` does not resolve there. If the item has a `hook_id`,
  read its row and use the hook's `angle` and its **opening beat for this format** as extra steer
  (`text` is the canonical execution for a text/carousel beat) — do not copy either verbatim.
  ```bash
...
this format. **These are two different failures and do not get the same response:**

- **Format not declared** — a hard stop. Do not skip the gate: skipping is how an ungated asset
  reaches publish, which is the entire failure this step exists to prevent. Fix the bank (declare
  the format **and** author its opening beat) or change the item's format to one the hook declares,
  then re-score. `content-plan` is supposed to have paired these at plan time; a mismatch here means
  that pairing was wrong and belongs back in the plan, not absorbed downstream.
- **No `hook_id`** — skip this gate and name the missing attribution in the report. The asset can
  never earn a prior; that is the price of an unattributed item and it must be stated, not silent.

**Two distinct failure shapes below the bar — read `prior_has_data`, not just `band`:**

- **Genuine low quality** (`band: "low"`, or `"uncertain"` with `prior_has_data: true`): the
  retention or pattern signal is real and weak. Rewrite once targeting `weakest_dim`, re-lint, and
  re-score. Still below threshold → downgrade to the cheapest high-scoring declared format (e.g.
  LinkedIn carousel → text, X thread → single, Instagram reel → carousel), update the asset's
  `format` and `hook_score_report`, re-lint, and re-score. If no declared format reaches
  `medium`/`high`, stop and report the blocker rather than shipping an ungated asset.
- **Cold start** (`band: "uncertain"` with `prior_has_data: false`): this hook × format has **zero**
  outcome-history rows, which is true of every hook until it has shipped and accrued ≥500
  impressions. No rewrite and no format downgrade can move this — `prior` is not a craft signal, and
  an asset cannot manufacture publish history. Treating it as genuine low quality deadlocks the
  whole lane: nothing publishes, so no hook earns a prior, so nothing ever clears `uncertain`.
  Instead **surface it to the operator** ("this hook has no track record yet — ship as a test to
  start collecting outcomes?") and, on their explicit yes, re-run `hook_score` with
  `--override-reason "<why>"` (refused if blank). That appends an audited `hook_score_override` row
  to `outcomes.jsonl`; the band does **not** change and the row is excluded from every future prior
  computation, so an override can never manufacture a fake prior for itself. Only proceed once
  `output.override.accepted == true` — a refused or blank reason is not tacit approval.

  This is the same rule `video-script` Step 2 applies, and it matters more now than it used to: with
  both banks carrying zero content outcome rows, **every** hook is currently cold-start.
