...
- **All six beats present**: signal → credit/control → hedged gap → **stakes (who asks, and when — prefer a dated forcing function)** → **frontier point (non-obvious, already buildable, anchored)** → one ask. A draft carrying only signal/gap/ask is correct and inert; beats 4 and 5 are what earn the reply.
...
- Exactly **one** ask, gating exactly **one** artifact, and the ask **names what the artifact does for them** ("the one-page teardown of how that clears the review"), never just the artifact ("the one-pager"). The teaser one-pager in touch 1, the recorded demo held for touch 3. Never a time-ask or calendar link ("worth a short call?" is a time-ask wearing an adjective).
