...
1. **Seat → lead pain.** Read the recipient's title, then take the lead pain and the gain straight
   off `voice.md`'s persona-axis table. This is a lookup: CEO → the enterprise deal it stalls;
   CTO/platform → fragmentation + build cost; CISO/risk → attribution; Head of AI → can't safely
   scale; CDO/DPO → reportable breach; CRO/compliance → examiner-provable evidence. **Do not fire the
   CISO's attribution pain at a founder** — linter rule `persona-lead-mismatch`.
...
1. **Pick the hook from the matrix, and DECLARE the cell.** Choose the cell at the persona ×
   signal intersection, rewrite it to name the *specific* signal (company, date, the actual
   event) — never paste the cell verbatim. One capability, one outcome. Record both in the pack
   header, verbatim in the matrix's own labels:

   ```
   **Hook cell:** <Persona> × <Signal>
   **Capability:** <one of the seven slugs in knowledge/capability-argument.toml>
   ```

   A hook nobody recorded cannot be checked. On 2026-09-04 six Tier-A packs shipped with no
   `hook_cell` at all and the matrix was never opened — the same failure `email-sequence` fixed
   for sequence specs in August. `capability` is what the body must then argue (beat 2).
...
4. **Write each channel** to the voice.md structure:
   ...
   - **Email** — five beats, in this order, 65–97 words. **Rewritten 2026-09-04 after six
     hand-rewrites of one batch:** the previous shape below the line mandated a credit clause,
     the hedge "my hunch — tell me if you've got this covered", and a CTA gating a one-pager.
     All three are now linter ERRORS (`credit-is-verdict`, the retired-phrase rules,
     `offer-does-their-work`). A generator that contradicts its own gate produces copy someone
     then fixes by hand, which is what happened. Draft to this and it passes clean:

     | # | beat | words | shape | gate |
     |---|---|---|---|---|
     | 1 | **The fact** | ≤22 | `I understand <one verified thing they do, from the dossier>.` Their own public words. Cannot be wrong. | `specificity` |
     | 2 | **The agent acts** | ≤22 | **The agent must be the grammatical SUBJECT, doing something.** "The agent holds a credential into their CRM and keeps it after the job closes." Not "the approval trail tends to be…" — a body with no agent in it is not about agentic governance. | `capability-unargued` |
     | 3 | **The seat's moment** | ≤24 | Where it decides something commercially: the next enterprise deal, the second merchant, the first buyer big enough to send a questionnaire. Use the seat's own vocabulary from voice.md's persona-axis table. | `seat-stakes-missing` |
     | 4 | **The out** | ≤8 | `Might already be sorted on your side.` A shrug, not an invitation to correct you. | `hedge-missing` |
     | 5 | **The offer** | ≤26 | Name the tenant's public anchor for the declared capability (`knowledge/capability-argument.toml` → `anchor`), then: `Would it be helpful if I <verb> <their thing> against <anchor>?` **Vary the verb across a batch** (mapped / checked / lined up / put next to / walked through / compared). | `offer-does-their-work`, `body-template-share` |

     The budget column is not advisory: a first pass that ignores it lands at ~105 words against
     a 99 hard cap, which is what a validation run of this template produced on 2026-09-04.
     Greeting and sign-off cost 4. **Compute the count with the linter, never estimate it** —
     that same run self-reported 96 for a 105-word body and `word-count-mismatch` caught it.
     ...
     **Retired 2026-09-04 and now failing:** "Happy to …", "I can / I could …", "Hi there,"
     ...
     handled". See voice.md calibration D and rules 10–12.
...
2. **Credibility diff before claiming a gap.** List what the dossier says they **already built**
   ...
   ("my hunch / my read, tell me if you've got this covered"). Match the mechanism to their **real
...
2b. **Declare the capability you are arguing from.** Put it in the pack header as
   `**Capability:** <slug>`, chosen from the seven groups in `knowledge/product.md`'s capability
   taxonomy: `identity`, `traffic-management`, `protocol-proxy`, `security-policy`,
   `credentials-delegation`, `observability`, `payments`. Pick it from the pain the **seat** owns,
   not from the one you argued last, and pick it *before* writing the body.

   The declaration is the whole point: an undeclared choice cannot be checked, and on 2026-09-04
   all five packs in one campaign argued `identity` while every per-file gate passed at zero
   errors — no per-pack rule can see a sibling. **At most two packs in one campaign may argue the
   same group**, enforced across the batch:
...
5. **Gift CTA offers one artifact the profile can actually produce** — the list lives in
   `profiles/<active>/knowledge/gift-artifacts.txt` and is enforced by linter rule
   `cta-unstaged-artifact`. It is *not* an inventory of files on disk: ~97% of cold sends never
...
8. **Stamp and lint.** The pack header carries `Rules-Version: <current>` and must pass the
   ...
   ```bash
   uv run python3 tests/linter/outreach_pack_linter.py --batch "content/<active>/accounts/*/email-*-<date>.md" \
     --signoff "<the sending colleague's real first name>"
   ```
...
- Opens on their signal, not a compliment or generic market line. **The credit clause is optional and off by default** (voice.md → *Warmth comes from the research*); `credit-is-verdict` fails an opener that grades the reader.
- Gap is hedged — **"Might already be sorted on your side."** ("my hunch" and "tell me if this is already handled" were retired 2026-09-04) — never asserted as fact about an environment you can't see, and **claims the CATEGORY, not their build** (voice.md rule 9): "platforms in that position often…", not "your approval record lives in X".
- **Three beats required, three optional** (voice.md → *The shape*): signal → **the problem, hedged, with why their stack can't close it** → one ask is a complete, sendable email, and for builder/founder seats it is usually the better one. Credit, stakes and the frontier point are earned additions — **take at most two of the three**, and only where this account's dossier supports them. An invented stake reads worse than an absent one, and six arguments inside 100 words is what makes a draft read as a brochure. (Until 2026-09-04 all six were mandatory; the correction is that mandatory ≠ good, not that beats 4 and 5 are wrong — they still earn the reply when they are real.)
...
- Exactly **one** ask. In a first touch it is the **anchored offer** (beat 5), not an artifact gate: `Would it be helpful if I <verb> <their thing> against <anchor>?` — `offer-does-their-work` fails an offer pointing inside their system with no public anchor. The artifact ladder belongs to touches 2–4. Never a time-ask or calendar link ("worth a short call?" is a time-ask wearing an adjective).
- DM ≤ 280 chars; email ≤ 100 words. **Compute the counts, never estimate them** — a self-reported number that disagrees with the body reads as verified and isn't; the linter recomputes and fails the mismatch.
- Subject is 1–4 words, lowercase, names the signal — written from *this* account's signal, never a formula carried across accounts.
