...
6. **The story — when there is a live conversation.** If the customer, or the champion inside the
   customer, can be talked to — a call, a recorded meeting, a written exchange — run
   [`references/interview-protocol.md`](references/interview-protocol.md) **before** Step 4's
   research. Five questions, in order, that ask for the moment they *doubted themselves most*
   rather than for "a story": asking for a story returns a polished success anecdote every time,
   and a success anecdote is what Step 6 rule 5 (*what they tried first, and why it failed*) is
   most often missing. The protocol's answers are what Step 4 then verifies; its three-way
   classifier (anecdote / accomplishment / true story) is re-run at Step 8.

   With no conversation available, skip it and **say in the report** that the first decision and
   the low point are inferred from materials. Inferred is a legitimate state; a low point presented
   as told when it was inferred is the melodrama every reader has learned to discount.
...
5. **Structural lints.** Fast, mechanical, and each one catches an error this skill has actually
   ...
   - Can you **point at decision 1, the low point, and decision 2** — one sentence each, in the
     draft? This is the classifier from [`references/interview-protocol.md`](references/interview-protocol.md).
     If any of the three has no sentence, the document is an *accomplishment* story — want, act,
     succeed, no obstacle — which readers recognise and discount. Say so in the report rather
     than shipping it under a title that promises a story it does not contain.
