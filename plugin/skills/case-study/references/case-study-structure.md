...
2. **State what remains, in the buyer's numbers.** Their operational metrics, their peers' survey
   ...
   **Then give it a moment.** The numbers are a *state* — what the organisation cannot do — and a
   state is what an accomplishment story has. A story has a moment: the point at which the people
   in it doubted themselves most. One sentence, from question 1 of
   [`interview-protocol.md`](interview-protocol.md), placed after the numbers so it lands on a
   reader who already knows what was at stake. With no interview behind it, write the moment as
   **inferred** and say so in the report — inferred is a legitimate state; an inferred low point
   presented as told is the melodrama every reader has learned to discount. Budget one sentence,
   not a paragraph: this section's whole allowance is 50 prose words.
...
**Front half — 215 prose words** (200 verified 1 page; 220 still fits, 248 spills):

| Section | Words |
|---|---|
| Why now | 28 |
| The challenge | 50 |
| The low point, as a moment | 15 |
| What they tried first | 22 |
| The solution | 56 |
| The outcome | 30 |
| Quote | 14 |

The low-point row (added 2026-09-07) is the one figure here **not** taken from a fresh page count.
It sits inside the tolerance this table already measured — 215 against a recorded "220 still fits" —
rather than resting on a new measurement, because a re-render on 2026-09-07 did not reproduce the
original single-page baseline configuration and so could not be substituted for it. Treat 215 as
the working ceiling and re-measure properly the next time the Word spec itself changes. One
sentence is the budget; a low point that needs a paragraph is a low point being narrated rather
than named.
