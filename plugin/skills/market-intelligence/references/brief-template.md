...
**The spine:** the five speakers are kept in separate sections; the divergence section is where
customer voice and BD focus meet. Never let a customer-voice claim borrow BD's framing as evidence, or
vice versa — and never let a spec proposal (`standards-voice`, §7b) or a competitor changelog
(`vendor-voice`) count as demand. Only `customer-voice` does.
...
clause here (High = ≥2 independent customer-voice sources) and point to §8 for the full source split —
the grading rule must not be first defined *after* the table that uses it.
...
## 8. How to read the ten sources — the five speakers   ⟨reference — keep it LAST, adjacent to App. A⟩
...
- **Five speakers, never merged.** Customer-voice cites customer-voice; BD focus is labelled our bet;
