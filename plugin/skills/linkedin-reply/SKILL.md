...
name: linkedin-reply
description: >-
  Draft value-first public comments and soft-sell connection notes to target LinkedIn posts in
  the brand voice. Trigger when the user says "reply to this LinkedIn post", "comment on this
  post", "draft a soft-sell reply", or "respond to this post / screenshot".
metadata:
  version: "0.7.2"
  phase: "3C"
...
> **§11.D's three templates are a compact daily-engagement set, not this skill's method** — they
> carry no register-by-author read (a peer gets a tight restate-the-claim line; a recognized
> authority gets the thesis restated in your own words) and no purpose scorecard. Use them only for
> a quick, low-stakes comment with no authority read to make; for anything else, *Compose* below
> governs and §11.D's phrasing does not override it.
...
- **Purpose scorecard (fill it, show it, before presenting).** The tell checks below prove the draft
  reads human; they do **not** prove it does its job. Grade the draft against the six social-selling
  goals in `docs/purpose-scorecard.md` §2 — relationship, further
  the conversation, warm the lead, best practice, indirectly sell, excite (lever only) — each with a
  one-line basis, and render the table above the draft. **Any ❌, or a weak score on a load-bearing
  goal (usually "indirectly sell" on a bullseye post), is a revise trigger, not a footnote** — fix the
  draft and re-score. Grade #6 as *lever present* only; excitement is verified by the reply, never
  claimed pre-send.
...
- **No banned fluff words** (voice.md list: excited/thrilled/reach out/touch base/synergy/
  ...
  reduced by default, no antithetical "not X, it's Y"). **Draft clean the first time, don't rely on the
  lint to catch it after:** don't open with a compliment to the poster before pivoting ("Great point,
  X — and…" / "Strong list — and Y's point is..."); don't stitch 2+ ideas into one semicolon-linked
  sentence dressed as prose (say the one strongest thing, or make it an actual list); don't close on a
  mirrored "X is table stakes, Y is the differentiator" formula. **Run this pass before presenting, not
  as a later vet** — it is cheaper as a first-draft gate than a correction round. Optional lint:
