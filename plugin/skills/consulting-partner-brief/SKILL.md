...
description: >-
  Build an executive joint-value and enablement brief for systems integrators and consulting
  partners. Trigger when the user says "build consulting partner brief for [partner]", "SI
  partner deck", "joint solution brief", or "enablement guide for [firm]".
metadata:
  version: "0.2.0"
  phase: "4"
...
- **`profiles/<active>/PROFILE.md`** — `output_folder`, target markets, budget caps.
  **No palette lives here** — resolve colour and type from the brand kit:
...
- **`knowledge/brand-notes.md`** — the documented image-generation prompts (it declares no colours).
  Take the generated imagery's constraints from the brand kit's `imagery.style` and
  `imagery.negative` rather than restating them inline — `negative` already bans text, logos and
  faces in abstract renders, and an inline paraphrase drifts from it.
