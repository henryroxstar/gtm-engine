...
name: email-quality
description: >-
  Close the loop between reading emails and improving them. Runs the four modes of the
  email-quality program: `judge` scores every row of a staged sequence against the rubric with
  a cheap pinned model and writes a verdict per row; `sheet` builds the blind labeling sheet
  an operator fills in and seals a class-balanced holdout from it; `apply` turns those human
  labels into durable changes to the next send list — disqualifying bad-fit accounts and
