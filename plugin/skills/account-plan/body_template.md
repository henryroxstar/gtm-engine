...
Flag single-threaded risk if only one contact is identified.
...
- **Compliance-as-wedge angle** (if regulatory pressure is a top trigger): cite the specific regulator + the eeCheck/Elderwise compliance framing from case-studies.md.
...
List up to 3 things that would materially change the plan if answered differently (e.g. "Does their CISO have an active AI risk mandate?", "Are they already evaluating Okta's ITDR product?"). These become the priority discovery questions for the first call.
