...
description: >-
  Prepare for a technical deep-dive by profiling the account's engineering stack and gathering
  functional scope (jobs-to-be-done, features, V1/V2 intuition), then producing a requirements
  question bank where every question is mapped to the design decision it unlocks. In Mode B
  (no product to map onto), profiles the prospect's manual workflow and tags the agent-shaped
  loop instead. Trigger when the user says "prep me for the technical deep-dive with
  [company]", "solution discovery for [company]", "what technical questions should I ask
  [company]", "profile [company]'s stack for the architecture call", "surface requirements for
  [company]", "SA prep for [company]", or "technical discovery for [company]". Business-level
  call prep is the `call-prep` skill — this one goes deeper technical and feeds
  `solution-design`. Reads PROFILE for markets/budget. Read-only research; never sends
  anything. Produces a discovery brief saved to the account folder
...
6. **Cohort dossier (only if the account's workflow maps to one; skip otherwise).** If the profile ships
   `knowledge/use-cases/` and the account's workflow matches one of the cross-org cohorts in
   `knowledge/use-cases/README.md`'s table, read only that one dossier — it feeds a **starting
   hypothesis**, not a confirmed fact, for §2 (dated agentic-shift evidence), §3 (trust map / multi-party
   questions worth adding to the question bank), and §5 (regulatory overlay, below). **Check freshness
   first:** read its "Sources & verification log" + "On refresh (Nd)" line; past that window, treat its
   time-sensitive claims as unconfirmed and mark them **unknown (ask)** like any other unverified stack
   fact rather than stating them as settled. Never upgrade a `Flagged`/`(~unverified~)` tag.
...
If a cohort dossier was loaded (Step 1, item 6), its §5 regulatory overlay names the use-case-specific
regs on top of the jurisdiction/audience defaults above — cite both, and flag any dossier claim past its
refresh window as unconfirmed rather than citing it as settled.
...
Give the must-ask bank a business-value spine, not just technical checklist items: where a stack gap
has a cost or risk attached, phrase it as an **implication question** ("what does this exposure cost
you if it isn't fixed before [compelling event]?"), not just a fact-finding one — per
`docs/sales-questions-by-deal-phase.md` (Phase 4). This is what lets a must-ask answer double as
ammunition for the value case, not just a design input.
...
- **Give the must-ask bank a business-value spine.** Frame stack-gap questions with a cost/risk attached
  as implication questions per `docs/sales-questions-by-deal-phase.md` (Phase 4) — question *content*
  still comes from `profiles/<active>/products/<product>/references/discovery-question-bank.md`.
...
- **Revisions carry a Version log.** A re-run/revision of an existing discovery brief (v2+) appends a
  short version log (v · date · section · change · why) so a reviewer can trace what moved; a first draft
  carries none.
