...
Fill the section skeleton in [`references/brief-template.md`](references/brief-template.md) — headings,
front-matter fields, and table columns come from there so every brief scans the same way. The
sections below say what goes in each. Keep each section tight.
...
If multiple attendees, identify the economic buyer vs. champion vs. influencer. If a cohort dossier was loaded, sharpen the economic-buyer and CISO/security-seat framing with its §7 dual buyer map instead of the generic persona card alone.
...
| Objection | Opening move | Rebuttal |
|---|---|---|
| (most likely for this persona) | (label the concern + one calibrated "how/what" question — see below) | (draw from `profiles/<active>/knowledge/product.md` competitive differentiation + `profiles/<active>/knowledge/case-studies.md` proof; if a cohort dossier was loaded, prefer its §8 cohort-specific objection over the generic one) |
| … | … | … |
| … | … | … |
...
Coach the opening move per `docs/sales-questions-by-deal-phase.md` (Phase 5): pause before responding, **label** the concern ("It sounds like the worry is…"), then a **calibrated question** ("How am I supposed to make this work within [their constraint]?") before the rebuttal — don't leap straight to counter-arguing. If price is likely to come up, don't coach the colleague to defer it — flag it as fair game for call one.

### 7. Discovery questions (5–7)
Pull from the discovery questions in `profiles/<active>/knowledge/product.md`; adapt to the meeting type and stage. Sequence per `docs/sales-questions-by-deal-phase.md` (Phase 2/4) — situation/problem questions first, then at least one **implication** question (cost/risk of the status quo) for first-touch discovery; add need-payoff framing for later-stage. This 5–7 is a sharp seed set for the brief — coach the colleague to expand it with genuine follow-ups on the call itself (the full call typically runs ~11–14 questions).

### 8. The ask
One specific, low-commitment action to close this meeting with, framed as a **concrete next step with a named owner and date** — not an open "let me know what you think" (`docs/sales-questions-by-deal-phase.md`, Phase 6). Match to stage:
...
- Sequence discovery questions, coach objection-handling, and frame the ask per the evidence-based method in
  `docs/sales-questions-by-deal-phase.md` — question *content* still comes from `profiles/<active>/knowledge/product.md`.
