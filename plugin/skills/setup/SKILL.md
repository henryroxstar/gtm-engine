...
name: setup
description: >-
  Guided one-time onboarding for the GTM engine plugin. This skill should be used when the
  user says "set me up", "set up gtm-engine", "onboard me", "configure my GTM profile", "get
  me started", or is running the plugin for the first time. Learns the company from their own
  site (or whatever they hand over), asks only the gaps a website can't answer, stages a full
  profile bundle for the founder to review before anything goes live, walks them through
  connecting optional tools (Vibe Prospecting, RocketReach, Firecrawl, Higgsfield) without
  ever storing a key in a file, captures each connected tool's plan/monthly-allowance/features
  so a flat-subscription tool can be paced against its real limit instead of only the dollar
  budget, and proves value with a real first output: extract → review → promote → first
  output.
metadata:
  version: "0.6.0"
  phase: "1"
...
6. **Scheduling link** *(skippable)* — their Calendly/booking URL, if they use one, so reply-stage drafts can offer a time. Skip if they don't book this way; blank is fine. Optionally mention: connect a CRM (HubSpot/Salesforce) inside *their own* Calendly plan — theirs to turn on, not ours.

Recommend the defaults explicitly; don't make them guess. Merge every answer into the draft's `settings` block, and fill in anything `gaps[]` flagged that their answer resolved — keep editing the same temp file from Step 2, don't start a new one. **Exception — the scheduling link:** `booking_url` is a PROFILE prose line (like `email_tool`), **not** a `ProfileDraft` settings field, so don't put it in the `settings` block. If they gave one, set the promoted profile's `booking_url:` line in `PROFILE.md` after Step 4; if they skipped, leave it blank.
...
- **Read back the buyer-journey layer you derived** — the top 2-3 "why now" triggers, and the journey/objections for the primary buyer — in one or two plain lines: *"I mapped when to reach these buyers (a new funding round, a new VP hire) and the objection to expect early ('this is just how it is')."* This layer is **inferred from your personas**, not taken off your site, so it needs a harder look than the rest — see the confirm items below.
...
**Walk the buyer-journey confirm items explicitly.** The journey/triggers/objections are inferred, so the draft's `buyer_journey.operator_confirm[]` (rendered as the ⚠️ list in `buyer-journey.md`) names exactly what only they know — real sales-cycle length, which trigger actually closes, who signs vs. who champions, the objections truly heard in the room. Ask these as plain questions and fold their answers back into the draft's `buyer_journey` before you re-render, the same way you handle any other change below. If they'd rather answer later, that's fine — the ⚠️ items stay in the file as a to-do; don't block go-live on them.
...
**For every tool the user just confirmed as connected, ask its plan in the same `AskUserQuestion` batch** — a name (e.g. "Ultimate + Phone", "Boost pack"), the real monthly allowance if they know it (a number + unit, e.g. "1000 lookups/mo" — offer "not sure, flag for later" as a valid answer rather than blocking on it), and which premium features it includes (e.g. RocketReach's intent data and phone are plan-gated — a lower tier may silently drop the facet rather than error, so knowing this now prevents `prospect` from assuming a filter works when it doesn't). This matters because a flat-subscription tool like RocketReach bills near-$0 per call, so the dollar budget from Step 3 can never cap its real limit — only a captured plan/allowance can. Merge each answer into `settings.connectors[]` (one entry per tool: `name`, `plan`, `billing`, `monthly_allowance: {unit, limit}`, `features[]`, and `notes` for anything they weren't sure of) alongside the other settings from Step 3, then re-render so it lands in `PROFILE.md` §"Connector plans & entitlements". Skip this follow-up for a tool the user declined to connect.
