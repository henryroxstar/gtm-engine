...
description: >-
  ...
  me started", or is running the plugin for the first time. Interviews the colleague,
  scaffolds their company profile bundle, walks them through connecting optional tools (Vibe
  Prospecting, Firecrawl) without ever storing a key in a file, and runs a 3-account proof.
metadata:
  version: "0.4.0"
  phase: "1"
...
Before the questions, orient them in about four lines so they know what they're walking into. Say it
in your own words — plainly, no file paths or jargon:

- **What I need from you:** a few basics (your name + how you sign off, target markets, a budget cap)
  and — for outreach that actually sounds like you — some raw material: company/product background, a
  case study or two, and how you write (a voice guide, or just a couple of your own emails as
  samples). Basics I'll ask you inline; material you can hand me whenever it's easy.
- **How I use it:** it becomes *your* private knowledge pack that every draft reads — so outreach
  sounds like you and stays true to your facts. It's stored with your profile; nothing is sent
  anywhere without you.
- **What I do vs. what you do:** *I* do the setup — write the files, build your profile, produce a
  sample run. *You* only (a) hand me material when you have it, and (b) connect any paid tools
  yourself in the app — I never see or touch your keys or passwords.
- **How to hand me material:** whatever's easiest — paste it into chat, point me at a file or folder
  on your machine, or drop documents into your **source inbox** (`knowledge/source/`) and I'll read
  and condense them. More material means sharper drafts, but you can start with almost nothing and
  add more later.

Keep it to those four beats, then move into the questions.
...
2. **`profiles/<company>/knowledge/`** — the company knowledge pack (company.md, product.md, icp-personas.md, case-studies.md, voice.md, voice-bans.txt, audience-psychology.md, and any guidance/source/brand subfolders). Create the folder; populate it from the company's own material. If the colleague handed you material any way — pasted in chat, a file/folder they pointed you at, or documents dropped into the **`knowledge/source/`** inbox — condense it into the pack via the `profile-onboard` flow (raw files stay in `source/` as provenance; the condensed topics are what skills read). If they have nothing yet, leave clearly-marked stub files and tell them they can add material to `knowledge/source/` any time and say "learn from my material" — no need to redo setup. `voice-bans.txt` (the machine-readable ban list the prose linter reads) is seeded from the voice `ban_list`; `audience-psychology.md` is built with `docs/audience-psychology-method.md`.
