...
values (`fabric`/`elements`/`radix`/`messaging`/`forge`/`website`) are the deck engine's six
fixed slot names — arbitrary theme-engine tokens, not any company's product names. Map the
active company's products onto them **by capability, never by any one tenant's product
names** (see the product → page theme mapping in `carousel-pdf/SKILL.md`):
