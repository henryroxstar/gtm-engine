...
class IngressHandlers(CockpitComponent):
    ...
    def _record_house_note(self, profile: str, notes: str) -> None:
        """Keep a gate edit as a dated house note, when this profile has a brief to learn about.

        An Edit at the plan gate is the clearest statement of taste this system ever gets, and it
        used to be spent on one draft and forgotten. Recorded only when the profile has at least one
        creator brief: without one there is no video lane here to teach, and a note file that fills
        with unrelated corrections stops being read, which is the same as having none.

        Best-effort by design — a note that cannot be written must never cost the operator their
        edit.
        """
        try:
            ...
            logger.debug("house note not recorded for profile=%s", profile, exc_info=True)
    ...
    async def on_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if self._awaiting_edit.pop(chat_id, False):
            ...
            self._record_house_note(profile, text)
        else:
            prompt = text
    ...
    async def on_document(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Download an inbound CSV export and route it to the brain for bulk-mode ingest.

        This is the Phase 1.5 receiver for bulk-mode prospect discovery (a Vibe/
        Explorium ``export-to-csv`` download the operator forwards into the
        cockpit). Non-CSV documents fall through untouched — the manual file-drop
        under ``content/<profile>/prospects/imports/`` remains the fallback path
        when this handler isn't reachable (e.g. a pure-headless VPS runtime with no
        Telegram operator in the loop).
        """
        if not self._is_allowed(update):
            return
        msg = update.message
        ...
        doc = msg.document
        is_csv = (doc.mime_type or "").strip().lower() in (
            "text/csv",
            "application/vnd.ms-excel",
        ) or (doc.file_name or "").lower().endswith(".csv")
        if not is_csv:
            return  # not a file type we handle here
        ...
        if doc.file_size and doc.file_size > MAX_CSV_BYTES:
            await msg.reply_text(
                f"{header}⚠️ File is larger than {MAX_CSV_BYTES // (1024 * 1024)} MB."
            )
            return
        ...
        try:
            tg_file = await doc.get_file()
            data = bytes(await tg_file.download_as_bytearray())
            dest = save_inbound_csv(self.cfg.content_root, profile, data)
        except ValueError as exc:
            await msg.reply_text(f"{header}⚠️ Could not accept file: {exc}")
            return
        except (OSError, telegram.error.TelegramError) as exc:
            logger.exception("CSV ingestion failed for chat_id=%s", chat_id)
            await msg.reply_text(
                f"{header}⚠️ Could not save file: {html.escape(type(exc).__name__)}."
            )
            return

        prompt = build_csv_prompt(dest, msg.caption)
    ...
    async def on_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        when STT yields nothing or a downstream step fails.
        """
        if not self._is_allowed(update):
            return
        msg = update.message
        ...
        if self._awaiting_edit.pop(chat_id, False):
            ...
            self._record_house_note(profile, transcript)
        else:
            prompt = transcript
