...
def _patched(conn):
    """Fake the DB everywhere the request path binds it — the router plus every service
    module (GET /runs reads through the queries service since Phase 1b), and deps."""
    scope = _scope_factory(conn)
    return (
        patch_everywhere(SCOPE_MODULES, "workspace_scope", scope),
...
def test_stream_heartbeat_on_idle():
    ...
    async def _go():
        with (
            patch_everywhere(SCOPE_MODULES, "workspace_scope", _scope_factory(conn)),
            patch.object(runs_stream, "_STREAM_HEARTBEAT_S", 0.05),
        ):
            req = _FakeRequest(MagicMock())
            resp = await runs_router.stream_run(RUN_ID, _ws(), req)
            it = resp.body_iterator
