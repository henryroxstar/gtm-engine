...
def test_d7_does_not_accept_a_background_image_as_the_visual():
    ...
    Six slides of a customer deck each carried a generated background image, passed D7
...
def _real_theme():
    """Resolve the real shipped theme: the in-repo home by default (since the 2026-08-29
    ownership flip, PRD §3.0.1), or `GTM_DECK_THEME` for a deliberate override. Shared by
    every test below that regresses against the actual theme rather than a tmp_path
    fixture — skips (never fails) when neither is present, so these stay opt-in on a
    checkout that genuinely lacks the theme rather than failing CI there."""
    import os
    from pathlib import Path

    root = Path(__file__).resolve().parents[2]
    default_theme = root / "docker" / "deck-renderer" / "deck-theme"
    configured = os.environ.get("GTM_DECK_THEME")
    if configured:
        theme = Path(configured).expanduser()
    elif default_theme.exists():
        theme = default_theme
    else:
        pytest.skip("no in-repo theme and GTM_DECK_THEME unset — nothing to check")
    if not theme.exists():
...
def test_the_shipped_theme_has_no_errors():
    """T17 (PRD 2026-08-29-deck-visual-craft-hardening.md §3.5/§8): the theme regression
    fixture for D9 — a real deck-theme must pass its own audit.

    Before the 2026-08-29 ownership flip (PRD §3.0.1) the theme lived in a separate deck
    workspace, not in this repo, at a machine-specific path — so this came from
    `GTM_DECK_THEME` and skipped when unset. That made it a gate nothing ran by default:
    `uv run pytest` with no env var set always skipped, silently, so the regression this
    docstring claims to hold never actually ran in CI or in a plain local run. Since the
    flip the theme IS an in-repo path (`docker/deck-renderer/deck-theme/`, no company or
    product token in it — the de-brand RELEASE gate has nothing to object to), so default
    to it and only fall back to `GTM_DECK_THEME`/skip for a deliberate override.
    """
    theme = _real_theme()
...
def test_no_animated_blur_in_keyframes():
    """T14 (PRD §4/D2.6, §8 test manifest): the de-blur fix, pinned directly rather than
    through D9's severity threshold. All six animated blurs (five `@keyframes` plus
    `.slidev-vclick-hidden`, a `transition` D9's keyframes-only scanner cannot see) are
    gone from the real theme's motion CSS — `filter: blur` does not appear as an animated
    endpoint anywhere in `styles/animations.css`."""
    import re

    theme = _real_theme()
    animations = theme / "styles" / "animations.css"
    assert animations.exists(), f"expected {animations} to exist"
    text = animations.read_text(encoding="utf-8")
    # Strip /* … */ comments first — the file documents the historical fix in prose,
    # including the literal string "filter: blur()", which is not live CSS.
    code = re.sub(r"/\*.*?\*/", "", text, flags=re.DOTALL)
    assert "blur(" not in code, (
        "styles/animations.css references filter: blur(...) in live CSS — D2.6 removed "
        "every animated blur (five @keyframes plus the .slidev-vclick-hidden transition); "
        "a new one regresses the export-bloat defect D2.6 fixed"
    )


def test_ambience_not_in_layouts():
    """T15 (PRD §4/D2.1, §8 test manifest): `CanvasAmbience` — the sub-brand accent layer
    D2.1 centralised into `global-bottom.vue` so it crossfades once per deck instead of
    cutting on every slide advance — is instantiated in exactly that one file among the
    theme's non-carousel surfaces, not per-layout.

    `MeshAurora`/`PulseHalo` are a *different* thing: layout-specific decorative
    backgrounds each of the seven deck layouts still legitimately varies (cover's halo
    isn't chapter's), never centralised by D2.1 and out of scope here. The carousel-*
    layouts also keep their own `CanvasAmbience` on purpose — PRD §9 open question 3,
    resolved: upstreaming carousels into a keynote-only global layer was rejected, so
    they're excluded from this check rather than silently exempted."""
    theme = _real_theme()
    layouts_dir = theme / "layouts"
    deck_layouts = sorted(
        p for p in layouts_dir.glob("*.vue") if not p.name.startswith("carousel-")
    )
    assert deck_layouts, f"expected non-carousel layouts under {layouts_dir}"
    offenders = [p for p in deck_layouts if "CanvasAmbience" in p.read_text(encoding="utf-8")]
    assert offenders == [], (
        f"CanvasAmbience should live only in global-bottom.vue, not in a per-slide layout: "
        f"{[p.name for p in offenders]}"
    )
    global_bottom = theme / "global-bottom.vue"
    assert global_bottom.exists(), f"expected {global_bottom} to exist"
    assert "CanvasAmbience" in global_bottom.read_text(encoding="utf-8")
