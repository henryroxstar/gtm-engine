...
# --- bare domains on code surfaces (2026-09-03) ------------------------------------- #
#
# The identity read of the 2026-09-03 cut found nineteen real organisations — prospects'
# parent/subsidiary pairs, a bank's brand-vs-legal split, health systems' .edu — as bare
# domains in docstrings, comments and fixture rows: no email, no URL, no tenant token, so
# every other rule passed them. As above, the samples here are synthetic, unlisted domains
# in the SHAPE of what leaked; never the real ones.


def _scan_at(tmp_path: Path, rel: str, body: str) -> int:
    path = tmp_path / rel
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(body, encoding="utf-8")
    return pii_check.main([str(path)])


@pytest.mark.parametrize(
    "leak",
    [
        "# a brand mailing from its holding company: unlisted-bank.com vs unlisted-legal.com\n",
        '"""A health system on unlisted-hospital.edu against unlisted-clinic.org."""\n',
        'PAIRS = {"unlisted-corp.co": "unlisted-corp.io"}\n',
        'url = "https://unlisted-prospect.com/newsroom"\n',  # a URL host, now checked outside tests/
    ],
)
def test_a_real_looking_bare_domain_on_a_code_surface_is_caught(tmp_path, leak):
    assert _scan_at(tmp_path, "gtm_core/mod.py", leak) == 1


@pytest.mark.parametrize(
    "ok",
    [
        "value = request.app.state.pool\n",  # attribute access is code, never a candidate
        "org = args.org.strip()\n",
        "x = self.net\n",
        "# a stand-in on a reserved TLD: acme.example, and mail.example.com\n",
        '"""Vendor endpoint: api.anthropic.com (allowlisted vendor)."""\n',
        "# the free-mail list holds gmail.com as a RULE INPUT\n",
        "# legacy fictional fixture on a real TLD, listed by name: acme.com\n",
        'row = ["a@x.com", "x.com"]\n',  # single-letter placeholder hosts
        "v = 1.2.3\n",  # no TLD
    ],
)
def test_code_that_is_not_a_domain_or_is_provably_fictional_does_not_fire(tmp_path, ok):
    assert _scan_at(tmp_path, "gtm_core/mod.py", ok) == 0


def test_the_bare_domain_rule_reads_only_strings_and_comments_in_python(tmp_path):
    """The same token is a finding as data and silent as code."""
    assert _scan_at(tmp_path, "gtm_core/a.py", "# see unlisted-thing.com\n") == 1
    assert _scan_at(tmp_path, "gtm_core/b.py", "value = unlisted_thing.com\n") == 0


def test_the_bare_domain_rule_is_scoped_to_code_surfaces(tmp_path):
    """docs/ and skill prose cite vendors and published benchmarks by design."""
    cite = "Source: unlisted-research.com, 2026 benchmark report.\n"
    assert _scan_at(tmp_path, "docs/guide.txt", cite) == 0
    assert _scan_at(tmp_path, "plugin/skills/x/body_template.md", cite) == 0
    assert _scan_at(tmp_path, "plugin/skills/x/scripts/helper.py", "# " + cite) == 1
    assert _scan_at(tmp_path, "profiles/_template/knowledge/company.md", cite) == 1
    assert _scan_at(tmp_path, "packs/x/graphs/y.toml", 'note = "' + cite.strip() + '"\n') == 1
