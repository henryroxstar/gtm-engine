...
#:
#: A PRICE is deliberately not matched HERE. "10 credits vs seedance's 36", "1 media credit per
#: billed minute", "~23 credits/min" are rate-card facts: they change on a vendor release cadence,
#: they are what a cost ESTIMATE is built from, and banning them outright would delete C5. The
#: discriminator for THIS rule is a balance word next to the figure, not the figure itself.
#:
#: Where a rate may LIVE is a different question, and §R17 answers it
#: (`tests/lint/provider_rate_check.py`): a rate belongs in the one module that owns it, pointed
#: at from everywhere else. This rule permitting a rate is not this rule endorsing a copy of one.
