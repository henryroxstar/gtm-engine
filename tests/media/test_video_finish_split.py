...
`split` is the inverse of `stitch` and exists because of a gap the 2026-08-28 cost fix opened.
`video-avatar` Step 3 reversed to rendering the presenter's whole script as ONE continuous take
(HeyGen bills per render per minute rounded up, so eight beats cost 8x one take). That made the
presenter footage a single file while `video-render`'s insert shots stayed separate files meant
to be intercut with it — and `stitch` only joins. `Bash(ffmpeg:*)` is denied, so "cut it locally"
