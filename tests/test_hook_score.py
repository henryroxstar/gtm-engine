...
# ── the prior is unchanged by the story_format tag (S8-T3, 2026-09-07) ────────────────


def _bank(content_root: Path, profile: str, *, story: bool) -> None:
    """Four rows over two hooks, optionally carrying the S8 tag on every one of them.

    The hook is deliberately BELOW its profile baseline (5% against 10%), so the prior lands
    mid-range rather than clamped at 0 or 100 — a saturated score cannot move, and a fixture that
    cannot move makes the equality test next to it meaningless.
    """
    extra = ["story_format:true"] if story else []
    for hook, impressions, engagements in (("acme-augmentation", 900, 45), ("other-hook", 600, 60)):
        oc.append_outcome(
            content_root,
            profile,
            {
                ...
                "outcome": "impressions",
                "value": impressions,
                "tags": [f"hook:{hook}", "format:linkedin-text", *extra],
            },
        )
        oc.append_outcome(
            content_root,
...
def test_the_prior_is_identical_with_and_without_the_story_format_tag(tmp_path: Path) -> None:
    """S8-T3. Tagging changes what a row SAYS, never what the scorer computes from it.

    `_compute_prior` reads `outcome`, `value`, and tags matching `hook:`/`format:`. An extra
    prefix is invisible to all three — but "invisible by inspection" is how a coupling gets
    introduced later, so pin the number rather than the reasoning.
    """
    plain = tmp_path / "plain"
    tagged = tmp_path / "tagged"
    for root, story in ((plain, False), (tagged, True)):
        (root / "testco").mkdir(parents=True)
        _bank(root, "testco", story=story)

    without = hs._compute_prior("acme-augmentation", "linkedin-text", plain, "testco")
    with_tag = hs._compute_prior("acme-augmentation", "linkedin-text", tagged, "testco")
    assert with_tag == without, (
        f"the story_format tag moved the prior: {without} -> {with_tag}. The tag records what a "
        "piece WAS; it must never feed the score that decides what ships"
    )


def test_the_prior_fixture_is_sensitive_enough_to_detect_a_change(tmp_path: Path) -> None:
    """S8-T3. Positive control: the equality above is evidence only if this bank can move it.

    Two identical constant returns would satisfy the test regardless of what the tag did.
    """
    base = tmp_path / "base"
    moved = tmp_path / "moved"
    (base / "testco").mkdir(parents=True)
    (moved / "testco").mkdir(parents=True)
    _bank(base, "testco", story=False)
    _bank(moved, "testco", story=False)
    oc.append_outcome(
        moved,
        "testco",
        {
    ...
    assert hs._compute_prior(
        "acme-augmentation", "linkedin-text", moved, "testco"
    ) != hs._compute_prior("acme-augmentation", "linkedin-text", base, "testco"), (
        "a real engagement change did not move the prior — the fixture proves nothing"
    )
