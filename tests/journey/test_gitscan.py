...
# ── regression: commit body containing "changed" must not be dropped ─────────


def test_commit_body_with_changed_word_is_not_dropped(tmp_path):
    """A commit whose message body contains the word 'changed' must still parse.

    Regression for the shortstat-heuristic bug: the parser treated ANY line
    containing the substring 'changed' as git's ' N files changed, …' summary,
    stripped the SHA/subject header before it, and dropped the commit — so a
    depth-1 checkout of such a commit yielded an empty list (CI failure on the
    'Release: dev → main' squash-merge, whose body says '…on the changed files').
    """
    env = {
        "GIT_AUTHOR_NAME": "t",
    ...
    _git(
        ...
        "feat(scan): a commit",
        "-m",
        "Verified `p/security-audit` on the changed files -> 0 findings.",
    )

    result = commits(repo_root=tmp_path)
    assert len(result) == 1
    assert result[0].subject == "feat(scan): a commit"
    assert "changed files" in result[0].body
