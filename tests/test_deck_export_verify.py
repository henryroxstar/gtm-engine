...
theme rasterizing something it shouldn't (too heavy) — the second is what caught the Appistoki
...
def test_weight_ceiling_flags_a_bloated_export(tmp_path):
    pdf = tmp_path / "d.pdf"
    pdf.write_bytes(_fake_pdf(15, int(1700 * 1024)))  # 1700 KB/page — the Appistoki bug
    rc = dev.main([str(pdf), "--pages", "15"])
    assert rc == 1
