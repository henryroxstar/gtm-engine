...
# LinkedIn
LI_HOOK_MAX = 140  # mobile "see more" fold — a hard cap, not a performance number
# Performance target, distinct from the cap above. Across 309,614 LinkedIn posts (AuthoredUp,
# Dec 2025 - May 2026) median engagement falls monotonically with hook length: 0-40 chars 2.61%,
# 41-80 2.39%, 81-120 2.23%, 121-200 2.15%, 200+ 2.08%. 80 captures the top two bands. Warn, never
# error: the cap is a platform constraint, this is advice, and a legitimate hook may exceed it.
LI_HOOK_TARGET = 80
...
# Spans wrapped in double quotes (straight or curly) are treated as verbatim source quotes and are
# exempt from the word/char-level prose rules (ban list, em dash): you quote a source exactly or
# cut it — you never reword a quote to satisfy a linter. See docs/prose-craft.md.
_QUOTED_SPAN_RE = re.compile(r"\"[^\"]*\"|“[^”]*”")


def _mask_quoted(text: str) -> str:
    """Blank out double-quoted spans so verbatim quotes don't trip word/char-level prose lints.

    Replaces each quoted span with spaces of equal length (preserving offsets); the same word
    appearing OUTSIDE quotes is still checked.
    """
    return _QUOTED_SPAN_RE.sub(lambda m: " " * len(m.group(0)), text or "")
...
def lint_linkedin(asset: dict) -> list[Violation]:
    ...
    elif len(hook) > LI_HOOK_TARGET:
        v.append(
            Violation(
                "warn",
                "li.hook",
                f"hook is {len(hook)} chars (>{LI_HOOK_TARGET} target) — shorter hooks measure "
                "better; tighten it if the meaning survives",
            )
        )
...
# Flattery-pivot opener: praise the post/person, then pivot on a dash (docs/prose-craft.md #9).
# e.g. "Great point, X —", "Strong list — and Y's take is right". Anchored to the very start and
# bounded to one clause (no sentence-ending punctuation before the dash) so it only catches the
# compliment-then-pivot shape, not an ordinary sentence that happens to open with "great"/"strong".
_FLATTERY_OPEN_RE = re.compile(
    r"^\s*(great|strong|solid|excellent|nice|good|love (this|that))\b[^.!?]{0,60}?(—| - )",
    re.IGNORECASE,
)

# Listicle-in-a-sentence: 2+ semicolons in one paragraph stitching parallel clauses together and
# dressing them as prose (docs/prose-craft.md #10). Checked per-paragraph so a long-form piece with
# scattered, ordinary semicolon use elsewhere doesn't get flagged.
_LISTICLE_SEMICOLON_MIN = 2

# Acronyms assumed universally readable without a spelled-out first use — extend as needed.
...
# "Full Name (ACRONYM)" or "ACRONYM (Full Name)" — either order counts as an expansion.
def _acronym_expanded(acr: str, text: str) -> bool:
    return bool(
        re.search(r"[A-Za-z][\w\s&/-]{2,60}\(\s*" + re.escape(acr) + r"\s*\)", text)
        or re.search(re.escape(acr) + r"\s*\(([^)]{3,80})\)", text)
    )
...
def lint_prose_quality(text: str, extra_bans: tuple[str, ...] = ()) -> list[Violation]:
    ...
    """
    text = text or ""
    # Verbatim quotes are exempt from the word/char-level rules (ban list, em dash) — mask them so
    ...
    for m in _ACRONYM_RE.finditer(text):
        acr = m.group(0)
        base = acr[:-1] if acr.endswith("s") and acr[:-1] in _ACRONYM_ALLOWLIST else acr
        if base in _ACRONYM_ALLOWLIST or acr in seen_acronyms:
            continue
        seen_acronyms.add(acr)
        if not _acronym_expanded(acr, text):
            v.append(
                Violation(
                    "warn",
                    "prose.acronym_unexpanded",
                    f"{acr!r} never spelled out — give the full form once, e.g. "
                    f"'Full Name ({acr})'",
                )
            )
    ...
    for opener in _GENERIC_OPENERS:
        if stripped_low.startswith(opener):
            ...
            break
    if _FLATTERY_OPEN_RE.search(unquoted):
        v.append(
            Violation(
                "warn",
                "prose.flattery_open",
                "opens with a compliment then pivots — skip the flattery, lead with the point",
            )
        )
    seen: set[str] = set()
    for term in tuple(_AI_TELLS) + tuple(extra_bans):
        ...
        if key in seen:
            continue
        if re.search(r"(?<!\w)" + re.escape(term) + r"(?!\w)", unquoted, re.IGNORECASE):
            seen.add(key)
            v.append(
                Violation(
                    "warn",
    ...
    for para in re.split(r"\n\s*\n+", unquoted):
        if para.count(";") >= _LISTICLE_SEMICOLON_MIN:
            v.append(
                Violation(
                    "warn",
                    "prose.listicle_sentence",
                    "2+ semicolons stitch parallel clauses into one paragraph — that's a list "
                    "wearing a sentence's clothes; break it into an actual list or cut to the "
