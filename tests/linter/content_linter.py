#!/usr/bin/env python3
"""Content linter — enforces the platform playbooks so every asset
passes before review. Stdlib-only (no deps), runnable as a CLI gate and importable by
content-studio. Rules from docs/{linkedin,x,facebook,instagram}-optimization.md.
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Content linter")
