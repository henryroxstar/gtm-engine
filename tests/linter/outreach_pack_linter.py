...
passing the word-level lint. These checks are the mechanical subset with teeth:
  ...
  Per-email
    ...
    - CTA: ends on a question, offers one artifact the profile can produce
      (--artifact-file), names what it does for them, never a time-ask
...
_NAME_RE = re.compile(
    r"^(?!(?:name|first|firstname|there|team|unconfirmed|unknown|tbd|none)$)"
    r"[^\W\d_][\w'\-.]*$",
...
# not know how. Flagged by the operator 2026-08-19 reading the Cross River draft.
#
...
# inside "excess and surplus book and workbench..." (found 2026-08-12, Hanover
# Insurance) because "book a" is literally a substring of "book and". `\bTOKEN\b`
...
_TOUCH_RE = re.compile(
    r"^[-*\s]*\*\*Touch\s+(?P<n>\d+)\s*[—–-]\s*Day\s+\d+\s*[—–-]\s*(?P<channel>\w+)",
    re.MULTILINE,
)
_SEQ_HEADER_RE = re.compile(r"^##\s*Sequence\s*[—–-]\s*(?P<n>\d+)\s+touches", re.MULTILINE)
_STATED_WORDS_RES = (
...
#   ## Email (touch 1)
#   ## Email (touch 1, to Gregory Wright)      <- supplement packs name the recipient
...
# Honorifics are not first names: "Dr Hongjuan Liu" greets Hongjuan, not "Dr".
...
def _parse_sequence(text: str) -> tuple[int | None, int, tuple[str, ...]]:
    ...
    touches = [(int(t.group("n")), t.group("channel")) for t in _TOUCH_RE.finditer(text)]
    touches.sort(key=lambda p: p[0])
    return declared, len(touches), tuple(c for _, c in touches)
...
def parse_prospect_pack(text: str) -> tuple[str | None, list[EmailBlock], PackMeta]:
    ...
    **Subject:** agent audit trail
    ...
    company = (lambda m: m.group(1).strip() if m else "")(
        re.search(
            r"^#\s*Outreach Pack\s*[—–-]\s*(.+?)\s*[—–-]\s*\d{4}-\d{2}-\d{2}", text, re.MULTILINE
        )
    )
...
def _anchors(body: str) -> set[str]:
    ...
    for m in re.finditer(r"(?<![.!?]\s)(?<!^)(?<!\n)\b([A-Z][A-Za-z0-9'-]+)", body):
        ...
        if tok in ANCHOR_STOPLIST:
            continue
        # Short ALLCAPS tokens used to be excluded wholesale, which made DTCC, WEX, UKG
        # and U.S. (Bank) invisible as named facts and failed renders whose only "flaw"
        # was a short brand name (2026-08-19). The stoplist above already carries the
...
def persona_of(header: str) -> str | None:
    ...
    for persona, cues in _PERSONA_RULES:
        if not any(c in low for c in cues):
            continue
        if any(a in low for a in _ANTI_CUES.get(persona, ())):
            continue
        return persona
    return None
...
def lint_email(
    ...
    if gift_artifacts and _OFFER_RE.search(last):
        if not any(
        ...
        if meta.touch_channels[0].lower().startswith("email"):
            v.append(
                Violation(
                    ...
                    meta.label,
                    "channel-order",
                    "touch 1 is email while a LinkedIn DM exists — lead with LinkedIn, then email",
                )
            )
    return v
...
_GOOD = """Rules-Version: 2026-08-19
