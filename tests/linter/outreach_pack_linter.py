...
def lint_email(
    ...
    # CTA must carry its own value, not just point at "the one-pager" in the abstract
    if re.search(r"\b(one-?pager|demo)\b", last, re.IGNORECASE) and not _anchors(last):
        v.append(
            Violation(
                "WARN",
                b.label,
                "cta-unanchored",
                f"CTA names the gift but no specific gap/mechanism in the ask itself: {last!r}",
            )
        )
