...
def test_linkedin_hook_over_target_warns_but_passes():
    """The warn tier that never fired. Before 2026-08-24 this branch read
    `elif len(hook) > LI_SEE_MORE` with LI_SEE_MORE=210 sitting ABOVE LI_HOOK_MAX=140, so it
    could only trigger for a hook that was simultaneously <=140 and >210 — unreachable for every
    possible input, and no test exercised it, which is how it survived. LI_HOOK_MAX stays the
    error (it is the mobile fold); LI_HOOK_TARGET is advice and must never block."""
    a = {"platform": "linkedin", "format": "text", "hook": "h" * 100, "body": "x" * 1500}
    assert passes(a)  # advisory only
    warns = [v for v in lint(a) if v.rule == "li.hook"]
    assert len(warns) == 1
    assert warns[0].severity == "warn"
    assert "target" in warns[0].message


def test_linkedin_hook_at_target_is_silent():
    a = {"platform": "linkedin", "format": "text", "hook": "h" * 80, "body": "x" * 1500}
    assert passes(a)
    assert [v for v in lint(a) if v.rule == "li.hook"] == []


def test_linkedin_hook_over_max_errors_without_also_warning():
    """Error and warn are one ladder, not two independent checks — a 141-char hook reports the
    cap once, not the cap plus the target."""
    a = {"platform": "linkedin", "format": "text", "hook": "h" * 141, "body": "x" * 1500}
    hook_violations = [v for v in lint(a) if v.rule == "li.hook"]
    assert len(hook_violations) == 1
    assert hook_violations[0].severity == "error"
...
# ── prose quality: verbatim-quote exemption (docs/prose-craft.md) ─────────────


def test_banned_word_inside_quote_exempt():
    # A banned word that is part of a verbatim source quote is not flagged.
    text = 'The expert wrote: "affected stakeholders lack systemic leverage to act."'
    assert not any(v.rule == "prose.banned_word" for v in lint_prose_quality(text, ("leverage",)))


def test_banned_word_outside_quote_still_flagged():
    text = "We leverage identity to close the gap."
    assert any(v.rule == "prose.banned_word" for v in lint_prose_quality(text, ("leverage",)))


def test_banned_word_flagged_outside_but_not_inside_same_text():
    # Same word: flagged for our wording, exempt for the quoted use → exactly one hit.
    text = 'We leverage it, and "systemic leverage" was the expert\'s phrase.'
    hits = [v for v in lint_prose_quality(text, ("leverage",)) if v.rule == "prose.banned_word"]
    assert len(hits) == 1


def test_em_dash_inside_quote_exempt():
    text = 'The report said: "governance — not technology — is the gap."'
    assert not any(v.rule == "prose.em_dash" for v in lint_prose_quality(text))


def test_curly_quotes_also_exempt():
    text = "The expert said “affected stakeholders lack systemic leverage.”"
    assert not any(v.rule == "prose.banned_word" for v in lint_prose_quality(text, ("leverage",)))


# ── prose quality: flattery-pivot opener (docs/prose-craft.md #9) ─────────────


def test_flattery_pivot_opener_warns():
    text = "Strong list, Mishari — and Harish's point is the right instinct."
    assert any(v.rule == "prose.flattery_open" for v in lint_prose_quality(text))


def test_flattery_word_without_pivot_passes():
    # "Great" opens the sentence but there's no dash-pivot within the clause — not the tell.
    text = "Great engineering teams still ship broken permission checks sometimes."
    assert not any(v.rule == "prose.flattery_open" for v in lint_prose_quality(text))


def test_plain_opinion_opener_passes():
    text = "The failure mode that worries me most isn't a broken permission."
    assert not any(v.rule == "prose.flattery_open" for v in lint_prose_quality(text))


# ── prose quality: listicle-in-a-sentence (docs/prose-craft.md #10) ──────────


def test_semicolon_listicle_warns():
    text = (
        "Whether logs are tamper-evident; whether approval becomes rubber-stamping; "
        "and whether injection holds across every modality."
    )
    assert any(v.rule == "prose.listicle_sentence" for v in lint_prose_quality(text))


def test_single_semicolon_passes():
    text = "The gate holds; the log does not lie."
    assert not any(v.rule == "prose.listicle_sentence" for v in lint_prose_quality(text))


def test_semicolons_in_different_paragraphs_pass():
    # One semicolon per paragraph, in two separate paragraphs — not a stitched-together listicle.
    text = "The gate holds; it always has.\n\nThe log doesn't lie; that's the point."
    assert not any(v.rule == "prose.listicle_sentence" for v in lint_prose_quality(text))
