...
def test_persona_mismatch_sourced_from_merged_data_is_not_the_copys_fault():
    # A recipient whose own signal clause contains "attribution" ("...InfluenceOS for
    ...
    assert _is_data_borne(v, "wishlink raises a series b to expand influenceos for attribution")
...
def test_a_digit_inside_the_company_name_is_not_a_stray_digit():
    ...
    # a blanket ban it also excluded every company whose NAME carries a digit — G2 Risk
    # Solutions and B2Gnow both had a verified trigger and no sendable clause.
    rows = [
        _row(
            email="g2@x.com",
            company="G2 Risk Solutions",
            signal_clause="G2 Risk Solutions added EverC AI powered merchant risk products",
        )
    ]
    v = lint_signal_relevance([_signal_touch()], rows)
...
def test_a_date_or_metric_in_the_clause_is_still_an_error():
    rows = [
        _row(
            email="g2@x.com",
            company="G2 Risk Solutions",
            signal_clause="G2 Risk Solutions raised 45 million for its AI merchant risk products",
        )
    ]
    v = lint_signal_relevance([_signal_touch()], rows)
