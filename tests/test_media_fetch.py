...
class TestAllowlist:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # heygen-product.s3-accelerate.amazonaws.com - pinned 2026-08-29 from the
            # audio_url of a live search_audio_sounds response (the licensed music/SFX
            # catalogue). A THIRD HeyGen surface, and the first on an S3 endpoint rather
            # than a heygen.ai one.
            "https://heygen-product.s3-accelerate.amazonaws.com/astral_generated_music/a.wav",
            # reap-video-main-bkt-prod — pinned 2026-08-29 from a live get_results clipUrl.
            # A DIFFERENT bucket from the reap_upload PUT host: files go up to one and come
            # back from the other, so neither pin implies the other.
            "https://reap-video-main-bkt-prod.s3-accelerate.amazonaws.com/studios/x/y.mp4",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "https://heygen-product.s3-accelerate.amazonaws.com.evil.example/x.wav",
            # The Reap UPLOAD bucket is pinned in reap_upload for PUT — it is deliberately NOT
            # a read host here. Each module's allowlist stands alone.
            "https://reap-user-upload-bkt-prod.s3-accelerate.amazonaws.com/x.mp4",
        ],
    )
    ...
    def test_s3_accelerate_pin_is_bucket_scoped_not_host_family(self):
        """The S3 entry admits ONE bucket, not the S3 transfer-acceleration endpoint.

        On an s3-accelerate endpoint the leftmost label is the bucket name, so
        `heygen-product.s3-accelerate.amazonaws.com` and `some-other-bucket.…` are
        different hosts. This is the control that would catch a future PR "simplifying"
        the entry to a `*.s3-accelerate.amazonaws.com` suffix match, which would admit
        every S3 bucket on the internet — a strictly worse failure than the CloudFront
        wildcard below, because anyone can create a bucket.
        """
        for host in (
            "some-other-bucket.s3-accelerate.amazonaws.com",
            "s3-accelerate.amazonaws.com",
            "heygen-product.s3.amazonaws.com",  # non-accelerate endpoint, not observed
        ):
            with pytest.raises(EgressRefused):
                _check_url(f"https://{host}/x.wav")
