...
def test_select_excludes_resolved_and_out_of_market(tmp_path):
    imports = [
        ...
        _row(
            "Thistle Ltd", "thistle.example", "united kingdom", "Commercial Banking"
        ),  # out of market
        _row(
    ...
    assert res["out_of_market_skipped"] == 1  # Thistle (UK)
    assert res["queued"] == 2
...
def test_select_ranks_high_cohort_first_and_writes_queue(tmp_path):
    ...
    assert int(rows[0]["icp_backlog_score"]) > int(rows[1]["icp_backlog_score"])
    assert res["queued"] == 2


def test_queue_does_not_write_a_column_named_score(tmp_path):
    """The spend ranking must not be named ``score``, or consolidate maps it onto ``GTM_Score``.

    ``prospects_consolidate.columns`` treats a bare ``score`` column as the qualification verdict
    (a 0-12 rubric). This queue carries a 0-53 spend ranking. While both were spelled ``score``,
    749 rows reached published HubSpot CSVs with a spend ranking in the qualification column.
    """
    from gtm_core.prospects_consolidate.columns import _ALIASES

    qualification_aliases = set(_ALIASES["score"])
    collisions = sorted(set(pb.QUEUE_COLS) & qualification_aliases)
    assert not collisions, (
        f"enrichment-queue column(s) {collisions} alias onto GTM_Score, the qualification "
        "verdict. Rename the queue column; the two scores are different scales."
    )
