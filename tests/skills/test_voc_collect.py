...
def _build_fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    # 1 Syften market-signals — digest files; the later embedded date must win.
    _write(croot / "market-signals" / "syften-digest-2026-07-04.md")
    _write(croot / "market-signals" / "syften-digest-2026-07-18.md")
    # 1a Weekly web sweep — produced by market-scan; separate source from Syften.
    ...
    # 1b–1f Phase-3 event lanes.
    _write(croot / "market-signals" / "own-product-2026-07-24.md")
    _write(croot / "market-signals" / "funding-moves-2026-07-28.md")
    _write(croot / "market-signals" / "regulatory-2026-07-28.md")
    _write(croot / "market-signals" / "incidents-2026-07-28.md")
    _write(croot / "market-signals" / "customer-moves-2026-07-28.md")
    # Harvest lanes that share the same directory.
    _write(croot / "market-signals" / "enterprise-signals-2026-07-28.md")
    _write(croot / "market-signals" / "market-intel-2026-07-28.md")
    _write(croot / "market-signals" / "harvest-raw" / "mcp-seps.jsonl", '{"d":"2026-07-28"}\n')
...
def test_speaker_map_is_fixed_per_source(tmp_path):
    ...
    assert src["web_sweep"]["speaker"] == "mixed"
    assert src["own_product_watch"]["speaker"] == "own-voice"
    assert src["funding_and_ma"]["speaker"] == "vendor-voice"
    assert src["regulatory_enforcement"]["speaker"] == "regulator-voice"
    assert src["incidents_benchmarks"]["speaker"] == "expert-lens"
...
def test_latest_picks_the_end_date_of_a_range_and_newest_file(tmp_path):
    ...
    assert src["web_sweep"]["latest"]["date"] == "2026-07-18"
    assert src["web_sweep"]["latest"]["age_days"] == 0
...
def test_fully_populated_fixture_has_all_sixteen_sources(tmp_path):
    """PRD §3.3: the collector knows sixteen sources (fifteen + `customer_moves`). A
    fully-populated profile should show every one present; missing ones must report
    present:false rather than error."""
    content, profiles = _build_fixture(tmp_path)
    manifest = voc.collect(content, profiles, "acme", today=TODAY)
    assert len(manifest["sources"]) == 16
    present = {s["id"] for s in manifest["sources"] if s["present"]}
    expected = {
...
def test_only_customer_voice_counts_toward_breadth(tmp_path):
    ...
    assert voc.counts_toward_breadth(voc.OWN_VOICE) is False
    assert voc.counts_toward_breadth(voc.REGULATOR_VOICE) is False
...
def test_harvest_artifacts_do_not_pollute_the_syften_source(tmp_path):
    """Harvest files share `market-signals/` with Syften digests. The Syften allow-list must
    ignore them, and unregistered artifact types must not default into Syften either."""
    content, profiles = _build_fixture(tmp_path)
    ...
    _write(signals / "some-new-artifact-2026-07-29.md")

    src = _by_id(voc.collect(content, profiles, "acme", today=TODAY))
    # Syften sees only its own digest files, and its latest is unchanged.
    ...
    # Web sweep keeps the market-scan weekly snapshots and ignores unregistered types.
    assert src["web_sweep"]["corpus_count"] == 2
    assert "some-new-artifact-2026-07-29.md" not in {
        Path(s["latest"]["path"]).name for s in src.values() if s.get("latest")
    }
...
def test_staleness_is_reported_separately_from_absence(tmp_path):
    ...
    content = tmp_path / "content"
    profiles = tmp_path / "profiles"
    # Only Syften is present; standards_watch has no harvest-raw directory.
    _write(content / "acme" / "market-signals" / "syften-digest-2026-07-18.md")
    # An unregistered old file must not pollute Syften.
