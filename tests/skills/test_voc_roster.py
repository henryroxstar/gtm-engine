...
def test_legal_suffixes_do_not_split_one_company_into_two():
    assert roster._norm_entity("Lumina Cloud, Inc.") == roster._norm_entity("Lumina Cloud")
    assert roster._norm_entity("Anvil.ai, Inc.") == roster._norm_entity("Anvil.ai")
    # ...but genuinely different filers stay different.
    assert roster._norm_entity("Ethon Technologies Inc.") != roster._norm_entity("Ethon Corp")


def test_ticker_parenthetical_does_not_split_one_company_into_two():
    """Confirmed live 2026-09-04: EDGAR's display_names carries a trailing "(TICKER)" that the
    short-name paragraph omits, so the un-fixed normaliser gave the same filer two keys."""
    assert roster._norm_entity("Atlassian Corp (TEAM)") == roster._norm_entity("Atlassian Corp")
    assert roster._norm_entity("Cimpress plc (CMPR)") == roster._norm_entity("Cimpress plc")
    assert roster._norm_entity("Intapp, Inc. (INTA)") == roster._norm_entity("Intapp, Inc.")
    # A bare ticker-less name is untouched.
    assert roster._norm_entity("Western Digital (WDC)") == roster._norm_entity(
        "Western Digital Corp"
    )


def test_filer_header_variant_is_not_imported_as_a_company():
    """The 2026-08-18 roster format headers its buyer-side table `| Filer | Filed | ... |`
    instead of `| Company | ... |`. Undetected, this imported the header row itself as a company
    literally named "Filer" — confirmed live in evidence.jsonl before this fix."""
    doc = """# Filer-header variant

## Roster — enterprise / buyer-side (unverified context)

### Financial services & payments — 1
| Filer | Filed | Inferred side | SIC |
|---|---|---|---|
| NORTHBRIDGE BANK CORP | 2026-02-20 | buyer | 6022 |
"""
    entities = {r.entity for r in roster.parse(doc)["records"]}
    assert "Filer" not in entities
    assert "NORTHBRIDGE BANK CORP" in entities
