...
def test_a_false_fact_subcheck_without_an_account_answer_does_not_disqualify():
    """The regression. A weak-but-honest signal on a good account must survive.

    The shape that motivated the whole change (2026-08-21 eval, predating the 2026-09-02
    merge into `fact_earns_its_place`): a hospital system whose opening fact said it was
    already centralising AI oversight and governance. That fact creates no problem we
    solve, does not support the pitch, and the operator would not send that email — a
    fact-scoped False, and the account is a perfectly good prospect. The operator's own
    note on the row was a copy complaint, not a fit complaint.

    The retired predicate read exactly this pattern as 'the account is bad'. It could never
    have done otherwise: the fact-scoped sub-check is scoped to the row's fact, so it can
    never express a judgment about the company. With no answer to the one account-scoped
    question, the correct output is NO durable action.
    """
    labels = [
        _label(
            "r1",
    ...
    assert plan.disqualifications == [], (
        "a good account was disqualified on a fact-scoped False — this is the exact "
...
def test_an_injected_row_never_disqualifies_a_real_account():
    ...
    The truth lives in the INTERNAL record, not the label. A blind sheet's own exporter
    (`build_from_sheet.py`) never puts `injected`/`injected_rule` in the labeler payload —
    the labeler must not see the answer key — so a real label produced by this program's own
    tooling always carries `injected: false`, regardless of the row's true status. Marking
    the label itself `injected=True`, as this test used to, describes a label no blind sheet
    can ever produce and would pass even with the pre-fix code that read `lab.injected`.
    """
    internal = {
        "r1": {**INTERNAL["r1"], "injected": True, "injected_rule": "company-allcaps"},
    }
    labels = [_bad_fit("r1")]  # a real blind label: injected defaults to False
    plan = build_plan(labels, internal, list_size=10)
    assert plan.actions == [], (
...
def test_a_blind_label_never_carries_the_injected_flag_and_the_guard_must_not_need_it():
    """Regression for the 2026-09-01 defect: the guard read `lab.injected`, which every
    real blind-sheet label reports False (the labeler cannot see the answer key), so a
    planted `signal-agent-homonym` row reached `suppress-person` in production. The guard
    must consult the INTERNAL record's `injected` flag, not the label's.
    """
    internal = {
        "r1": {**INTERNAL["r1"], "injected": True, "injected_rule": "signal-agent-homonym"},
    }
    # Exactly what build_from_sheet.py exports: no `injected` key at all on the label.
    label = Label(
        row_id="r1",
        spec_sha256=SPEC_SHA,
        csv_sha256=CSV_SHA,
        send_it=False,
        right_person=False,
        note="looks like an agent impersonating a person",
    )
    assert label.injected is False, "the label fixture no longer represents a blind export"
    plan = build_plan([label], internal, list_size=10)
    assert plan.suppressions == [], (
        "a planted signal-agent-homonym row reached suppress-person because the guard "
        "trusted the label's (always-false) injected flag instead of the internal record's"
    )
