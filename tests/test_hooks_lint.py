...
# --- video beats carry both streams ---------------------------------------------------------
#
# Added 2026-09-04. Every video opening beat in both live banks carried `video` (the spoken
# cold-open line) and no `text`, so the burned-in caption had no declared source on the hook and
# each script re-invented one downstream, unattributed. On a muted autoplay feed the caption is
# the only stream that lands, which makes it the half that was missing.


def test_lint_rejects_a_video_beat_with_no_caption():
    bank = _bank_with_beat(hk.OpeningBeat(format="short", video="The spoken cold open."))
    errors = hl.lint_bank(bank)
    assert len(errors) == 1
    assert "has no `text`" in errors[0]


def test_lint_rejects_a_video_beat_with_no_spoken_line():
    bank = _bank_with_beat(hk.OpeningBeat(format="reel", text="The burned-in caption."))
    errors = hl.lint_bank(bank)
    assert len(errors) == 1
    assert "has no `video`" in errors[0]


def test_lint_accepts_a_video_beat_carrying_both_streams():
    bank = _bank_with_beat(
        hk.OpeningBeat(format="reel", video="The spoken cold open.", text="The caption.")
    )
    assert hl.lint_bank(bank) == []


def test_lint_leaves_non_video_beats_alone():
    """A linkedin-text beat has no spoken stream, and must not be asked for one."""
    bank = _bank_with_beat(hk.OpeningBeat(format="linkedin-text", text="Just the text."))
    assert hl.lint_bank(bank) == []


def test_lint_treats_whitespace_only_caption_as_missing():
    bank = _bank_with_beat(hk.OpeningBeat(format="clip", video="Spoken.", text="   "))
    assert any("has no `text`" in e for e in hl.lint_bank(bank))
