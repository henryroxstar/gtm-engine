...
— one test per named rejection rule — plus a golden-equivalence test proving the first
real pack graph
...
@pytest.mark.parametrize(
    "field_toml",
    [
        'revisable_from = ["a"]\nmax_visits = 3\n',  # bounded — still refused
        'revisable_from = ["a"]\n',  # unbounded
        "max_visits = 3\n",  # the bound alone, with nothing to bound
    ],
    ids=["bounded", "unbounded", "max_visits_alone"],
)
def test_rejects_any_revision_declaration(tmp_path, field_toml):
    """The engine has no revision-loop semantics, so NO declaration is accepted.

    Previously the loader accepted a *bounded* back-edge and rejected only an unbounded one
    (rule ``unbounded_revision``). But ``agent/packs.py:pack_graph_to_engine_graph`` drops
    both fields and ``agent/graph.py:Node`` has nowhere to put them, so an accepted
    declaration validated and then silently did nothing — the inert-component failure mode.
    Bounded is refused alongside unbounded precisely because bounded was the shape that
    used to pass.
    """
    path = _write(tmp_path, "g.toml", _GRAPH_HEADER + '[[nodes]]\nid = "a"\n' + field_toml)
    with pytest.raises(PackValidationError) as exc:
        load_pack_graph(path)
    assert exc.value.rule == "unsupported_revision"


def test_node_without_revision_fields_is_allowed(tmp_path):
    """Positive control for the rule above — a plain node still loads."""
    path = _write(tmp_path, "g.toml", _GRAPH_HEADER + '[[nodes]]\nid = "a"\n')
    assert load_pack_graph(path).ids == ("a",)
...
def test_rejects_external_effect_on_non_gated_node(tmp_path):
    """gate=false ⇒ side-effect-free must be enforced, not aspirational (A1 loader rule)."""
    path = _write(
        tmp_path,
        "g.toml",
        _GRAPH_HEADER + '[[nodes]]\nid = "a"\nexternal_effect = "publish"\n',
    ...
    with pytest.raises(PackValidationError) as exc:
        load_pack_graph(path)
    assert exc.value.rule == "unsafe_external_effect"


def test_effect_free_non_gated_node_is_allowed(tmp_path):
    """Positive control for unsafe_external_effect — the ordinary node still loads."""
    path = _write(tmp_path, "g.toml", _GRAPH_HEADER + '[[nodes]]\nid = "a"\n')
    graph = load_pack_graph(path)
    assert graph.node("a").external_effect is None


# ── API-layer header annotations (title / description / min_entitlement) ──────


def test_header_annotations_load_and_default_to_none(tmp_path):
    annotated = _write(
        tmp_path,
        "annotated.toml",
        'pack = "p"\nvariant = "v"\ntitle = "Display Title"\n'
    ...
    graph = load_pack_graph(annotated)
    assert (graph.title, graph.description, graph.min_entitlement) == (
        "Display Title",
        "One paragraph.",
        "pro",
    )
    bare = _write(tmp_path, "bare.toml", _GRAPH_HEADER + '[[nodes]]\nid = "a"\n')
    plain = load_pack_graph(bare)
    assert (plain.title, plain.description, plain.min_entitlement) == (None, None, None)


def test_rejects_unknown_min_entitlement(tmp_path):
    path = _write(
        tmp_path,
        "g.toml",
        _GRAPH_HEADER + 'min_entitlement = "platinum"\n[[nodes]]\nid = "a"\n',
    )
    with pytest.raises(PackValidationError) as exc:
        load_pack_graph(path)
...
def test_rejects_none_as_min_entitlement(tmp_path):
    """'none' is the resolver's un-synced sentinel, never an authorable tier."""
    path = _write(
        tmp_path,
        "g.toml",
        _GRAPH_HEADER + 'min_entitlement = "none"\n[[nodes]]\nid = "a"\n',
    )
    with pytest.raises(PackValidationError) as exc:
        load_pack_graph(path)
    assert exc.value.rule == "unknown_entitlement"
...
def test_inputs_missing_file_is_no_inputs(tmp_path):
    """No inputs.toml = a declared-no-inputs pack (prospecting, solution-architecture)."""
    inputs = load_pack_inputs(tmp_path / "does-not-exist" / "inputs.toml")
    assert inputs.settings == () and inputs.knowledge == ()
...
def test_make_executor_from_pack_builds_a_working_executor(cfg, monkeypatch):
    ...
    async def _fake_execute_stage(
        cfg_arg, profile, stage_name, manifest, prompts=None, stage_roles=None, **_kw
    ):
...
SHORT_FORM_VIDEO = REPO / "packs" / "creator" / "graphs" / "short-form-video.toml"


def test_short_form_video_graph_loads_with_storyboard_gate():
    pack, engine_graph = load_engine_graph(SHORT_FORM_VIDEO)
    assert pack.variant == "short-form-video"
    assert pack.node("storyboard").skill == "video-storyboard"
    assert pack.node("storyboard").gate is True
    assert engine_graph.node("render-vertical").depends_on == ("storyboard",)
    assert engine_graph.node("render-feed").depends_on == ("storyboard",)
    assert engine_graph.node("storyboard").depends_on == ("script",)
