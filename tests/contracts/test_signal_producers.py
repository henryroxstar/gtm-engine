...
#: Types `agent.optout_sweep` emits directly, without the classifier: an opt-out is not
#: classified, it is MATCHED (`gtm_core.optout_watch`), and SC9 gave both match outcomes a
#: route to the provider DNC mirror. Listed rather than derived because the sweep passes
#: them as literals on purpose — see `test_the_classifier_result_reaches_build_signal`.
SWEEP_DIRECT_TYPES = frozenset({"optout_detected", "optout_unreadable"})

#: Every declared producer's own type set. Add a module here when it starts emitting.
PRODUCED_TYPES = frozenset(CLASSIFIED_TYPES) | SWEEP_DIRECT_TYPES
...
def test_the_classifier_result_reaches_build_signal():
    ...
    Two claims, because the sweep now has more than one producer. At least one
    `build_signal` call must pass a COMPUTED type — that is the classifier's result
    reaching the ledger. And no call may hardcode a type the classifier can return, which
    is the actual defect shape (every reply recorded as the same type again).

    A literal is legitimate for an event the classifier has no say in: an opt-out is an
    `optout_detected`, full stop — nothing classified it and nothing should.
    """
    from gtm_core.reply_classify import CLASSIFIED_TYPES
    ...
    assert calls, "no build_signal() call found in agent/optout_sweep.py"

    computed = [c for c in calls if not isinstance(c.args[1], ast.Constant)]
    assert computed, (
        "every build_signal() call in optout_sweep passes a literal signal_type — the "
        "classifier's result reaches nothing, so every reply is recorded as the same type"
    )

    hardcoded_classified = [
        c.args[1].value
        for c in calls
        if isinstance(c.args[1], ast.Constant) and c.args[1].value in CLASSIFIED_TYPES
    ]
    assert not hardcoded_classified, (
        f"build_signal() hardcodes classifier type(s) {hardcoded_classified} — a type the "
        "classifier can return must come FROM the classifier, never from a literal"
    )
