...
def test_creator_pack_gates_pause_the_run_and_leave_downstream_pending(cfg):
    ...
    for downstream in ("script", "render-vertical", "render-feed", "score", "publish"):
...
def test_creator_pack_budget_guard_fires_before_the_render_batch(cfg, monkeypatch):
    ...
    def _budget_ok(cfg_arg, profile):
        calls["n"] += 1
        # radar, plan, script, storyboard each dispatch as their own single-node batch;
        # the render fan-out is the 5th batch — that's the one that must be refused.
        return calls["n"] <= 4
