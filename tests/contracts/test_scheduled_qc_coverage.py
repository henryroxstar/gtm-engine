...
def test_no_gate_is_listed_that_does_not_exist() -> None:
    ...
    A gate may be a module OR a package (several were split into packages in PRD Phase 3);
    both are "the module is still there", and neither present is still a stale entry."""
    for gate in DETERMINISTIC_GATES:
        base = REPO / "gtm_core" / gate
        assert base.with_suffix(".py").is_file() or (base / "__init__.py").is_file(), (
            f"gtm_core/{gate} is gone — neither {gate}.py nor {gate}/__init__.py exists"
        )
