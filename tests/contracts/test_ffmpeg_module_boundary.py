...
#: The complete, deliberate allowlist of OWNERS, keyed by path relative to ``gtm_core/``. An
#: entry is a module or a package directory; a file inside a listed package belongs to that
#: owner (``video_finish`` became a package in PRD Phase 2 without its ownership changing).
#: Adding a fourth entry is a boundary change: it needs the same "why here, and why not inside
#: video_finish's orchestration" reasoning the existing three carry in their own docstrings —
#: not just a green test. Removing the hand-rolled-chain prohibition from a skill body is a
#: separate regression, pinned by ``test_video_finish_prohibition_wired.py``.
_ALLOWED_MEDIA_MODULES = frozenset(
    {
        "video_finish",  # package: owns grade / stitch / mux / loudness — single-grade invariant
        "video_lint",  # package: owns probe-based verification (ffprobe + measurement)
...
def _owned_by(rel: str, owners: frozenset[str]) -> bool:
    """True when ``rel`` IS an owner or lives inside one (a package owner)."""
    parts = pathlib.PurePosixPath(rel).parts
    return rel in owners or any("/".join(parts[: n + 1]) in owners for n in range(len(parts) - 1))
...
def test_only_the_allowlisted_modules_execute_ffmpeg():
    offenders = {
        ...
        if not _owned_by(rel, _ALLOWED_MEDIA_MODULES) and (found := _resolved_binaries(path))
    }
    assert offenders == {}, (
...
def test_every_allowlisted_module_does_execute_ffmpeg():
    ...
    entry for an owner that stopped shelling out, or one that no longer exists)."""
    for rel in sorted(_ALLOWED_MEDIA_MODULES):
        path = GTM_CORE / rel
        assert path.exists(), f"{rel} is allowlisted but does not exist"
        files = sorted(path.rglob("*.py")) if path.is_dir() else [path]
        assert any(_resolved_binaries(f) for f in files), (
            f"{rel} is allowlisted but resolves no media binary"
        )
