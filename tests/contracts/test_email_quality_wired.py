...
@pytest.mark.parametrize(
    "verb",
    ["score", "rules", "reconcile", "self-agreement", "intra-rater"],
)
...
def test_the_judge_scores_every_row_not_a_sample():
    ...
    # The sdk path batches, and since 2026-09-08 it batches PER LANE so one prompt carries
    # one rubric. Checked by calling the splitter rather than by grepping for the loop it
    # used to be written as: the property is that every row is covered exactly once, which
    # a string match cannot see and which a lane-grouping bug would break silently.
    from agent.mcp.judge.scoring import SDK_BATCH_ROWS, lane_batches, lane_of

    rows = [{"lane": lane} for lane in ["generic", "signal", "generic", "", "repair"] * 7]
    batches = lane_batches(rows, SDK_BATCH_ROWS)

    covered = [i for _, idxs in batches for i in idxs]
    assert sorted(covered) == list(range(len(rows))), (
        "the sdk path no longer covers every row exactly once"
    )
    for lane, idxs in batches:
        assert idxs, "an empty batch would spend a subprocess on nothing"
        assert len(idxs) <= SDK_BATCH_ROWS, "a batch exceeded the configured width"
        assert {lane_of(rows[i]) for i in idxs} == {lane}, (
            "a batch mixed lanes — one prompt carries one rubric, so its rows would be "
...
# ── calibrated means a recorded PASS, never "a holdout file exists" ──────────────


def _score_verdict(passed: bool):
    from types import SimpleNamespace

    if passed:
        return SimpleNamespace(passed=True, tpr=1.0, tnr=1.0, kappa=1.0, flip=0.0, reasons=[])
    return SimpleNamespace(passed=False, tpr=0.88, tnr=0.25, kappa=0.12, flip=0.28, reasons=["x"])


def test_judge_calibrated_is_false_when_a_holdout_exists_but_the_judge_failed_it(
    tmp_path, monkeypatch
):
    """2026-09-01: a holdout was sealed, the judge FAILED all four bars, and under the old
    file-exists rule the next `write-verdicts` would have stamped `judge_calibrated=true` —
    letting `account_integrity` obey ~49 drops from a judge that had just failed."""
    from gtm_core import judge_calibration as jc
    from gtm_core.adjudication import Adjudication, main, write_records
    from gtm_core.eval_calibration import evals_dir

    monkeypatch.setenv("GTM_CONTENT_ROOT", str(tmp_path))
    d = evals_dir("acme")
    d.mkdir(parents=True)
    holdout = d / "eval-2026-01-01-holdout.json"
    holdout.write_text("[]", encoding="utf-8")
    jc.write_score_record(holdout, _score_verdict(False), n=16, provisional=False, predictions="p")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        w.writerow({"email": "a@acme.example"})
    records = tmp_path / "rec.jsonl"
    write_records([Adjudication(email="a@acme.example", verdict="drop", score=1)], records)
    out = tmp_path / "out.csv"
    argv = ["write-verdicts", "--csv", str(csv_path), "--records", str(records), "--out", str(out)]
    main([*argv, "--profile", "acme"])
    assert next(iter(csv.DictReader(out.open(encoding="utf-8"))))["judge_calibrated"] == "false"

    # Positive control: only a recorded PASS flips it.
    jc.write_score_record(holdout, _score_verdict(True), n=16, provisional=False, predictions="p")
    main([*argv, "--profile", "acme"])
    assert next(iter(csv.DictReader(out.open(encoding="utf-8"))))["judge_calibrated"] == "true"


def test_judge_columns_are_consumed_by_name_not_position():
    """`judge_defect_class` made JUDGE_COLUMNS a 4-tuple; a positional unpack would have
    silently shifted, so no production code may index or unpack it."""
    import re

    offenders = []
    for base in (REPO / "gtm_core", REPO / "agent"):
        for path in base.rglob("*.py"):
            src = path.read_text(encoding="utf-8")
            if re.search(r"JUDGE_COLUMNS\s*\[", src) or re.search(
                r"^\s*[\w, ]+=\s*JUDGE_COLUMNS\s*$", src, re.M
            ):
                offenders.append(str(path.relative_to(REPO)))
    assert not offenders, f"JUDGE_COLUMNS is indexed/unpacked positionally in: {offenders}"


def test_filter_by_verdict_has_one_implementation():
    """The status dashboard once audited a different population than the gate because it
    re-implemented the filter and omitted the calibrated-judge drop. The comparison that
    obeys a judge `drop` may exist in exactly one production module."""
    needle = 'judge_verdict") or "").strip().lower() == "drop"'
    owners = sorted(
        str(p.relative_to(REPO))
        for base in (REPO / "gtm_core", REPO / "agent", REPO / "backend")
        for p in base.rglob("*.py")
        if needle in p.read_text(encoding="utf-8")
    )
    assert owners == ["gtm_core/account_integrity.py"], owners


# ── the outcomes loop has a denominator and the skill points at the real producers ─────
...
# ── feedback reaches the next spec ──────────────────────────────────────────────


def test_sequence_skill_reads_the_defect_report():
    body = (REPO / "plugin" / "skills" / "email-sequence" / "body_template.md").read_text(
        encoding="utf-8"
    )
    flat = " ".join(body.split())
    assert "defect-report-*.md" in flat and "never paraphrase a failing sentence" in flat
    assert "Operator guidance" in body


def test_email_quality_regenerate_cites_fresh_session_and_never_worker_draft():
    body = (REPO / "plugin" / "skills" / "email-quality" / "body_template.md").read_text(
        encoding="utf-8"
    )
    section = body.split("### Regenerating a spec from its defect report", 1)[1].split(
        "### The repair pass", 1
    )[0]
    assert "python -m agent --profile" in section and "fresh one-shot session" in section
    assert (
        "never the DeepSeek worker" in section
        and "regen-count" in section
