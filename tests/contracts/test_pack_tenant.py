...
def test_override_rejects_added_node_with_unknown_skill(base_graph):
    """The merge re-runs the per-node loader rules — an override cannot introduce a node
    the base loader would have rejected (this was a documented-but-unenforced promise)."""
    override = PackOverride(
        pack="marketing",
        variant="linkedin-post",
        add_nodes=(PackNode(id="rogue", depends_on=("studio",), skill="not-a-skill"),),
    )
    with pytest.raises(PackValidationError) as exc:
        merge_pack_override(base_graph, override)
...
def test_override_rejects_added_node_with_effect_but_no_gate(base_graph):
    override = PackOverride(
        pack="marketing",
        variant="linkedin-post",
        add_nodes=(PackNode(id="rogue", depends_on=("studio",), external_effect="publish"),),
    )
    with pytest.raises(PackValidationError) as exc:
        merge_pack_override(base_graph, override)
...
def test_override_rejects_added_node_declaring_a_revision_back_edge(base_graph):
    """A tenant override cannot smuggle in a revision back-edge the engine cannot run.

    This path matters specifically because ``merge_pack_override`` constructs ``PackNode``
    directly and never calls ``_build_node`` — so a refusal placed in the raw-dict builder
    would have missed it entirely. The rule lives in ``validate_node_semantics``, which the
    merge re-runs over the whole merged graph, which is why this is covered.
    """
    override = PackOverride(
        pack="marketing",
        variant="linkedin-post",
...
def test_merge_preserves_api_annotations(base_graph):
    """title/description/min_entitlement survive the merge unchanged."""
    import dataclasses

    annotated = dataclasses.replace(base_graph, title="T", min_entitlement="pro")
    override = PackOverride(
        pack="marketing",
        variant="linkedin-post",
        strengthen=(StrengthenEntry(id="research", add_gate=True),),
    )
    merged = merge_pack_override(annotated, override)
    assert (merged.title, merged.min_entitlement) == ("T", "pro")
...
# ── F6: overrides are additions-only — an unrecognised key must be REJECTED, never
# silently ignored (Phase E) ─────────────────────────────────────────────────────────────


def test_load_pack_activation_rejects_an_unknown_override_key(tmp_path):
    """Reproduces the exact live footgun: before this check, remove_nodes parsed with no
    error and did nothing — safe only by accident."""
    path = tmp_path / "packs.toml"
    path.write_text(
        'active = ["marketing"]\n\n[overrides.marketing]\nvariant = "linkedin-post"\n'
        'remove_nodes = ["finish"]\n'
    ...
    with pytest.raises(PackValidationError) as exc:
        load_pack_activation(path)
    assert exc.value.rule == "unknown_override_key"
    assert "remove_nodes" in str(exc.value)


def test_load_pack_activation_rejects_an_unknown_strengthen_key(tmp_path):
    path = tmp_path / "packs.toml"
    path.write_text(
        'active = ["marketing"]\n\n[overrides.marketing]\nvariant = "linkedin-post"\n'
        '[[overrides.marketing.strengthen]]\nid = "publish"\nremove_gate = true\n'
    ...
    with pytest.raises(PackValidationError) as exc:
        load_pack_activation(path)
    assert exc.value.rule == "unknown_strengthen_key"


def test_load_pack_activation_rejects_a_strengthen_entry_with_no_id(tmp_path):
    path = tmp_path / "packs.toml"
    path.write_text(
        'active = ["marketing"]\n\n[overrides.marketing]\nvariant = "linkedin-post"\n'
        "[[overrides.marketing.strengthen]]\nadd_gate = true\n"
    ...
    with pytest.raises(PackValidationError) as exc:
        load_pack_activation(path)
    assert exc.value.rule == "missing_strengthen_id"
...
def test_reachability_returns_active_packs_skills_only(tmp_path):
    ...
    result = active_skills_for_profile(tmp_path, "acme", REPO / "packs")
    # Union across EVERY variant in the pack, not just one graph: linkedin-post +
    # long-form-blog contribute the content-* chain, case-study contributes itself.
    assert result == {
        "content-radar",
        "content-plan",
        "content-research",
