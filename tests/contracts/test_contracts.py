...
def test_eight_schemas_present():
    # Count guard against accidental schema additions. 18 = 7 core + journey-state +
    ...
    # partition) + pack-descriptor + run-event + agent (generic client API contract)
    # + run-artifact (file deliverable listing item) + shots (long-form shot list, Phase E)
    # + email-eval-label (golden-set operator labels)
    # + creator-brief (the nine pre-spend video decisions, 2026-09-06). Bump when adding one.
    assert len(ALL_SCHEMAS) == 19, [p.name for p in ALL_SCHEMAS]
