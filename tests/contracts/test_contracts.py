...
def test_eight_schemas_present():
    # Count guard against accidental schema additions. 11 = 7 core + journey-state +
    # story-cluster (journey engine) + profile-draft (profile onboarding) + community-signal-model
    # (community-signal-analysis dashboard). Bump when adding one.
    assert len(ALL_SCHEMAS) == 11, [p.name for p in ALL_SCHEMAS]
