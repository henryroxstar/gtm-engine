...
from gtm_core.slugify import slug as canonical_slug

FIXTURE = Path(__file__).parent / "fixtures" / "vibe-export-sample.csv"


def test_slug_delegates_to_the_canonical_slugify_module():
    # prospects_import._slug is now a direct import-alias of gtm_core.slugify.slug —
    # this pins that delegation so a future refactor can't silently reintroduce a
    # second, divergent slug implementation.
    assert pi._slug is canonical_slug
    for name in ("A Home for Nana", "Quorum.AI", "", "!!!"):
        assert pi._slug(name) == canonical_slug(name)
...
def test_domain_disambiguates_similar_names(tmp_path):
    # BUG 2: two DISTINCT companies with names that normalize to the same
    # suffix-stripped key must NOT collapse — different domains keep them apart.
    csv_path = tmp_path / "sim.csv"
    csv_path.write_text(
        "business_name,business_domain,business_country_name\n"
        "Acme Inc,acme.example,united states\n"
        "Acme Holdings,acmeholdings.example,united states\n",
        encoding="utf-8",
    )
    cands = pi.parse_vibe_export(csv_path)
    companies = {c["company"] for c in cands}
    assert companies == {"Acme Inc", "Acme Holdings"}  # both kept, not merged


def test_non_ascii_company_kept_with_nonempty_id(tmp_path):
    # BUG 3: a pure non-ASCII name (empty legacy _norm/_slug) must be kept, and
    # the fallback id must be non-empty so it doesn't collide downstream.
    csv_path = tmp_path / "intl.csv"
    csv_path.write_text(
        "business_name,business_domain,business_country_name\n山田商事株式会社,yamada-shoji.example,japan\n",
        encoding="utf-8",
    )
    cands = pi.parse_vibe_export(csv_path)
    assert len(cands) == 1
    assert cands[0]["company"] == "山田商事株式会社"
    assert cands[0]["id"]  # non-empty stable id (hashlib fallback)


def test_count_export_rows_counts_raw_rows():
    # Billing basis: every exported data row, including the in-file duplicate.
    assert pi.count_export_rows(FIXTURE) == 8
...
def test_ingest_writes_candidates_and_logs_cost(tmp_path):
    ...
    # cost logged to costs.jsonl against RAW billed rows (Vibe bills per exported
    # row, before in-file dedup): 8 rows × 2 = 16 credits (not the 7 deduped)
    assert summary["billed_rows"] == 8
    costs = (tmp_path / "acme" / "costs.jsonl").read_text().strip().splitlines()
    rec = json.loads(costs[-1])
    assert rec["tool"] == "vibe-prospecting"
    assert rec["units"]["credits"] == 16
...
def test_finalize_merges_into_latest_and_writes_hubspot(tmp_path):
    ...
    assert row["Email Status"] == "RocketReach A"


def test_finalize_email_status_reaches_ready_to_load_via_consolidate(tmp_path):
    """Regression: a bulk-mode item with a graded RocketReach email must land in
    sequences/ready-to-load.csv after consolidate, not the hidden
    sequences/.pool/needs-verification.csv hold queue. Before `email_status` was added
    to `_HUBSPOT_COLUMNS`, `finalize` silently dropped the grade on the floor, so
    `prospects_consolidate` had nothing to classify against and every bulk-mode contact
    fell to `conf_tier == "unknown"` regardless of how well-verified its email was.
    """
    ...
    pi.finalize("acme", scored, "run-z", content_root=tmp_path)
    ...
    assert "dana.cho@verified.example" in ready
    assert "dana.cho@verified.example" not in needs_verification


def test_finalize_research_record_reaches_ready_to_load_via_consolidate(tmp_path):
    """Regression: a bulk-mode item carrying the full research record
    (`gtm_core.signal_record.RECORD_COLUMNS`, plus `conf` and `signal_column`) must
    have every one of those values survive `finalize` -> `consolidate` into
    sequences/ready-to-load.csv, and the resulting row must clear
    `signal_record.check_record` with zero findings — the same per-row check the
    enrollment gate (`account_integrity.audit_rows`) runs before a list is loaded
    into the sequencer.

    Before these columns were added to `_HUBSPOT_COLUMNS`/`_write_hubspot_csv`, a
    bulk-mode item's verdict/category_relation/signal_* fields were silently dropped
    on the floor, so every bulk-mode row reached the gate with `verdict` and
    `category_relation` blank — both are block-level findings
    (`verdict-missing`, `relation-unresolved`) that fail the gate outright, exactly
    the class of bug the sibling `email_status` fix addressed for deliverability.
    """
    import datetime
    ...
    evidence = (
        "Recorded Co raised a $40M Series B round, the company confirmed in a blog post today."
    )
    scored = [
        {
            "id": "recorded-co",
            ...
            "signal_source_url": "https://news.example/2026/08/20/recorded-co-series-b/",
            "signal_observed": "2026-08-20",
            "signal_evidence": evidence,
            "signal_subject": "Recorded Co",
            "signal_agent_kind": "none",
            "signal_column": "Funding event",
        },
    ]
    pi.finalize("acme", scored, "run-v", content_root=tmp_path)

    pc.consolidate("acme", content_root=tmp_path)
    pdir = tmp_path / "acme" / "prospects"
    ready = {r["email"]: r for r in pc._load_master(pdir / "sequences" / "ready-to-load.csv")}
    row = ready["sam.lee@recorded.example"]

    assert missing_record_columns(list(row)) == []
    assert row["conf"] == "0.9"
    assert row["verdict"] == "send"
    assert row["category_relation"] == "prospect"
    assert row["signal_source_url"] == "https://news.example/2026/08/20/recorded-co-series-b/"
    assert row["signal_observed"] == "2026-08-20"
    assert row["signal_evidence"] == evidence
    assert row["signal_subject"] == "Recorded Co"
