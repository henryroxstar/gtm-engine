...
def _make_content(tmp_path: Path) -> Path:
    ...
    _pack(
        root / "acme" / "accounts" / "wavelet",
        "prospects-20260702-outreach-wavelet.md",
        (
            "# Wavelet — outreach pack (give-first)\n\n"
...
def test_falls_back_for_nonstandard_title_and_missing_tier(tmp_path):
    root = _make_content(tmp_path)
    rows = collect_rows(root, "acme")
    wavelet = next(r for r in rows if r.account == "Wavelet")
    assert wavelet.draft_date == "2026-07-02"  # recovered from the filename
    assert wavelet.tier == ""
    assert wavelet.channels == "LinkedIn"  # no "## Email" section in this pack


def test_only_reads_the_given_profiles_accounts(tmp_path):
    root = _make_content(tmp_path)
    rows = collect_rows(root, "acme")
    assert {r.account for r in rows} == {"Nimbus", "Wavelet"}
