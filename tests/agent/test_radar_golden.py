...
def test_content_signal_clusters_are_high_priority_and_schema_valid(tmp_path):
    path = tmp_path / "content-signals-2026-07-29.json"
    path.write_text(
        json.dumps(
            [
                {
                    "signal_id": "sig-1",
                    "pillar": "AI Policy",
                    "angle": "What regulators are signalling now",
                    "title": "EU AI Act enforcement window opens",
    ...
    clusters = radar.content_signal_clusters(path, PILLARS, set())
    assert len(clusters) == 1
    assert clusters[0]["score"] == 100
    assert clusters[0]["source_items"] == ["sig-1"]
    schema = _story_schema()
    errs = validate(clusters[0], schema)
    assert errs == [], f"content-signal cluster invalid: {errs}"


def test_content_signal_clusters_skip_unverified_and_seen_ids(tmp_path):
    path = tmp_path / "content-signals-2026-07-29.json"
    path.write_text(
        json.dumps(
            [
    ...
    clusters = radar.content_signal_clusters(path, PILLARS, {"sig-new"})
    assert clusters == []


def test_render_digest_renders_content_signal_source_with_url():
    clusters = [
        {
            "id": "cs-001",
            "pillar": "AI Policy",
            "score": 100,
            "source_items": ["sig-1"],
            "why_it_matters": "Market-intelligence signal",
            "angle_seeds": [],
            "platform_fit": [],
        }
    ]
    ...
    digest = radar.render_digest(clusters, [], "2026-07-29", "example", content_signals)
    assert "EU AI Act enforcement window opens" in digest
    assert "https://example.com/eu-ai-act" in digest
