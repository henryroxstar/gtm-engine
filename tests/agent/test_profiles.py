...
@pytest.mark.parametrize(
    "name",
    ["../../etc", "..", ".", "a/b", "a\\b", "evil\x00", ""],
)
def test_profile_dir_rejects_traversal_segments(name):
    """A traversal segment is rejected as unsafe, before any path is built from it.

    The PROFILE.md existence check alone already made these unreachable in practice;
    this pins the explicit guard so a future caller that drops the existence check
    does not silently reopen the tenant boundary.
    """
    with pytest.raises(ValueError, match="unsafe profile"):
        profiles.profile_dir(PROFILES_ROOT, name)
