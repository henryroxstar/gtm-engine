...
but via the env var since the CLI resolves its own Config). `GTM_CONTENT_ROOT` is
pinned alongside it for the same reason: promote writes a ledger event, and without
the override that write lands in the real `<repo>/content` tree.
"""
