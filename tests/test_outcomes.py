...
# ── story_format on the attribution path (S8, 2026-09-07) ─────────────────────────────


def _score(tmp_path, payload: dict):
    path = tmp_path / "score.json"
    path.write_text(json.dumps(payload))
    return path


def test_attribute_carries_a_top_level_story_format_as_a_tag(tmp_path):
    """S8-T1. A tag, not a meta key: only tags are queryable by the correlator."""
    repo_root = tmp_path
    content_root = tmp_path / "content"
    finish_path = _write_finish(repo_root, content_root)
    score_path = _score(tmp_path, {"recommended": {"band": "high"}, "story_format": True})

    record = oc.attribute(
        content_root, "acme", finish_path=finish_path, repo_root=repo_root, score_path=score_path
    )
    assert "story_format:true" in record["tags"], record["tags"]


def test_attribute_carries_a_measured_false_distinctly_from_silence(tmp_path):
    """S8-T2. `false` on the score path is a CHECKED negative and is allowed to say so.

    The rule that absent stays absent is about rows nobody checked, not about a piece that was
    scored and genuinely is not a story.
    """
    repo_root = tmp_path
    content_root = tmp_path / "content"
    finish_path = _write_finish(repo_root, content_root)
    score_path = _score(tmp_path, {"recommended": {"band": "high"}, "story_format": False})

    record = oc.attribute(
        content_root, "acme", finish_path=finish_path, repo_root=repo_root, score_path=score_path
    )
    assert "story_format:false" in record["tags"], record["tags"]


def test_a_score_json_that_says_nothing_produces_no_story_tag(tmp_path):
    """S8-T2. The three-state field: every score.json written before S8 is unknown, not false."""
    repo_root = tmp_path
    content_root = tmp_path / "content"
    finish_path = _write_finish(repo_root, content_root)
    ...
    assert not any(t.startswith("story_format:") for t in record["tags"]), record["tags"]


def test_a_story_format_inside_recommended_is_not_promoted_to_a_tag(tmp_path):
    """S8-T2. The placement rule, pinned.

    `_predictor_band` sweeps every scalar in `recommended` into `meta["predictor"]`. A
    `story_format` written there would land in meta — which the correlator cannot query — and only
    when a band happened to be set. The field must be read from the top level or not at all.
    """
    repo_root = tmp_path
    content_root = tmp_path / "content"
    finish_path = _write_finish(repo_root, content_root)
    ...
    assert not any(t.startswith("story_format:") for t in record["tags"]), (
