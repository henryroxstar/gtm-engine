...
from agent.publish import PublishResult, approval_hash, content_hash  # noqa: E402
...
def _stage_scheduled(cockpit, monkeypatch, post, when):
    raw = f"Drafted it.\n⟦GATE:publish⟧\n⟦POST⟧\n{post}\n⟦/POST⟧\n⟦SCHEDULE⟧{when}⟦/SCHEDULE⟧"
    monkeypatch.setattr(cockpit.store, "run", stream_of(raw))
    msg = FakeMsg(CHAT_ID, text="draft and schedule the post")
    update, context = text_update(CHAT_ID, msg)
    asyncio.run(cockpit.on_text(update, context))
    return msg
...
def test_scheduled_gate_shows_the_time_and_binds_the_approval_to_it(monkeypatch, tmp_path):
    """The operator must SEE the send time, and the staged hash must cover it —
    otherwise they are approving bytes while the time rides along unreviewed."""
    cockpit = _make_cockpit(tmp_path, publisher=FakePublisher())
    post, when = "Scheduled copy.", _future()
    msg = _stage_scheduled(cockpit, monkeypatch, post, when)

    token = approval_hash(post, (), when)[:16]
    pending = cockpit._pending_publish[token]
    assert pending["scheduled_at"] == when
    assert pending["hash"] == approval_hash(post, (), when) != content_hash(post, ())
    preview = "\n".join(msg.replies)
    assert when in preview and "schedule" in preview.lower()
    assert "no further review" in preview.lower(), "the operator must be told it fires unattended"


def test_an_unschedulable_time_is_refused_at_staging_not_at_dispatch(monkeypatch, tmp_path):
    """Never let someone approve a slot that would bounce seconds later."""
    cockpit = _make_cockpit(tmp_path, publisher=FakePublisher())
    msg = _stage_scheduled(cockpit, monkeypatch, "Copy.", "2020-01-01T09:00:00Z")
    assert cockpit._pending_publish == {}
    assert any("Not staged for publish" in r for r in msg.replies)


def test_approving_a_scheduled_post_passes_the_time_through_to_the_publisher(monkeypatch, tmp_path):
    publisher = FakePublisher(PublishResult(ok=True, status="scheduled", post_id="p9"))
    cockpit = _make_cockpit(tmp_path, publisher=publisher)
    post, when = "Scheduled copy.", _future()
    _stage_scheduled(cockpit, monkeypatch, post, when)
    _press(cockpit, f"pub:ok:{approval_hash(post, (), when)[:16]}")

    assert publisher.scheduled_at_calls == [when]
    rec = _history_records(tmp_path)[-1]
    assert rec["event"] == "scheduled"
    assert rec["content_sha256"] == content_hash(post, ())


def test_a_scheduled_draft_is_refused_at_staging_when_the_kill_switch_is_off(monkeypatch, tmp_path):
    """An operator must never approve a slot that dispatch will refuse seconds
    later with `schedule_disabled` — the kill switch is checked at staging, same
    as the send-time validity check right below it."""
    publisher = FakePublisher(schedule_enabled=False)
    cockpit = _make_cockpit(tmp_path, publisher=publisher)
    post, when = "Scheduled copy.", _future()
    msg = _stage_scheduled(cockpit, monkeypatch, post, when)
    ...
    assert "HERMES_SCHEDULE_ENABLED" in reply
    assert publisher.calls == []


def test_an_unscheduled_draft_still_stages_when_the_schedule_kill_switch_is_off(
    monkeypatch, tmp_path
):
    """The schedule kill switch must gate ONLY scheduled drafts — an ordinary
    immediate publish is unaffected by HERMES_SCHEDULE_ENABLED."""
    publisher = FakePublisher(schedule_enabled=False)
    cockpit = _make_cockpit(tmp_path, publisher=publisher)
    post = "Ordinary immediate post."
    _stage(cockpit, monkeypatch, post)
