...
class FakePublisher:
    ...
    def __init__(self, result=None, max_chars=3000, schedule_enabled=True):
        # Defaults True so every EXISTING scheduled-staging test keeps exercising the
        # time/content checks it was written for, undisturbed by the kill-switch
        # check added at staging. Tests of the kill switch itself pass False.
        self.settings = PublishSettings(max_chars=max_chars, schedule_enabled=schedule_enabled)
...
def publish_block(post, media=(), prose="Drafted the post below.", identity=()):
    """The exact ⟦GATE:publish⟧ block shape the content-publish skill emits.

    ``identity`` (Phase 12) is a tuple of render-identity handle names
    (``"soul"``/``"element"``/``"voice"``) to declare via ⟦IDENTITY⟧ — defaults to
    ``()`` so every pre-Phase-12 call site is unaffected.
    """
    block = f"{prose}\n⟦GATE:publish⟧\n⟦POST⟧\n{post}\n⟦/POST⟧"
    if media:
        block += "\n⟦MEDIA⟧\n" + "\n".join(media) + "\n⟦/MEDIA⟧"
...
def reply_block(body, thread_id="", to="", prose="Drafted the reply below."):
    """The exact ⟦GATE:reply⟧ block shape the inbound-triage skill emits."""
    block = f"{prose}\n⟦GATE:reply⟧\n⟦REPLY⟧\n{body}\n⟦/REPLY⟧"
    if thread_id:
