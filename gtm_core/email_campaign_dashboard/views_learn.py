...
def _learn_view(m: dict) -> str:
    ...
    lift_card = _scoped_out(
        ...
        "Minimum detectable effect is sized from the pool's per-group counts. At 4 enrolled "
        "and 6 hand-sent packs this campaign cannot power a comparison at all, which is the "
        "honest answer rather than a forecast.",
    )
    cm = m["cells"]
    cells = cm["cells"]
    sup = m["supply"]
