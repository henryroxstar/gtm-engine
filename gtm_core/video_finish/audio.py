...
def duck_music_bed(
    ...
    *,
    ...
    """DUCK DEPTH: DECIDED, NOT CHANGED (C6, 2026-09-07). The corpus argues for a deeper duck than
    this function applies by default, and C6 deliberately did not move it. Two reasons. The depth
    that is correct depends on the VO duty cycle — a bed under a near-continuous read wants a
    shallower, steadier duck than one under a sparse read with long instrumental gaps, and this
    function does not know which it is being handed. And the number is currently a fixed,
    measurable quantity precisely because `build_music_bed`'s `level_envelope` flattens the cue's
    own swell first; loosening it here would put the drift back. Owner: the audio lane, with the
    duty-cycle measurement in hand. Tracked in PENDING.
...
def build_music_bed(
    ...
    * ``loop_from_s`` picks where the repeat is taken from, so it can be LEVEL-MATCHED to the
    ...
    ``start_at_s`` says where in the TRACK the bed begins, and it is a different question from
    ``loop_from_s`` above — worth stating plainly, because the two look alike and are not.
    ``loop_from_s`` picks where a REPEAT is taken from, to level-match a splice in the middle.
    ``start_at_s`` picks where the bed OPENS, to skip a cue's slow build so the film starts on
    the music instead of thirty seconds ahead of it. A catalogue track is written to be listened
    to from the top; a film is not obliged to start there. Default 0.0 keeps every existing
    caller byte-identical.

    They are refused in combination when they contradict: a ``loop_from_s`` earlier than
    ``start_at_s`` would splice back into a passage the bed deliberately skipped, which is the
    intro arriving in the middle of the film.
    ...
    if not 0 <= start_at_s < body_end:
        raise ValueError(
            f"start_at_s={start_at_s} is outside the usable body (0..{body_end}) — there would "
            "be no bed left to lay under the film"
        )
    if not 0 <= loop_from_s < body_end:
        raise ValueError(f"loop_from_s={loop_from_s} is outside the usable body (0..{body_end})")
    # An unset `loop_from_s` (its 0.0 default) with a non-zero `start_at_s` snaps to `start_at_s`:
    # "repeat from where the bed opens" is the only coherent meaning, and an explicit 0.0 there
    # is the very contradiction the next check refuses. This used to be `if loop_from_s and
    # …` — a truthiness test that let the default sail through, so a bed opened at the drop got
    # the skipped intro spliced back in on its first repeat. Found in review; the tests had always
    # passed loop_from_s explicitly, which is why they never saw it.
    if start_at_s > 0 and loop_from_s == 0.0:
        loop_from_s = start_at_s
    if loop_from_s < start_at_s:
        raise ValueError(
            f"loop_from_s={loop_from_s} is earlier than start_at_s={start_at_s}: the repeat would "
            "splice back into the passage the bed deliberately skipped, so the intro would arrive "
            "in the middle of the film. Take the repeat from at or after where the bed opens."
        )
    ...
    head_len = body_end - start_at_s
    if duration_s > head_len and gain_per_repeat <= 0:
        raise ValueError(
    ...
    repeats = 0 if duration_s <= head_len else math.ceil((duration_s - head_len) / gain_per_repeat)
    ...
    cur = _materialize(start_at_s, body_end, workdir / "bed_seg0.wav")
    if repeats:
