...
``video-avatar`` Step 3 reversed to one continuous presenter take on measured per-render billing,
which made the presenter footage a single file while ``video-render``'s inserts stayed separate
files meant to be intercut with it. ``stitch`` only joins, so the creator pack's ``finish`` node
