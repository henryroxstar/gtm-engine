...
def split(
    ...
    *,
    ...
    `video-avatar` renders the presenter's whole script as a single take because HeyGen bills per
    render, per minute, rounded up: eight beats rendered separately cost 8x one take (184 credits
    versus 23, measured 2026-08-27). That makes the presenter footage ONE file while
    `video-render`'s insert shots are still separate files meant to be intercut with it — and
    :func:`stitch` only joins. Without this verb the creator pack's `finish` node ("stitch the
    presenter and insert clips into the shot order the script declares") had no sanctioned way to
    get the beats back out of the take, because ``Bash(ffmpeg:*)`` is denied (2026-08-18) and this
    module is the only door to ffmpeg. "Cut it locally" was an instruction with no reachable
    implementation.
