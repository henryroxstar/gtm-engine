...
# Element is a pure in-memory constructor (no parsing), so it carries no XXE surface;
# the actual untrusted-input parse entry points below go through defusedxml instead.
from xml.etree.ElementTree import Element  # nosemgrep: use-defused-xml  # nosec B405
...
def _num(geom: Element | None, key: str) -> float:
    if geom is None:
        return 0.0
    try:
...
def parse_page(diagram: Element, index: int) -> Page:  # noqa: C901, PLR0912, PLR0915
    ...
    for node, (ax, ay, depth) in zip(page.nodes, resolved, strict=True):
...
def parse_file(path: Path) -> list[Page]:
    ...
    if root.tag == "mxGraphModel":
        wrapper = Element("diagram", {"name": path.stem, "id": "single"})
        wrapper.append(root)
        ...
        "pages": [
            {
                ...
                "name": p.name,
                "index": p.index,
                "bounds": dict(zip(("x0", "y0", "x1", "y1"), page_bounds(p), strict=True)),
                "analysis": analyze(p),
