...
def load_scene_data(path: Path) -> dict[str, Any]:
    ...
    if len(data) > MAX_INPUT_BYTES:
        _fail(f"source exceeds the {MAX_INPUT_BYTES // (1024 * 1024)} MiB limit")
    try:
        document = json.loads(
            data.decode("utf-8"),
            parse_constant=lambda token: _fail(f"invalid geometry: {token} is not finite"),
        )
...
def parse_scene(path: Path, document: dict[str, Any]) -> Scene:  # noqa: C901, PLR0912, PLR0915
    ...
    for element in live:
        ...
        groups = (
            ...
            if isinstance(element.get("groupIds"), list)
            else []
        )
        scene.nodes.append(
            Node(
    ...
    for element in live:
        ...
        source, target = (
            (end_bound, start_bound) if start_head and not end_head else (start_bound, end_bound)
        )
...
def analyze(scene: Scene) -> dict[str, Any]:
    nodes = scene.nodes
    edges = scene.edges
    drawable = [node for node in nodes if node.shape != "text" and (node.label or node.children)]
    ...
    orphans = [
        name_of(n) for n in leaves if not n.in_degree and not n.out_degree and n.shape != "frame"
    ]
...
def digest(path: Path, scene: Scene, max_rows: int) -> str:
    ...
    out.append("")
    out.append(f"1 scene: {_escape_inline(scene.name)} ({len(scene.nodes)}n/{len(scene.edges)}e)")
    out.append("")
    out.append(f"## Scene — {_escape_inline(scene.name)}")
    out.append("")
    out.append(
        f"- source canvas: {int(x1 - x0)}×{int(y1 - y0)} px (aspect {((x1 - x0) / (y1 - y0)):.2f})"
        if y1 > y0
    ...
    if dropped:
        out.append(
            "- discarded: "
            + ", ".join(f"{count} {key.replace('_', ' ')}" for key, count in dropped.items())
        )
    if info["hubs"]:
        hubs = ", ".join(
...
def to_json(path: Path, scene: Scene) -> str:
    payload = {
        ...
        "scene": {
            "name": scene.name,
            "bounds": dict(zip(("x0", "y0", "x1", "y1"), scene_bounds(scene), strict=True)),
