...
SKILL = GTMSkill(
    ...
    description=(
        ...
        "already carries. READS THE PIXELS of every shot bound to an existing REAL asset "
        "(`vision`'s `extract_text`, then `gtm_core.spoiler_check`) and refuses one whose "
        "visible text names a product the script has not introduced by that timecode: a reused "
        "console capture at 1:48 showed a DID reading `...:fabric-gateway-...`, naming the "
        "product 75 seconds before the script says it at 3:02, and every gate passed it because "
        "the asset was real, current and on-brand — nothing asked what the picture SAID. The "
        "term vocabulary is closed to the profile's own products, so a screenshot can never "
        "introduce, widen or steer the check, and an unreadable asset is reported NOT CHECKED "
        "rather than clean (`extract_text` never raises; it returns `[vision-error] ...`). "
        "Writes "
