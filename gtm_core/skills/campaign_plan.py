...
SKILL = GTMSkill(
    ...
    description=(
        ...
        "Chooses the campaign's **message portfolio** — which arguments run, drawn from the "
        "profile's hook-matrix cells, and which persona cohort each is assigned to — so the "
        "message is a plan decision rather than whatever the first sequence spec happens to "
        "say. North-star metric is SQLs; every send stays human-gated — this skill plans, it "
        "never sends."
    ),
)
