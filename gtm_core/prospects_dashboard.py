...
def render_html(status: dict) -> str:
    ...
    any_active = any((s.get("status", "") or "").upper() == "ACTIVE" for s in sequences)
    ...
    if sequences:
        ...
        <div class="card">
          ...
          {"".join(cards)}
          <p class="note">Live from Saleshandy stats. {
            "Sending is live — deliverability (bounce %) is the number to watch during mailbox ramp."
            if any_active
            else "Nothing sends until a human resumes a paused sequence — deliverability (bounce %) is the number to watch during mailbox ramp."
        }</p>
        </div>"""
...
<div class="card next">
  ...
  <ol class="steps">
    <li><b>{"Sending now" if any_active else "Send now"}:</b> {
        f["ready"]:,} contacts are verified and ready — {
        "already loaded and sending in Saleshandy; watch replies and bounce rate as the mailboxes ramp."
        if any_active
        else "review the paused sequence in Saleshandy, then activate."
    }</li>
