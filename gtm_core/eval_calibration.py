...
def is_calibrated(profile: str, content_root: Path | None = None) -> bool:
    """Whether any sealed holdout exists for ``profile``. See :func:`sealed_holdouts`."""
    return bool(sealed_holdouts(profile, content_root))
...
#:
#: The first four are scoped to the ROW's fact and copy. `account_fit` is the one
...
def render_labeling_sheet(rows: Sequence[GoldenRow], prefill: dict[str, dict] | None = None) -> str:
    """Markdown labeling sheet: rendered subject + body + the row's own context — nothing a
    recipient would not themselves know, and nothing this pipeline computed about the row
    (no linter verdict, no adjudication score, no injection flag, no batch identity).
    ...
    **The blank rows are the methodologically load-bearing ones.** Pre-filling is a real
    ...
    lines = [
        ...
        "respect), then `fact_creates_problem` / `fact_supports_pitch` / `frame_fits_seat` / "
        "`right_person` / `account_fit` (Y/N/`-` for not-applicable), then a one-line note on "
        ...
        "",
        "**Answer `send_it` on your own read of the email, not by adding up the four "
        "sub-checks.** It is the label the judge is validated against; deriving it from the "
        "others turns a taste judgment into arithmetic.",
        "",
    ]
    ...
    for i, r in enumerate(rows, 1):
        ...
        if pre:
            answers = (
                ...
                f"`fact_creates_problem:` {_fmt(pre.get('fact_creates_problem'))}  "
                f"`fact_supports_pitch:` {_fmt(pre.get('fact_supports_pitch'))}  "
                f"`frame_fits_seat:` {_fmt(pre.get('frame_fits_seat'))}  "
                f"`right_person:` {_fmt(pre.get('right_person'))}  "
                f"`account_fit:` {_fmt(pre.get('account_fit'))}"
            )
            ...
            answers = (
                ...
                "`frame_fits_seat:` ___  `right_person:` ___  `account_fit:` ___"
            )
            note = "`note:`"
            marker = "  **← label this one cold**" if prefill else ""
        lines += [
            f"## Row {i} — `{r.row_id}`{marker}",
            "",
...
class Label:
    ...
    send_it: bool
    touch: int = 1
    fact_creates_problem: bool | None = None
    fact_supports_pitch: bool | None = None
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "touch": self.touch,
            "fact_creates_problem": self.fact_creates_problem,
            "fact_supports_pitch": self.fact_supports_pitch,
            "frame_fits_seat": self.frame_fits_seat,
            "right_person": self.right_person,
            "account_fit": self.account_fit,
...
def label_from_dict(d: dict) -> Label:
    ...
    return Label(
        ...
        touch=d.get("touch", 1),
        fact_creates_problem=d.get("fact_creates_problem"),
        fact_supports_pitch=d.get("fact_supports_pitch"),
        frame_fits_seat=d.get("frame_fits_seat"),
        right_person=d.get("right_person"),
        account_fit=d.get("account_fit"),
        send_it=d["send_it"],
        note=d.get("note", ""),
...
_ROW_HEADER_RE = re.compile(r"^## Row\s+(\d+)\s+—\s+`([0-9a-f]{16})`", re.M)
    ...
    for i, m in enumerate(headers):
        ...
        out.append(
            Label(
                ...
                send_it=send,
                fact_creates_problem=_parse_bool(
                    fields.get("fact_creates_problem", ""), field="fact_creates_problem", row=row_id
                ),
                fact_supports_pitch=_parse_bool(
                    fields.get("fact_supports_pitch", ""), field="fact_supports_pitch", row=row_id
                ),
                frame_fits_seat=_parse_bool(
                    fields.get("frame_fits_seat", ""), field="frame_fits_seat", row=row_id
                ),
                right_person=_parse_bool(
                    fields.get("right_person", ""), field="right_person", row=row_id
                ),
                # Absent from an older sheet => None => never disqualifies. The field is
                # parsed here rather than only in the HTML labeler so a markdown sheet that
                # DID ask the question cannot silently drop the answer.
                account_fit=_parse_bool(
                    fields.get("account_fit", ""), field="account_fit", row=row_id
                ),
...
def intra_rater_agreement(labels: Sequence[Label]) -> dict:
    ...
    **This number is the ceiling on the whole program.** A judge cannot meaningfully score
    ...
    if not repeats:
        return {"duplicate_rows": 0, "agreements": 0, "agreement": None, "disagreed_rows": []}
...
def reconcile_outcomes(
    ...
    *,
    ...
    negative_outcomes = {"opt_out", "reply_negative", "objection:do-not-contact", "unsubscribe"}

    replied_positive = {
        (row.get("ref") or "").strip().lower()
        for row in outcome_rows
        if row.get("channel") == "email" and row.get("outcome") in positive_outcomes
    }
    replied_negative = {
        (row.get("ref") or "").strip().lower()
        for row in outcome_rows
        if row.get("channel") == "email" and row.get("outcome") in negative_outcomes
    }
    had_any_outcome = replied_positive | replied_negative
