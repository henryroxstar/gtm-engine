...
#: Formats whose opening beat is a *video* execution. Kept in step with
#: ``gtm_core.content_quality.model._VIDEO_FORMATS`` and ``format-router``'s routing table.
_VIDEO_BEAT_FORMATS = frozenset({"short", "reel", "clip"})
...
def _lint_hook(hook: hk.Hook, index: int, banned: hk.BannedConfig) -> list[str]:
    ...
    for beat in hook.opening_beats:
        ...
        if beat.format in _VIDEO_BEAT_FORMATS:
            # A video beat is two streams, not one. ``video`` is the spoken cold-open line;
            # ``text`` is the burned-in caption the viewer reads in the first three seconds --
            # and on a muted autoplay feed the caption is the only stream that lands. Every
            # video beat in both live banks carried ``video`` and no ``text``, so the caption
            # had no declared source and each script re-invented one downstream, unattributed
            # to the hook. video-script's rule that a caption is CUT FROM the spoken line is
            # what makes both fields the same beat rather than two competing hooks.
            if not (beat.video or "").strip():
                errors.append(
                    f"{prefix} opening_beat for video format {beat.format!r} has no `video` "
                    "(the spoken cold-open line)"
                )
            if not (beat.text or "").strip():
                errors.append(
                    f"{prefix} opening_beat for video format {beat.format!r} has no `text` "
                    "(the burned-in caption; cut it from the spoken line, do not write a "
