...
ALLOWED_HOSTS: frozenset[str] = frozenset(
    {
        ...
        "resource2.heygen.ai",
        # HeyGen — the licensed background-music / SFX catalogue. Pinned 2026-08-29 from the
        # `audio_url` of a live `search_audio_sounds` response, never guessed: the catalogue
        # tool returns a short-lived pre-signed S3 URL and no MCP tool hands back the bytes,
        # so without this pin a score is representable but not obtainable.
        #
        # This is a BUCKET-scoped host, not an AWS-wide one: on an S3 transfer-acceleration
        # endpoint the leftmost label IS the bucket, so this admits exactly HeyGen's own
        # `heygen-product` bucket and nothing else on s3-accelerate.amazonaws.com. That is
        # the same shape already pinned on the write side in `gtm_core.reap_upload`
        # (`reap-user-upload-bkt-prod.s3-accelerate.amazonaws.com`) — a precedent, not a new
        # class of destination. A `*.s3-accelerate.amazonaws.com` wildcard would admit every
        # S3 bucket on the internet and must never be written here.
        "heygen-product.s3-accelerate.amazonaws.com",
        # Reap Video Studio — finished project media (captioned/reframed exports). Pinned
        # 2026-08-29 from the `clipUrl` of a live `get_results` response on a completed
        # captions project. This is the GET counterpart to the PUT host already pinned in
        # `gtm_core.reap_upload` (`reap-user-upload-bkt-prod…`): a file goes up to the upload
        # bucket and comes back from this one, and they are different buckets — neither pin
        # implies the other. Bucket-scoped for the same reason as the HeyGen entry above.
        "reap-video-main-bkt-prod.s3-accelerate.amazonaws.com",
    }
)
