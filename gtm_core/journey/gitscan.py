...
import os
import re
import subprocess  # nosec B404 — fixed-argv `git` calls only (see _git); never a shell
...
# A git --shortstat line, e.g. " 3 files changed, 45 insertions(+), 12 deletions(-)".
# Anchored to the real format (leading count + "file(s) changed") so a commit body
# that merely contains the word "changed" is never mistaken for a stat line.
_STAT_LINE_RE = re.compile(r"^\s*\d+ files? changed")
...
def _run(args: list[str], repo_root: Path | None = None) -> str:
    ...
    result = subprocess.run(  # nosec B603 — list argv, shell=False; root is a path, args are caller literals
        ["git", "-C", str(root)] + args,
        capture_output=True,
        text=True,
        check=True,
...
def commits(
    ...
    for raw_block in blocks:
        ...
        if not stripped:
            continue
        # Extract optional leading stat line (git's own " N files changed, …" summary).
        # Match the anchored shortstat format, NOT the bare word "changed" — a commit
        # body line like "…on the changed files" must not be peeled off as a stat line.
        stat_line = ""
        remainder = stripped
        lines = stripped.splitlines()
        stat_candidates = [ln for ln in lines if _STAT_LINE_RE.match(ln)]
        if stat_candidates:
