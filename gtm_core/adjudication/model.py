...
class Adjudication:
    ...
    #: silent confound. A ``re-angle`` under ``seat-only`` says the seat or the framing is
    #: wrong; the same verdict under ``full`` may only mean the opener carried no fact.
    #:
    #: Why the split exists: through 2026-09-06 every lane was scored on all three items,
    #: including the GENERIC lane, whose defining property is that it carries no per-row
    #: signal. ``fact_earns_its_place`` asks whether "the recipient's own recorded evidence
    #: establishes what the body then claims" — a question a signal-free row cannot pass by
    #: construction. On the 2026-09-04 SG-builder roster that produced 23 rejections of 23
    #: rows, 20 of whose notes name the same absence ("signal_clause and signal_evidence are
