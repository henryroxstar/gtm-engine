...
from gtm_core.prospects_consolidate import MarketGate, _org_token, _pool_dir
...
def load_rubric(profile: str, profiles_root: Path | None = None) -> dict:
    ...
    # Normalize keyword casing once so scoring is a plain substring test.
...
def score_account(rec: dict, rubric: dict) -> tuple[int, str]:
    ...
    haystack = f"{rec.get('industry', '')} {rec.get('description', '')}".lower()
    score = 0
    cohort = ""
...
def select_backlog(
    ...
    *,
    ...
    queue.sort(key=lambda r: (-r["score"], -r["top_intent_score"], r["company"].lower()))
