...
#: ``with_disclosure`` is OFF, and the Art. 50 duty is not weakened by that — it never lived here.
#: ``agent.publish.validate_disclosure`` reads the POST TEXT and fails closed on a synthetic-identity
#: post that does not carry the tenant's ``[disclosure].line``; the burned-in copy was always a second
#: rendering of a line the gate already enforces somewhere else. Removed 2026-09-09 on the operator's
#: instruction after it shipped on an end card at 0.040h — 77px at 9:16, set larger than the CTA and
#: second only to the wordmark, so a legal footnote read as the card's message. **Do not reinstate an
#: on-card disclosure, and do not reintroduce a minimum size for one.** If a future surface genuinely
#: needs the line burned in, it gets a footnote treatment sized against the caption scale and argued
#: for on that surface — never a floor asserted here as a duty.
_TITLE_CLOSE = _TitleSpec("title-close", _SPINE_ONE, _SPINE_TWO, has_cta=True)
...
#: Wraps a string to a pixel width. (It previously carried a rule that the disclosure line be drawn
#: at a FIXED size "because its size is a legibility duty rather than a design choice" — deleted
#: 2026-09-09. That reasoning is what produced 77px type at 9:16: a size floor asserted as a legal
#: obligation, when the obligation is discharged on the post text by ``validate_disclosure``. No
#: element on this card carries a minimum size.)
def _wrap_px(
...
def render_title_card_frames(
    *,
    ...
    #: The CTA's window is set HERE, independent of any disclosure. It used to be assigned only
    #: inside the disclosure branch below, so when the on-card disclosure was removed on 2026-09-09
    #: the CTA silently inherited its inert 0.70..0.70 default and stopped rendering at all — the
    #: card lost "On the App Store" without erroring. Caught by
    #: ``test_a_film_may_state_its_own_call_to_action``, which is the reason that test earns its
    #: keep: it compares two cards that differ ONLY in their CTA, so a CTA that renders nowhere
    #: makes them identical. With no disclosure after it, the CTA is the card's last element and
    #: takes the tail.
    if cta_text and not disclosure:
        ONE_START, ONE_END = 0.04, 0.20
        TWO_START, TWO_END = 0.30, 0.46
        RULE_START, RULE_END = 0.52, 0.64
        CTA_START, CTA_END = 0.70, 0.86
    ...
    #: Was ``if cta_text and disclosure`` until 2026-09-09 — a card with a CTA and NO disclosure had
    #: no layout branch at all, so ``cta_font`` stayed None and the CTA silently did not draw. That
    #: never surfaced while every CTA card also carried a disclosure. It is ``if cta_text`` now: with
    #: no disclosure ``disc_block_h`` is 0, so ``disc_y_fixed`` collapses to the caption band and the
    #: CTA anchors directly above it — the same upward stack, one element shorter.
    if cta_text:
        probe = ImageDraw.Draw(ground)
        block_h_probe = (line_px * 2 + round(h * 0.030)) if spec.line_two else line_px
