...
def hand_send_path(profile: str, content_root: Path | None = None) -> Path:
    """The rows a PERSON sends by hand, already filtered to what may be sent.

    Visible beside ``ready-to-load.csv`` rather than hidden in ``.pool``, because it is the
    other file a human acts on. It holds the rows the merge-field gate refuses — a role
    inbox with no first name cannot render ``Hi {{First Name}},`` — which therefore never
    reach a sequencer and never pass through ``account_integrity`` on the way in.

    That is the whole reason this file exists. The enrollment gate is a LOAD-TIME filter,
    so a list that is never loaded is never filtered. On 2026-09-04 a hand-assembled
    role-inbox CSV carried 8 rows of which 5 were Gate-B ``drop`` verdicts — accounts
    research had already refused — and the only thing standing between them and a real
    send was the operator remembering which 3 of the 8 to copy. Writing the list here, at
    consolidation time, with the drops already removed, replaces that memory with a file
    that cannot express the mistake.
    """
    return _sequences_dir(profile, content_root) / "hand-send.csv"
