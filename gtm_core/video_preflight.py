...
#: HeyGen bills per render, per minute, ROUNDED UP — measured from this tenant's own cost ledger
#: on 2026-08-27 (nine renders totalling ~61s cost 209 credits; a separate 3.7-second re-render
#: cost 23), i.e. ~23 credits per job whatever its length.
#:
#: HeyGen's API docs quote 0.1 credits/sec — 6 per minute — for the enterprise surface. That is a
#: DIFFERENT BILLING PLANE from the MCP plan we actually draw against, and where the two
#: disagree the ledger wins: it prices the surface that sends us the invoice.
#:
#: CORRECTED 2026-08-29 — the rate is a BAND, not a number, and this is why the estimate is
#: reported as a range. A verification render on `avatar_iv` at 720p/16:9 cost **11** credits
#: (balance 356 -> 345), against the ~23 measured on the 2026-08-27 `avatar_v` + `avatar_iii`
#: batch. So cost moves with the ENGINE and the RESOLUTION, neither of which exists at routing
#: time — which is exactly why C5 says a coarse figure must not be dressed as a quote. Quoting
#: the top of the band as a point estimate would over-state a cheap lane by 2x and quietly
#: discourage the lane; quoting the bottom would under-state the expensive one. Report both.
#:
#: Still per-JOB, not per-minute, on both measurements. The name is kept for the existing callers
#: but read it as "per render": a multi-shot presenter run costs roughly this times the SHOT
#: COUNT, and PENDING.md V-1 tracks making the engine an input instead of a band.
HEYGEN_CREDITS_PER_RENDER_MINUTE = 23

#: The cheap end of that band — `avatar_iv` at 720p, measured 2026-08-29.
HEYGEN_CREDITS_PER_RENDER_MIN = 11
...
class LaneRequirement:
    ...
    #: the lane reports no estimate at all — a wrong number is worse than none.
...
LANE_REQUIREMENTS: dict[str, LaneRequirement] = {
    ...
    "presenter-video": LaneRequirement(
        ...
        credits_per_minute=HEYGEN_CREDITS_PER_RENDER_MINUTE,
        credits_per_minute_min=HEYGEN_CREDITS_PER_RENDER_MIN,
    ),
...
def _render_cost(lane: LaneStatus) -> str:
    ...
    A band is rendered as a band. Collapsing "11-23" to either endpoint is how a coarse figure
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        pf = preflight(args.profile, args.product, profiles_root=args.profiles_root)
