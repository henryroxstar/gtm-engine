...
def load_live_rows(source: SourcedTouches, matrix: Matrix | None = None) -> list[dict]:
...
def all_live_rows(
    ...
    for src in sources:
        ...
        rows.extend(load_live_rows(src, matrix=matrix))
...
INJECTION_RECIPES = [
    ...
    _mutate_row(
        "premise-unsupported",
        # The Autodesk/Navigator/ARKA shape: one product on one platform, under a body whose
    ...
    _mutate_row(
        ...
        # The CACI shape: the clause names one company, the email goes to another. Renders
