...
class RenderManifest:
    ...
    #: How many of ``draft_pool_size`` actually came back. C5: a pool of five where two renders
    #: failed is a choice made from THREE, and recording it as "1 of 5" overstates how much was
    #: looked at — which is the number a later reader would use to judge whether the pick was
    #: informed. ``None`` means every requested member succeeded.
    pool_succeeded: int | None = None
    #: HOW the winner was chosen (``operator_contact_sheet``, a scorer's name). C5: which member
    #: won is only meaningful alongside this; without it the record cannot tell an operator's eye
    #: from a scorer's number. Optional on the pre-C5 hook pool, whose criterion was implicit.
...
def write_render_manifest(
    ...
    *,
    ...
    validate_draft_pool(
        manifest.draft_pool_size,
        manifest.draft_rank,
        manifest.pool_predictor_score,
        manifest.pool_succeeded,
    )
...
def load_render(path: Path) -> RenderManifest:
    ...
    _fill_identity_fields(filtered, data, path)
    return RenderManifest(**filtered)


#: The four fields :class:`RenderManifest` requires positionally. A hand-authored manifest that
#: omits any of them made ``RenderManifest(**filtered)`` raise ``TypeError`` — not a
#: :class:`ManifestError` a caller can report, but a stack trace out of the loader, which is why
#: ``render_manifest validate`` could not be run at all against the multi-shot manifests the
#: ``video-render`` skill actually writes (reproduced 2026-09-03 on
#: ``content/<tenant>/video/2026-09-03-launch-film/render-9x16.json``: *"__init__()
#: missing 3 required positional arguments"*). A validator that cannot load its input is not a
#: gate. These three are all recoverable from what the file and its location already say.
_REQUIRED_FIELDS = ("profile", "slug", "ratio", "asset_path")


def _fill_identity_fields(filtered: dict, data: dict, path: Path) -> None:
    """Recover the identity fields a companion-shaped manifest states elsewhere.

    Deliberately narrow: it recovers only facts the file or its own path ALREADY asserts —

    * ``profile`` / ``slug`` from ``content/<profile>/video/<slug>/render-<ratio>.json``, the
      layout ``CLAUDE.md`` pins and ``ledger_cli reconcile-renders`` already globs;
    * ``ratio`` from the filename stem, which is where the writer put it;
    * ``asset_path`` from ``stitched_asset.path`` — the multi-shot shape's own name for "the
      asset this manifest is about".

    It recovers **nothing about cost**. ``cost_credits``/``cost_source``/``cost_ledger_ts`` are
    left exactly as written, so a manifest that never metered its spend still fails
    :func:`_validate_cost` rather than being quietly repaired into looking compliant. Inferring a
    spend from a free-text ``credits_spent`` blob would defeat the gate this loader exists to let
    run.
    """
    parts = path.resolve().parts
    if not filtered.get("slug"):
        filtered["slug"] = path.parent.name
    if not filtered.get("profile") and "content" in parts:
        after = parts[parts.index("content") + 1 :]
        if after:
            filtered["profile"] = after[0]
    if not filtered.get("ratio") and path.stem.startswith("render-"):
        filtered["ratio"] = path.stem[len("render-") :]
    if not filtered.get("asset_path"):
        stitched = data.get("stitched_asset")
        if isinstance(stitched, dict):
            filtered["asset_path"] = str(stitched.get("path", "") or "")
    still_missing = [f for f in _REQUIRED_FIELDS if not filtered.get(f)]
    if still_missing:
        raise ManifestError(
            f"{path} cannot be loaded as a render manifest: missing {still_missing} and nothing "
            "in the file or its path supplies them. Expected either the fields themselves or the "
            "conventional layout content/<profile>/video/<slug>/render-<ratio>.json with the "
            "asset named by `asset_path` (or `stitched_asset.path`)."
        )
...
def validate_render_file(path: Path, *, synthetic: bool, repo_root: Path | None = None) -> None:
    ...
    **The cost rule runs here too, since 2026-09-03.** It did not, and that omission is half of
    why the gate never bound: ``write_render_manifest`` refused an unmetered synthetic spend, but
    the video lane never called the writer — it hand-authored the JSON and then ran *this*
    function, which checked identity, prompt, lip sync and paths and said ``OK`` to a manifest
    recording 405 credits and no ledger row. A gate that is only enforced on the path nobody takes
    is not enforced.

    ``_validate_storyboard`` and ``_validate_engine`` are still writer-only, deliberately: unlike
    cost, neither fact has a home in the companion shape the skill writes, so running them here
    would fail every existing manifest for reasons unrelated to the spend it never metered. They
    bind on the ``write_render_manifest`` path; closing them on this one is separate work.
    """
    m = load_render(path)
    ...
    _validate_cost(m.cost_source, m.cost_credits, m.cost_ledger_ts, synthetic=synthetic)
    validate_draft_pool(m.draft_pool_size, m.draft_rank, m.pool_predictor_score, m.pool_succeeded)
    root = repo_root if repo_root is not None else Path.cwd()
...
# Re-exported from the bottom so `render_manifest` can call it while the sibling imports
# `ManifestError` from here — the same cycle-avoidance idiom `gtm_core/cells.py` uses. Bound at
# import time, so every call site inside a function resolves it normally.
#
# This import MUST stay above the `__main__` block. It used to sit below it, which meant that under
# `python -m gtm_core.render_manifest` — the only form any skill actually invokes — `main()` ran
# before the name was bound and every synthetic validation died with
# `NameError: name 'validate_draft_pool' is not defined`. Imported as a library the name resolved
# fine, so the module's own tests passed and the CLI gate was inert. Found 2026-09-08 by running
# the validation step CLAUDE.md mandates.
from .render_manifest_pool import validate_draft_pool  # noqa: E402
