...
POLICIES: tuple[WindowPolicy, ...] = (
    ...
    WindowPolicy(
        lane="own_product_watch",
        source_id="own_product_watch",
        label="Own product watch (releases, blog, changelog)",
        first_run_days=90,
        note="Our own public releases. A short window is fine, but a first run should look back "
        "far enough to establish the current baseline — 90 days catches the last major release.",
    ),
    WindowPolicy(
        lane="funding_and_ma",
        source_id="funding_and_ma",
        label="Funding & M&A moves",
        first_run_days=180,
        min_days=30,
        note="Funding rounds are lumpy. A 7-day window often returns nothing, which reads as "
        "'no capital activity' when it means 'wrong window'. A 30-day floor is the minimum "
        "meaningful view; cold-start pulls 180 days.",
    ),
    WindowPolicy(
        lane="regulatory_enforcement",
        source_id="regulatory_enforcement",
        label="Regulatory & enforcement actions",
        first_run_days=180,
        min_days=30,
        note="Enforcement actions and applicability dates are event-driven; the next run after "
        "a quiet week should still inspect the last 30 days so a genuine action is not missed.",
    ),
    WindowPolicy(
        lane="incidents_benchmarks",
        source_id="incidents_benchmarks",
        label="Incidents & benchmarks",
        first_run_days=180,
        min_days=30,
        note="Research round-ups and benchmark releases are sparse. A 30-day floor keeps a "
        "same-week re-run from reporting an empty incident landscape.",
    ),
    WindowPolicy(
        lane="customer_moves",
        source_id="customer_moves",
        label="Customer & prospect moves (funding, AI units, relevant hires)",
        first_run_days=180,
        min_days=30,
        note="Corporate events at accounts we sell to. Deliberately separate from "
        "funding_and_ma, which iterates the competitor registry and therefore can only ever "
        "see rivals — a prospect raising a round was invisible to every lane before this one. "
        "Lumpy like any funding signal, so it carries the same 30-day floor: a short window "
        "returns nothing, which reads as 'our accounts are quiet' when it means 'wrong window'.",
    ),
)
