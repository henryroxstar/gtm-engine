...
OWN_VOICE = "own-voice"  # our own releases, blog, changelog — the baseline, not demand
REGULATOR_VOICE = (
    "regulator-voice"  # enforcement / applicability dates — forcing functions, not demand
)
# A corporate event at a customer or prospect — a round raised, an AI unit stood up, a relevant
# exec hired. Deliberately NOT `customer-voice`: it is a *capacity* signal (new budget, new
# urgency, a named owner), never a statement of need. Deliberately NOT `vendor-voice` either —
# conflating a prospect with a rival in one chip provokes exactly the wrong question of a reader.
# The register rule still governs: if the same company files a risk disclosure saying it cannot
# control its agents, that passage is `customer-voice` and arrives via `enterprise_filings`.
...
def _allow_list(signals_dir: Path, patterns: list[str]) -> list[Path]:
    """Return files directly in *signals_dir* whose name matches any allow-list *pattern*.

    Patterns are shell-style globs (``fnmatch``) unless they start with ``^``, in which
    case they are treated as regex and matched with ``re.fullmatch``. The caller owns the
    sort order; this helper returns an unsorted list so callers can sort once.
    """
    matched: list[Path] = []
    for p in signals_dir.glob("*"):
        if not p.is_file():
            continue
        for pat in patterns:
            if pat.startswith("^"):
...
def collect(
    ...
    #     Only artifacts that follow the syften-digest naming convention belong here.
    sig = sorted(_allow_list(signals_dir, ["syften-digest-*.md"]))
    sources.append(
        _source(
            id="syften_market_signals",
            # The label is what a reader sees in the brief's source table, and what
            # gtm_core.brief_lint hands a writer as the replacement when the id leaks. So it
            # names the KIND of source, not the vendor we buy it from — "Syften" means
            # nothing to a product or sales reader.
            label="Social listening — practitioner posts",
    ...
    # 1a — Weekly web sweep produced by market-scan (news, regulators, competitors, own-brand).
    #      MIXED: split at read time into customer-voice, vendor-voice, own-voice, regulator-voice.
    web_sweep = sorted(
        _allow_list(signals_dir, [r"^\d{4}-\d{2}-signals\.md$", "*-2wk-snapshot-*.md"])
    )
    sources.append(
        _source(
            id="web_sweep",
            label="Weekly web sweep (news, regulators, competitors, own-brand)",
            speaker=MIXED,
            provenance_class="external-plus-interpretation",
            present=bool(web_sweep),
            corpus_count=len(web_sweep),
            latest=_latest(web_sweep, today),
            max_age_days=14,
            note="Produced by market-scan. Mixed genre — split at read time: third-party "
            "news/regulation = customer-voice; competitor mentions = vendor-voice; our own "
            "brand/product releases = own-voice; regulator announcements = "
            "regulator-voice. Does NOT count toward breadth as a source.",
        )
    )

    # 1b — Own product watch. OWN VOICE: what we shipped — the baseline demand is measured against.
    own = sorted(_allow_list(signals_dir, ["own-product-*.md"]))
    sources.append(
        _source(
            id="own_product_watch",
            label="Own product watch (releases, blog, changelog)",
            speaker=OWN_VOICE,
            provenance_class="own-public",
            present=bool(own),
            corpus_count=len(own),
            latest=_latest(own, today),
            max_age_days=90,
            note="Our own public releases and changelog. This is the capability baseline — "
            "counting it as demand would be marking our own homework. Renders as §3b.",
        )
    )

    # 1c — Funding & M&A moves. VENDOR VOICE: competitor/peer financing, not customer demand.
    funding = sorted(_allow_list(signals_dir, ["funding-moves-*.md"]))
    sources.append(
        _source(
            id="funding_and_ma",
            label="Funding & M&A moves",
            speaker=VENDOR_VOICE,
            provenance_class="external-financial",
            present=bool(funding),
            corpus_count=len(funding),
            latest=_latest(funding, today),
            max_age_days=60,
            note="Funding rounds, acquisitions, and other capital events in the agentic-AI "
            "identity/governance space. Vendor-side signal — never counts toward demand breadth.",
        )
    )

    # 1d — Regulatory & enforcement actions. REGULATOR VOICE: forcing function, not demand.
    regulatory = sorted(_allow_list(signals_dir, ["regulatory-*.md"]))
    sources.append(
        _source(
            id="regulatory_enforcement",
            label="Regulatory & enforcement actions",
            speaker=REGULATOR_VOICE,
            provenance_class="external-regulatory",
            present=bool(regulatory),
            corpus_count=len(regulatory),
            latest=_latest(regulatory, today),
            max_age_days=60,
            note="Enforcement actions, penalty notices, and applicability dates from regulators "
            "and standards bodies. A forcing function that creates urgency — no customer said "
            "anything. Renders as §4c.",
        )
    )

    # 1e — Incidents & benchmarks. EXPERT LENS: measured/observed failures and research round-ups.
    incidents = sorted(_allow_list(signals_dir, ["incidents-*.md"]))
    sources.append(
        _source(
            id="incidents_benchmarks",
            label="Incidents & benchmarks",
            speaker=EXPERT_LENS,
            provenance_class="external-research",
            present=bool(incidents),
            corpus_count=len(incidents),
            latest=_latest(incidents, today),
            max_age_days=60,
            note="Security incidents, benchmark studies, and research round-ups. Expert-lens "
            "signal — useful for framing, not evidence of customer demand.",
        )
    )

    # 1f — Customer & prospect moves. ACCOUNT EVENT: capacity and timing, never stated need.
    moves = sorted(_allow_list(signals_dir, ["customer-moves-*.md"]))
    sources.append(
        _source(
            id="customer_moves",
            label="Customer & prospect moves (funding, AI units, relevant hires)",
            speaker=ACCOUNT_EVENT,
            provenance_class="external-financial",
            present=bool(moves),
            corpus_count=len(moves),
            latest=_latest(moves, today),
            max_age_days=60,
            note="Corporate events at accounts we sell to — rounds raised, AI/automation units "
            "stood up, relevant exec hires. Answers 'who just acquired budget and urgency', which "
            "is a TIMING signal for sales, not evidence of demand. Never counts toward breadth. "
            "Holds third-party PII, so it stays under content/ per §R9. Renders as §4e.",
        )
    )

    # 2 — Standards & spec watch. STANDARDS VOICE: where the category is DEFINED.
    ...
    filings = sorted(_allow_list(signals_dir, ["enterprise-signals-*.md"]))
    sources.append(
        _source(
    ...
    digest = sorted(_allow_list(signals_dir, ["market-intel-*.md"]))
    sources.append(
        _source(
    ...
    return {
        ...
        "speakers": {
            ...
            "own-voice": "our own public releases, blog, and changelog — the baseline demand is "
            "measured against, not demand itself",
            "regulator-voice": "enforcement actions and applicability dates — a forcing function "
            "that creates urgency, but no customer said anything",
            "account-event": "a corporate event at a customer or prospect (round raised, AI unit "
            "stood up, relevant hire) — who just acquired budget and urgency. A timing signal for "
            "sales, never a statement of need",
