...
# EDGAR's own `display_names` carries a trailing "(TICKER)" — e.g. "Atlassian Corp (TEAM)" —
# that the roster's short-name paragraph omits. Stripped before the legal-form pass, or the same
# filer normalises to two keys (confirmed live 2026-09-04: 5 backlog rows split from their
# already-verified counterpart this way, e.g. "atlassiancorpteam" vs "atlassian").
...
# First-column header labels seen across roster markdown formats. "filer" (used from
# 2026-08-18) went undetected and imported the header row itself as a company named "Filer" —
# confirmed live in evidence.jsonl (date "Filed", copied from the adjacent header cell).
_HEADER_LABELS = frozenset({"company", "filer"})
...
def _cells(line: str) -> list[str] | None:
    ...
    if len(cells) < 2 or not cells[0]:
        return None
    if cells[0].lower() in _HEADER_LABELS or set(cells[0]) <= {"-", ":"}:
        return None
    return cells
