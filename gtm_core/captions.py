...
@dataclass(frozen=True)
class WordTiming:
    """One word's real start/end, e.g. from Reap ``transcribe`` (Phase E). Supersedes the
    evenly-apportioned estimate :func:`split_screens` produces from ``total_s`` alone."""
...
def contrast_against_backdrop(
    ...
    for off in range(0, len(raw) - 2, 3):
        ...
        # Rec.601 luma is enough to decide "is this a glyph"; the WCAG maths runs on the mean.
        if (0.299 * r + 0.587 * g + 0.114 * b) < _GLYPH_LUMA_FLOOR:
            sums[0] += r
            sums[1] += g
            sums[2] += b
            kept += 1
    if kept:
        ...
        # Every pixel is glyph-bright: the backdrop is blown out, which is its own failure.
        n = max(1, len(raw) // 3)
...
def fit(
    ...
    *,
    ...
    while font_px >= min_px:
        ...
        lines = _wrap_greedy(text, font, safe_w, scratch)
...
def _draw_line_with_emphasis(
    ...
    *,
    ...
    for word in line.split(" "):
        ...
        fill = accent_rgba if bare in emphasis_lower else (255, 255, 255, 255)
