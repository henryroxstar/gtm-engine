...
Subcommands::
    ...
    python -m gtm_core.ledger_cli meter-render      --profile P --credits N --op OP [--model M] [--slug S]
...
def reconcile_renders(
    ...
    Totals are reported in **USD as well as credits** where the profile declares a rate
    (:mod:`gtm_core.credit_rates`). Credits alone never answered the question this verb is asked:
    the cap is denominated in dollars, so "529.8 credits unmetered" needs a second lookup before
    anyone can tell whether it mattered. Where no rate is declared the USD figures are ``None``
    and ``rate_error`` says why — never ``0.0``, which is indistinguishable from the free case
    and is precisely the reading that let this spend hide.
    """
    from gtm_core.credit_rates import rate_report
        ...
        rows.append(
            {
                ...
                # A manifest with no `cost_credits` KEY is a different animal from one claiming
                # zero, and collapsing the two is how one tenant's launch film reconciled clean: it
                # spent 405 credits, states them only in a free-text `credits_spent` blob this
                # verb does not read, and so contributed 0.0 to every total while looking
                # accounted for. Recorded as its own fact and counted separately — the credits
                # are UNKNOWN, not absent, and no total may imply otherwise.
                "cost_stated": "cost_credits" in data,
    ...
    unstated = [r for r in priced if not r["cost_stated"]]
    ...
    rate = rate_report(profile, repo_root=root)
    usd_per_credit = rate["usd_per_credit"]
    for row in priced:
        row["cost_usd"] = (
            None if usd_per_credit is None else round(row["cost_credits"] * usd_per_credit, 6)
        )
    return {
        "profile": profile,
        "month": month,
        ...
        "credits_unmetered": credits_unmetered,
        "usd_per_credit": usd_per_credit,
        "rate_error": rate["error"],
        "usd_claimed": (
            None if usd_per_credit is None else round(credits_claimed * usd_per_credit, 6)
        ),
        "usd_unmetered": (
            None if usd_per_credit is None else round(credits_unmetered * usd_per_credit, 6)
        ),
        "unmetered_count": len(unmetered),
        "unmetered": [r["path"] for r in unmetered],
        # Manifests that state no cost at all. Their spend is missing from every total above, so
        # a non-zero count here means the credit figures are a FLOOR, not a measurement.
        "cost_unstated_count": len(unstated),
        "cost_unstated": [r["path"] for r in unstated],
        "rows": rows,
    }


def _meter_render(led: Any, args: Any) -> int:
    """``meter-render``: turn a credit spend into a costs.jsonl row, and hand back its ``ts``.

    The bridge that did not exist. A Higgsfield render bills in credits; the monthly cap is in
    USD; nothing in the repo converted between them, so no honest row could be written and none
    was — 405 credits on a tenant's 2026-09-03 launch film sat at ``total_usd: 0.0`` against a $25
    cap the spend had in fact eaten $17.42 of. Doing it as one verb rather than "look up the rate,
    multiply, hand-write the JSON, remember to keep the ts" is the point: every manual step is a
    step a run under time pressure skips.

    Fails CLOSED and writes NOTHING when the profile has no declared rate — an unconvertible
    spend must not become a $0 row, which is the exact defect being closed.
    """
    from gtm_core.credit_rates import CreditRateError, credits_to_usd, usd_per_credit

    try:
        rate = usd_per_credit(args.profile, repo_root=args.repo_root)
        cost_usd = credits_to_usd(args.credits, args.profile, repo_root=args.repo_root)
    except CreditRateError as exc:
        print(f"[ledger] cannot meter {args.credits} credits: {exc}", file=sys.stderr)
        return 1
    ...
    written = led.append_cost(record)
...
def main(argv: list[str] | None = None) -> int:
    ...
    mr = sub.add_parser("meter-render")
    mr.add_argument("--profile", required=True)
    mr.add_argument("--credits", type=float, required=True, help="Credits this call spent.")
    mr.add_argument("--tool", default="higgsfield", help="Ledger 'tool' field (the credit pool).")
    mr.add_argument("--op", required=True, help="e.g. 'video_render', 'virality_predictor'.")
    mr.add_argument("--model", default=None, help="Provider model/SKU, e.g. 'seedance_2_0'.")
    mr.add_argument("--slug", default=None, help="Script/video slug this spend belongs to.")
    mr.add_argument("--note", default=None, help="Free text, e.g. how the credits were measured.")
    ...
    if args.cmd == "append-cost":
        # Print the row AS WRITTEN, not "ok": the stamped `ts` is the row's natural key and is
        # what a render manifest records as `cost_ledger_ts`. Returning "ok" meant the one value
        # the caller needed next was thrown away at the point it was created.
        print(json.dumps(led.append_cost(_load_json(args.payload)), ensure_ascii=False))
        return 0

    if args.cmd == "meter-render":
        return _meter_render(led, args)
