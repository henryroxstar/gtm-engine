...
def _story_format(score_path: Path) -> bool | None:
    """``score.json``'s TOP-LEVEL ``story_format``, or ``None`` when it says nothing.

    Deliberately not read from the ``recommended`` block: :func:`_predictor_band` sweeps every
    scalar there into ``meta["predictor"]``, so a ``story_format`` placed inside it would land in
    meta — which the correlator cannot query — and only when a ``band`` happened to be present.

    ``None`` is the third state and it is load-bearing: a piece scored before this field existed
    is UNKNOWN, not "not a story". The caller writes no tag at all for it.
    """
    try:
        data = json.loads(score_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
...
def attribute(
    ...
    *,
    ...
    if score_path is not None and score_path.is_file():
        ...
        story = _story_format(score_path)
