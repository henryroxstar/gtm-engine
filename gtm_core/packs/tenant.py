...
#: The complete, deliberate override schema. Additions-only, monotone-stricter by construction —
#: there is no removal verb (F6). An unrecognised
#: key (e.g. a hypothetical future `remove_nodes`) must be REJECTED, not silently ignored: before
#: this check, `overrides.<pack>.remove_nodes = ["finish"]` parsed with no error and did nothing —
#: safe only by accident, and a live footgun the moment someone implemented that key without also
#: updating every reader of this dict.
_KNOWN_OVERRIDE_KEYS = frozenset({"variant", "add_nodes", "strengthen"})
_KNOWN_STRENGTHEN_KEYS = frozenset({"id", "add_depends_on", "add_gate"})
...
def load_pack_activation(path: Path) -> TenantPackActivation:
    ...
    malformed override (unknown pack referenced, an unrecognised override/strengthen key,
    etc.) — activation itself never silently no-ops on bad config."""
    ...
    for pack_name, raw_override in raw.get("overrides", {}).items():
        unknown = set(raw_override) - _KNOWN_OVERRIDE_KEYS
        if unknown:
            raise PackValidationError(
                "unknown_override_key",
                f"overrides.{pack_name}: unknown key(s) {sorted(unknown)} — only "
                f"{sorted(_KNOWN_OVERRIDE_KEYS)} are recognised. Overrides are additions-only "
                "(monotone-stricter by construction); a key that isn't one of these has no "
                "effect in this schema and would otherwise be silently ignored rather than "
                "rejected.",
            )
        ...
        for s in raw_override.get("strengthen", []):
            unknown_s = set(s) - _KNOWN_STRENGTHEN_KEYS
            if unknown_s:
                raise PackValidationError(
                    "unknown_strengthen_key",
                    f"overrides.{pack_name}.strengthen: unknown key(s) {sorted(unknown_s)} — "
                    f"only {sorted(_KNOWN_STRENGTHEN_KEYS)} are recognised.",
                )
            if "id" not in s:
                raise PackValidationError(
                    "missing_strengthen_id", f"overrides.{pack_name}.strengthen: entry has no 'id'"
                )
        strengthen = tuple(
            StrengthenEntry(
                id=s["id"],
...
def merge_pack_override(base: PackGraph, override: PackOverride) -> PackGraph:
    ...
    # The docstring's "re-validates the merged graph as a whole" promise, made real:
    # an override must not introduce a node the base loader would have rejected
    # (unknown skill/role, unsafe gate, external effect on an ungated node).
    validate_node_semantics(tuple(merged_nodes))
