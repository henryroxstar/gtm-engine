...
:func:`_resolve_storage_uri` is the ONE storage decision in the codebase: ``backend/broker.py``
resolves the SSE/gate fan-out through it too, so the limiter and the fan-out can never
end up pointed at different Redis instances. A5 closed the rest of the multi-worker
story — the run queue is durable in Postgres and the gate/SSE registries fan out through
that broker — and ``BACKEND_WORKERS > 1`` now REFUSES to boot without a reachable one.
"""
...
_PROXY_SECRET_HEADER = "x-gtm-proxy-secret"  # nosec B105 — header NAME; the value is RATELIMIT_PROXY_SECRET from env
