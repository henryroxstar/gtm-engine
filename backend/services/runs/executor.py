...
_SENTINELS = ("⟦GATE:plan⟧", "⟦GATE:publish⟧")
        ...
        async for chunk in sessions.run(
            ...
            **_run_kwargs,
            ...
            for sentinel in _SENTINELS:
                ...
                # A prompt run holds its gate in-process only: no run_gates row, because
                # the SDK session dies with the process and cannot be resumed (diff-table
                # row 7 — reconcile_gates fails these explicitly rather than pretending).
                ...
                # rewrite `pending_content` so the gate's audit row (and any downstream
                # consumer) sees the approved bytes, not the original draft. NOTE: the
                # backend records the approved content; the server-side publish CALL is
                # not wired in this runtime (it lives in the Telegram cockpit via
                # agent/publish.py), so this cannot ship the wrong bytes.
                ...
                # Continue streaming. `handled_gates` already contains this sentinel,
                # so it will not re-fire; do NOT clear it.
