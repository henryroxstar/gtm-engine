...
(`mcp_config.py`), never the Claude Code model itself (spec §4).
...
- Resolve the active profile **per Telegram chat** (session-bound; no global mutable
  ...
  **never** from `plugin/`.
- Run skills; call tools **only via MCP** (spec §4B) — never raw HTTP.
...
- Write the run manifest + history/cost ledgers under `content/<profile>/` (spec §10),
  via `ledgers.py`.
