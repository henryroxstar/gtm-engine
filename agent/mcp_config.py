...
Thirteen gated servers (see each block below for its gating env/config field):
``news``, ``google``, ``worker``, ``vision``, ``gemini_image``, ``higgsfield_video``,
...
def build_mcp_servers(cfg: Config, profile: str) -> dict[str, dict]:
    ...
    # falls back to a manual, paste-ready plan. Design: docs/prds/2026-07-13-email-sequence.md.
