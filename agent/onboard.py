...
> **criteria below are yours** and override any example shown there. Replace every `<…>`. If the
> rubric grows large, move it to its own `knowledge/buyer-intent-signals.md` and link it here.

**Segments:** `<Enterprise / Startup, or SMB / Mid-market, or one segment>`. **Publish ≥ 6; Tier-A ≥ `<~70% of the ceiling>`.**

**Gate A — Common ICP qualification (hit ≥`<N>` of `<M>`):** `<the buying signals every genuine fit shows>`.

**Gate B — Universal disqualifiers (an account must fail ALL to survive):** `<hard nos — wrong stage/setting, no champion, a constraint you can't meet>`.

**Gate C — Firmographic floor:** `<size / stage / revenue minimums>` · HQ or material ops in a PROFILE `target_market`.

**Rubric (out of `<ceiling>`):** `<criterion (pts)>` · `<criterion (pts)>` · … — weight the signals that most predict a buy the highest.

**Heat (topic-intent, after the rubric):** intent topics live in `market-scan-config.md`; +2 either feed / +1 both, capped at the ceiling. No feed connected → heat = 0; Tier-A is fit + 🆕 new-in-role + 🔥 recency.
"""
