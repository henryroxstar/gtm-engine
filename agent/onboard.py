...
def _knowledge_topic_relpath(relpath: str) -> str | None:
    """The path of a rendered file relative to its nearest ``knowledge/`` dir, or None when the
    file isn't under one (e.g. ``PROFILE.md``, ``products/<slug>/PRODUCT.md``). Lets a per-product
    knowledge file be classified by the same rule as a profile-level one."""
    parts = relpath.split("/")
...
def _ensure_knowledge_frontmatter(files: dict[str, str], draft: dict, today: date) -> None:
    ...
    ``files`` that lacks it — in place, keys unchanged. Reuses the shipped ``knowledge_meta``
    helpers so the output matches ``knowledge_meta seed`` + the parser by construction, so a freshly
    onboarded profile passes ``knowledge_meta_check`` with no manual seed
    (docs/prds/2026-07-18-knowledge-lifecycle.md). Idempotent: files that already carry frontmatter
    (e.g. the ``_template`` starters) are left byte-for-byte."""
