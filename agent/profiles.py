...
def profile_dir(profiles_root: Path, name: str) -> Path:
    ...
    ``name`` is guarded as a bare path segment first. The PROFILE.md check already
    made traversal unexploitable in practice, but only incidentally — this is the
    tenant-boundary guard stated explicitly, and it runs before any path is built.
    """
