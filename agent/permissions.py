...
def _classify_mcp(tool_name: str) -> Decision:
    """Allow an MCP tool as a class, unless it belongs to an allowlisted connector family.

    ``tool_name`` is ``mcp__<server>__<leaf>``; ``<server>`` may itself contain ``__``, so the leaf
    is taken from the **last** separator. A leaf matching a family prefix must be on that family's
    allowlist — otherwise it is denied (AP-02).
    """
