...
class Node:
    ...
    ``gate``/``external_effect`` are additive metadata (A10) carried from the pack
    layer so the runtime can recognise a gate/dispatch node **by declaration**
    rather than by its NAME. Defaults leave every existing graph — including
    ``DEFAULT_GRAPH`` — byte-identical.
    """
