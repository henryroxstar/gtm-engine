...
class Config:
    ...
    # --- Apollo (contact-resolution backstop + company intent) -----------------
    # Env-gated in-repo stdio worker (agent/mcp/apollo) — the LAST paid source in
    # the prospect skill's contact-enrichment waterfall (RocketReach -> Vibe ->
    # Apollo -> web). When absent the server is not spawned and the skill falls
    # back to web for contacts (§R6: external I/O only via MCP). Doppler-injected;
    # never inlined or echoed. Apollo also ships an official OAuth hosted MCP
    # connector (registered separately in plugin/.mcp.json) — this key is only for
    # the in-repo stdio surface.
    ...
    # --- Cockpit approve-and-resume (P1-6) ---
    # When armed, an ESCALATED tool call on the Telegram path (nested shell /
    # unrecognised tool — never the dangerous-program deny floor or secret reads)
    # becomes an inline Approve/Deny ask to the operator instead of a hard deny.
    # Default OFF: the shipped posture is unchanged until the operator deliberately
    # sets APPROVE_RESUME_ENABLED. Decline or timeout ⇒ deny (fail closed).
    approval_ask_enabled: bool = False
    approval_ask_timeout_s: float = 180.0
    ...
    def from_env(cls, repo_root: Path | None = None) -> Config:
        ...
        return cls(
            ...
            approval_ask_enabled=(
                (os.getenv("APPROVE_RESUME_ENABLED") or "").strip().lower()
                in ("1", "true", "yes", "on")
            ),
            approval_ask_timeout_s=float(os.getenv("APPROVE_RESUME_TIMEOUT_S") or 180.0),
        )
