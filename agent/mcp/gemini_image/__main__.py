"""Entrypoint for the Gemini image-generation worker MCP — ``python -m agent.mcp.gemini_image``.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="agent.mcp.gemini_image",
        description="Gemini image-generation worker MCP server (headless image path).",
    )
    parser.add_argument(
        "--transport",
        default="stdio",
