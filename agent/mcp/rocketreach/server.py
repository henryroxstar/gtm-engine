...
# --- Universal search endpoint indirection (EXPERIMENTAL — off by default) ----- #
# RocketReach also exposes a newer "Universal" search API (POST /v1/search/person,
# POST /v1/search/company — a different host path than classic v2, not just a
# different suffix under ROCKETREACH_BASE_URL) offering order_by=score ranking and
# page_size up to 100 vs classic's 25. It requires "Full API Access", a plan-gated
# feature (see PROFILE.md §"Connector plans & entitlements" -> rocketreach.features).
#
# This repo deliberately stays on classic v2 as the default: lookup/bulk_lookup
# never move (billing-clean, verified charge-on-success-only; Universal's newer
# "Universal Credits" metering model is unverified for us), and search's richer
# firmographic facets are largely redundant with Vibe Prospecting's discovery role
# in this skill's division of labor. What follows is *just* the plumbing so a
# future switch is a config flip, not a rewrite — it has NOT been exercised
# against a live Universal-enabled account. Verify against a real key before ever
# setting ROCKETREACH_SEARCH_API=universal outside a test.
_SEARCH_API_MODE = os.getenv("ROCKETREACH_SEARCH_API", "classic").strip().lower()
_UNIVERSAL_BASE_URL = os.getenv(
    "ROCKETREACH_UNIVERSAL_BASE_URL", "https://api.rocketreach.co/v1"
).rstrip("/")
_UNIVERSAL_SEARCH_PATH = {"person": "/search/person", "company": "/search/company"}
_CLASSIC_SEARCH_PATH = {"person": "/person/search", "company": "/searchCompany"}


def _search_url(kind: str) -> str:
    """Resolve the search endpoint URL for ``kind`` ('person' | 'company'), honouring
    ROCKETREACH_SEARCH_API. Classic (default) reproduces today's exact behavior;
    Universal is opt-in and unverified — see the module-level note above."""
    if _SEARCH_API_MODE == "universal":
        return f"{_UNIVERSAL_BASE_URL}{_UNIVERSAL_SEARCH_PATH[kind]}"
...
# Monthly LOOKUP allowance (the finite unit — see _meter). RocketReach is a flat subscription, so
# the dollar-based §R2 budget guard (vps_budget_ok) can never trip on it (cost_usd is 0). This is
# the count-based analogue: a hard stop once the month's logged `units.lookups` would exceed the
# plan's allowance. Injected via Doppler, mirroring PROFILE.md §"Connector plans & entitlements"
# (settings.connectors[].monthly_allowance). Unset/unparseable ⇒ uncapped (backwards compatible).
_MONTHLY_LOOKUP_CAP_ENV = "ROCKETREACH_MONTHLY_LOOKUP_CAP"


def _monthly_lookup_cap() -> int | None:
    """The configured monthly lookup allowance, or ``None`` when uncapped/misconfigured."""
    raw = (os.getenv(_MONTHLY_LOOKUP_CAP_ENV) or "").strip()
    if not raw:
        return None
    try:
        cap = int(raw)
...
def _allowance_remaining() -> int | None:
    """Lookups remaining under this month's allowance, or ``None`` when uncapped.

    Fail-OPEN on any ledger-read error (consistent with ``gtm_core.metering.check_budget``): a
    metering hiccup must not block a legitimate resolve, and the ``_BULK_MAX`` cap plus the flat
    plan bound the blast radius. Returns a value that may be ``<= 0`` when the cap is already spent.
    """
    cap = _monthly_lookup_cap()
    if cap is None:
        return None
    ...
    try:
        ...
        used = Ledgers(Config.from_env(), profile).month_unit_total("rocketreach", "lookups")
...
async def _lookup_one(client: httpx.AsyncClient, key: str, query: dict) -> dict:
    ...
    while status in ("searching", "progress", "waiting") and pid and polls < _MAX_POLLS:
        ...
        polls += 1
        try:
            # GET /person/checkStatus, query param `ids` is documented as array-of-int
            # (verified against RocketReach's OpenAPI spec) — pass a list so httpx emits
...
async def _lookup(query: dict) -> str:
    ...
    if remaining is not None and remaining <= 0:
        cap = _monthly_lookup_cap()
        return (
            f"[rocketreach-cap] monthly lookup allowance exhausted (~{cap} this month; used ≥{cap}). "
            "Person/company SEARCH remains free — do NOT retry lookups; report to the operator."
        )
...
async def _bulk(people: list[dict]) -> str:
    ...
    remaining = _allowance_remaining()
    if remaining is not None and len(people) > remaining:
        # Reject rather than clamp: the brain must re-prioritize which finalists to spend the
        # remaining allowance on — a silent clamp would misreport which people were resolved.
        if remaining <= 0:
            return (
                "[rocketreach-cap] monthly lookup allowance exhausted — resolve no more this month. "
                "Person/company SEARCH remains free; report to the operator."
            )
        return (
            f"[rocketreach-cap] batch of {len(people)} exceeds remaining allowance ({remaining}); "
...
async def _search(
    ...
    ``kind`` is 'person' or 'company' — resolved to classic v2 or Universal via
    ``_search_url`` depending on ``ROCKETREACH_SEARCH_API`` (classic by default;
    see the module-level note). Credit-free by contract — deliberately no
    ``_meter`` call here.
    """
    key = os.environ.get(_API_KEY_ENV)
    if not key:
    ...
    # existed — no behavior change for the default path.
    if order_by and _SEARCH_API_MODE == "universal":
...
async def rocketreach_lookup(
    ...
    linkedin_url, recommended_email, emails[], phones[], status}``. RocketReach
    intent is company-level only (see ``rocketreach_company_search``'s ``intent``
    facet) — a person lookup never carries a buyer-intent field. On any failure
    returns a ``[rocketreach-error] …`` string — fall back to Vibe
...
async def rocketreach_person_search(
    ...
    ``order_by`` (``relevance`` | ``popularity`` | ``score``) is honoured only
    when ``ROCKETREACH_SEARCH_API=universal`` is set — classic v2 (the default)
    doesn't document it and it's silently dropped. See the module docstring's
    "Universal search endpoint indirection" note before setting that env var.
    ...
    return await _search(
        "person", "profiles", _shape_search_person, query, start, page_size, order_by
    )
...
async def rocketreach_company_search(
    ...
    """
    return await _search(
        "company", "companies", _shape_search_company, query, start, page_size, order_by
    )
