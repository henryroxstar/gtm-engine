"""Entrypoint for the Apollo worker MCP — ``python -m agent.mcp.apollo``.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="agent.mcp.apollo",
        description=(
            "Apollo worker MCP server (contact-resolution backstop: email; "
            "company search with buying-intent filters; job-posting hiring signals)."
        ),
    )
    parser.add_argument(
        "--transport",
