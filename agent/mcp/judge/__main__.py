"""Entrypoint for the Haiku email-judge MCP — ``python -m agent.mcp.judge``.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="agent.mcp.judge",
        description="Haiku email-judge MCP server (downstream per-row send/re-angle scorer).",
    )
    parser.add_argument(
        "--transport",
        default="stdio",
