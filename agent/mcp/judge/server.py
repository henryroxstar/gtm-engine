...
def _rendered(row: dict, touch) -> tuple[str, str, dict, int]:
    ...
    return (
        ...
        int(getattr(touch, "n", 1) or 1),
    )


async def _score_api(
    rows, touch, *, profile, reverse, spec_path, csv_path, repair_attempt, repaired, op
    ...
    async with httpx.AsyncClient(timeout=_HTTP_TIMEOUT_S) as client:
        for index, row in enumerate(rows):
            ...
            subject, body, context, touch_n = _rendered(row, touch)
            payload = {
                "model": JUDGE_MODEL,
                ...
                "temperature": 0,
                "messages": [
                    {"role": "user", "content": _prompt(subject, body, context, reverse=reverse)}
                ],
            }
            verdict = None
            try:
...
async def _score_sdk(
    ...
    for start in range(0, len(rows), SDK_BATCH_ROWS):
        if not budget_ok(profile):
            stopped = (
                f"stopped at row {start}: profile {profile!r} is at or over its monthly cost cap"
            )
            break
        chunk = rows[start : start + SDK_BATCH_ROWS]
        rendered = [_rendered(row, touch) for row in chunk]
        prompt = _batch_prompt([(s, b, c) for s, b, c, _ in rendered], reverse=reverse)
        ...
        for row, (subject, body, _ctx, touch_n), verdict in zip(
            chunk, rendered, verdicts, strict=True
        ):
            records.append(
                build_record(
                    row,
...
async def score_emails(
    ...
    if not calibrated:
        payload["UNCALIBRATED"] = (
            "No sealed holdout exists for "
            f"{profile or '(no profile given)'} — this judge has NEVER been measured against "
            "a human label, so these verdicts have no known error rate. Treat them as a "
