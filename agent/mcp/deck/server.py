...
async def export_deck(slides_md_path: str, format: str = "pdf", with_clicks: bool = False) -> str:
    ...
    Args:
        ...
        with_clicks: pass through to Slidev's own `--with-clicks` export flag — each
            `v-click` step becomes its own page instead of the PDF collapsing every
            reveal onto the slide's final state. Off by default (fewer pages, matches
            prior behaviour); turn on when the deliverable needs the reveal sequence
            itself (e.g. printed handouts that walk a build-up). Passthrough only — no
            new destination or path is representable, same as `format`.
    ...
    # ── 4. Call sidecar ────────────────────────────────────────────────────────
    payload = {"slides_md_path": str(slides_path), "format": fmt, "with_clicks": bool(with_clicks)}
    try:
