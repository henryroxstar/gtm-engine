...
price-parsing sites cannot drift apart, :func:`positive_timeout_env` for the
same reason on HTTP ceilings, and :func:`positive_int_env` for byte and count caps.
"""
...
def positive_int_env(env_var: str, default: str) -> int:
    """Read a positive integer cap (bytes, a count) from ``env_var``, rejecting anything else.

    Caps bound what a worker will open and ship. A zero or negative cap refuses every input,
    which surfaces as "the tool is broken" rather than as the misconfiguration it is; a
    fractional or non-numeric one would crash at the first call instead of at boot. Same
    fail-at-import posture as the two guards above. No upper bound: an over-generous cap is a
    reviewable choice, and the provider's own request ceiling is the backstop.
    """
    raw = os.getenv(env_var) or default
    try:
        value = int(raw)
