...
Python rules (§R1–§R10) referenced throughout live in [`docs/RULES.md`](RULES.md).
...
- **Egress is a small, pinned, enforced set.** The agent's own I/O still flows only through MCP
  tools, but the FastAPI layer itself makes two deliberate outbound calls, both to
  operator-config-pinned (never tenant- or model-supplied) destinations: an external identity
  provider's JWKS key fetch and the FCM v1 push path. No destination is derivable from tenant
  input, and a static-analysis rule scans `backend/**` with an explicit allowlist for those two
  files, so any *new* backend egress trips CI.
