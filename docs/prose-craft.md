...
**9. Flattery-pivot openers.** "Great point, X —", "Strong list — and Y's take is right," "Love this
take, but…" Praising the post (or the person) before disagreeing or adding is a stock reply-comment
tell — it reads as a setup, not a reaction. Cut the compliment; open on the actual point.

**10. Listicle-in-a-sentence.** Two or more parallel ideas stitched together with semicolons ("whether
X…; whether Y…; and Z…") crammed into one sentence and dressed as flowing prose. It is a bullet list
wearing a trench coat. Either make it a real list, or cut everything but the single strongest point.

**11. Abstraction instead of the specific.** Naming a category (a mechanism, "robustness," "the
modality") instead of the concrete thing itself. Broader than the abstract-opener check above — this
can happen mid-paragraph, not just in line one. Test: if any other example could replace it and the
sentence would still read the same, it isn't specific enough yet.
...
A direct quote from a source (an expert, a report, a person) is **evidence, not your prose**. Quote it
**exactly or cut it** — never reword a quote to satisfy a rule here. If a banned word or an em dash lives
inside the quote, that's the source's voice, and editing it fabricates the source. Pick a different
sentence to quote, or paraphrase openly *outside* quotation marks (and then it's your prose again, subject
to the rules).

The linter enforces this: it masks double-quoted spans (straight `"…"` or curly `“…”`) before the
ban-list and em-dash checks, so a flagged word only counts when it's *your* wording, not a quote. The same
word outside quotes is still flagged.
