...
## Read-only views over working state (dashboards + consolidation)

Nothing below is a new source of truth — each is a **derived, regenerable view** computed
...
- **Prospect consolidation** ([`gtm_core/prospects_consolidate/`](../gtm_core/prospects_consolidate/))
  ...
  always merges into it so a partial run never wipes prior accounts.
- **Prospects status model** ([`gtm_core/prospects_dashboard.py`](../gtm_core/prospects_dashboard.py))
  — `build_status()`, the consolidated prospect state (who's ready, who needs a verified contact)
  as a dict. **Model only — it renders nothing.** Its own HTML renderer and the
...
appropriate to its jurisdiction; the repo does not *hardcode* one, since that decision depends on
the operator's own data-protection obligations. The **live single-operator VPS deployment** does
set concrete windows, recorded here so the choice is auditable (this is the retention policy the
security self-assessment's EU AI Act Art. 12 row links to):

| Data class | Location | Retention | Erasure |
|---|---|---|---|
| Audit + cost ledgers (`history.jsonl`, `costs.jsonl`, `runs/`) | `content/<profile>/` | **≥ 12 months** (exceeds Art. 12's 6-month logging minimum), append-only + hash-chained (NIST AU-9; verify with `python -m gtm_core.ledger_verify <path>`) | rotated/archived, not silently truncated — a rotation copies then trims, preserving the chain tail as the new genesis |
| Per-account PII (dossiers, prospect lists, outreach drafts under `accounts/<slug>/`) | `content/<profile>/accounts/` | **reviewed at 12 months**; stale accounts pruned via the per-directory `delete` | one directory removal per account/subject |
| Backups (Hetzner daily VM snapshots; laptop `content/` mirror to the private backup repo) | infra / backup repo | snapshot rotation is infra-owned; the laptop mirror is a manual one-way push | snapshots age out on the provider's rotation; the mirror is overwritten on next push |

**Export** for a portability/access request is the same enumeration described below plus the ledger
files themselves; **erasure** is the `delete` / `delete_workspace` cascade. Neither window is a
code-enforced TTL yet — a scheduled retention-sweep job is a tracked follow-up (`PENDING.md`); today
the windows are operator-run against these documented durations.
