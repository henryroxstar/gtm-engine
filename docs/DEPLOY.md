...
| `HERMES_SCHEDULE_ENABLED` | scheduling's **own** opt-in on top of `HERMES_PUBLISH_ENABLED`; `false` (default) means a `⟦SCHEDULE⟧`-bearing approval dispatches nothing — it never falls back to publishing immediately |
| `HERMES_SCHEDULE_MAX_HORIZON_DAYS` | how far ahead a schedule may be requested (default 90) |
...
See [`CLAUDE.md`](../CLAUDE.md) for the runtime security and tenant invariants a deployment
must preserve, and [`docs/RULES.md`](RULES.md) for the code-level rules that enforce them.
