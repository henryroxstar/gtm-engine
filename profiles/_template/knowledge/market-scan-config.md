...
## Competitor watchlist  *(scan step 1B / 3)*

**Source of truth:** `profiles/<active>/knowledge/competitors.toml` (loaded via
`python -m gtm_core.resolve_knowledge competitors.toml --profile <active>`).

The competitor registry owns the canonical watchlist, tiers, aliases, and
`watch_urls`. `market-scan` iterates the registry for Step 1B and Step 3. Add
fictional placeholder competitors to the template `competitors.toml` and replace
them with real competitors for the active tenant.
